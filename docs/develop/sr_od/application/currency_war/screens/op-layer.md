...
> 本文 = 画面 op 层唯一设计正本:**§1 行为规范**(每个画面 op 必须表现成什么样,合同)、**§2 上报接口**(观察数据怎么进容器)、**§3 形态分型**(35 画面 op 的形态分类)、**§4 辖域边界**(本层不承载的机制)、**§5 观察解析工具箱**(obs/ 地图)、**§6 流程转点观测锚**(在库机制)。各画面的具体文档 = 同目录各篇(一画面一文档,索引 = [README.md](README.md))。代码锚 = `文件::符号名`(路径根 = `src/sr_od/application/currency_war/`)。
...
画面 op 入口
  ...
  │        完成门 miss = 画面已离开,round_success 交回;就绪等待变体门
  │        (在册唯一,WaitOneOne)= 未就绪未超时 = round_wait 等下一帧,
  │        超时 = round_fail(见下方「在册变体」条))
...
- **观察标准化门(用户裁定 2026-09-22,全域行为规范)**:观察内容在所属域存在标准注册数据(名字类:投资环境/投资策略/角色/装备等注册表)时,观察 node 必须在观察时把读数转换成标准注册数据再上报——转换两段:①形变归一(分隔符/全半角等 OCR 形变族)后精确匹配;②不中再 LCS 相似匹配(域级单一实现 `kernel/cw_obs_core.py::lcs_resolve_strict`,阈值常量住各屏);两段皆不中 = 转换失败。**相似匹配(LCS)阶段执行次高分差拒判——最高分未明显高于次高分(差值不足在册常量)= 歧义 = 转换失败,禁直接取最高分**(先例 = 投资环境屏;用户裁定 2026-09-22 全族统一)。**任一候选转换失败 = 观察失败**:观察 node round_fail 早退、零写零上报,交外循环重观察重读(读不准重读,禁带病上报)。多候选命中同一注册名(选项互斥屏)= 识别质量不足以区分,同判转换失败——重复名合法的观察面(同卡多张/名单类)不适用此判,各屏转换成功性的附加边界由屏契约(screens/ 各篇)登记。标准化后下游全链只在标准名上工作:决策消费标准名打分;动作上报只携带选择序号(idx),名字由容器标准名单一源按序号提供,动作/上报层零名字转换。收敛在途:投资环境屏 = 2026-09-22-screen-review 迭代首个落地;其余名字类观察屏为欠账,逐批收敛,禁新增未标准化直报。
- **选择坐标观察上报(用户裁定 2026-09-22,全域行为规范)**:观察上报选项时,除将识别内容归一化到标准注册数据(观察标准化门,本节前条)外,还必须一并上报该选项的具体坐标存入 game state——即选中这类选项的具体坐标;归一化与坐标在同一次观察一并入容器。**坐标单一真相源 = 观察上报**:由决策侧半现算改为观察时上报——策略侧只输出下标,动作 op 根据下标从 game state 获取坐标来执行;命名规约(名字由容器单一源按序号提供,本节前条)的同款思路扩展到坐标,动作/上报层零坐标现算。适用范围 = 商店与各需选择的 overlay。辖域判据(防双义):坐标是否为**本次观察读到的选项集的构成数据**——选项/候选的点击坐标随观察选项集构成、无论布局固定与否 = 本条辖(观察上报,固定槽位域如商店 5 槽/巨星左右同辖);确认钮/刷新钮/开收店按钮等静态控件锚不属选项集构成 = screen_info area 执行期现取现状维持,不入本条。过渡期规则:容器坐标域骨架落地前,新选择面的坐标随其首用屏批建域,禁走 env.target 现算(禁第二坐标源不因骨架未落而豁免)。收敛在途:现役坐标源形态不一(多数 = 决策半现算经 env 传入动作 op;遭遇 = 动作 op 内 area 现取,缺失显式失败;商店 = 动作侧 screen_info 现取),逐批收敛到观察上报,禁新增第二坐标源。
...
- **画面 op 不支持局外单独调用(用户裁定 2026-09-22)**:画面 op 是对局上下文内的组件,不支持脱离对局(`cw_match` 缺席)单独调用调试,此类支持代码不做。不设单跑防御分支;无 match 场景属未支持用法,由上下文缺失自然失败(用户裁决 2026-09-22 全族删门),不设任何兜底决策路径——流程侧兜底决策同时受决策控制分层铁律(本节前条)禁止。
...
- **重入裁决留在决策动作 node 顶部(仅限未迁移屏)**:「确认已发 → 下一轮锚不在 = 落地」是动作落地裁决,属循环出口判定,不回观察 node。chosen_* 类落地记录在此刻写(见 §2 动作事实边界;巨星/伙伴 = 选择点维持(派发前写,各屏文档 §6 在册)、祈愿/星徽等 = 重入裁决点维持,投资两屏已改动作执行时经获得链记,遭遇已改动作侧发射即写,补给确认即写留守选卡分支)。
- **显式读屏只在观察 node**:决策动作 node 迭代用 node runner 进 node 时给的 `last_screenshot`(`round_wait` 每轮新帧)。除观察 node 外,画面 op 内不调 `screenshot()`。**在册例外只有三类**:①**(动作通道)**策略显式发射 `CwActionObsParam(scope='in_place')`(环内重观察)= 经注册表派发 `CwActionObsOp` → 宿主画面 op `reobserve_in_visit`(现役 = `CwScreenPrep`)重跑 heavy 观察链——漏斗直写容器即「重新观察上报」,决策环原地续跑(通道细则 = [../flow/action_exec.md](../flow/action_exec.md) §3);②**(终结臂留证读)**终结动作执行后、交回前的一次机械重读——零决策零判效,读数只作缺陷台账留证输入,不进决策、不判落地(落地仍归重入裁决/观察对账;现役 = `cw_screen_invest_env.py::CwScreenInvestEnv._decide_and_act` 刷新终结臂的刷后帧重读,依据 = 用户裁定 2026-09-14 刷新零效果留证 + unified-action-factory 批4 裁决3 迁观察侧)。③**(编排壳直调机械臂读)** 宿主画面 op 的 bare-loop 决策环直调原子机械函数(无 node runner 轮界帧供给:宿主 `last_screenshot` 取自节点轮首,环内前序动作已改屏,现取帧为该原子动作结构性必需)——函数内读数用途封闭 = 自带幂等门判定(推进型空决策的门判定流程义务,§3 判据;非观察上报、零逻辑态写(容器事实仅动作回执簿记,kernel receipts 唯一写点)、零决策)+ 点击目标定位(瞄准,不是观察,`flow/action_ops.md` §1 增补 3 同口径);落地对账仍归下一帧观察侧,机械交回语义不变。现役成员封闭集 = `operations/cw_op/cw_op_open_shop.py::open_shop`(宿主 op 限 = `CwScreenPrep`);宿主语境封闭清单 = 该成员的全部生产直调点,编辑时点现值一处——`cw_screen_prep.py::CwScreenPrep._open_shop_phase`(显式开店);新成员与新宿主语境先登记本条再落码,禁类推扩员。新增读屏点必须先登记本条再落码;除上述三例外,其余路径零读屏。
...
- **每个动作 op 单独一个文件(用户裁定 2026-09-22)**:动作 op 一 op 一文件,禁多个动作 op 同居一个文件——与上报侧 `kernel/cw_action_report/<action>.py` 每动作一文件对称(§2.1);禁新增同类同居。
...
- **合法性判定全部内化于策略器**(提议侧约束);执行侧只余**守卫断言**——防 bug 路栏而非控制流分支,非法返回 = 策略器 bug 响亮暴露(禁静默跳过或降级续跑)。现役守卫 = `operations/cw_op/cw_shop_action_ops.py::guard_proposal_vs_expected`(提案对象在期望态存在且未被消费——防策略器算术 bug,恒炸)。守卫登记(逐屏条目,新增屏随其批补录):
  - 命运卜者:`operations/cw_screen/cw_screen_fortune.py::CwScreenFortune.act` 决策出口——守卫①:decide_fortune 返回词表外/None = 具名 round_fail 零盲发(§1.1 出口③);守卫②:pick.idx 越界 = 守卫断言 AssertionError(本节)。
...
- 画面 op 按观察/决策/上报三面的有无分型(全形态/多屏管线/节点循环/推进型空决策/画面框 op/驻留状态机/重型屏),判据与 35 屏分类总表 = §3。
...
画面 op 共 35 个:`operations/cw_screen/` 33 类 + 画面框 2 类(`operations/cw_op/cw_op_open_shop.py::CwOpOpenShop`、`operations/cw_op/cw_op_settle_confirm.py::CwOpSettleConfirm`;`CwOpCloseShop` 编排壳已退役——关店执行位收编进动作 op `CwActionCloseShopOp` 真机械执行,见 §4 动作面。原 CwScreenEquipPick 随选择装备屏误判退役删除,2026-09-22)。分七型:

| 型 | 判据 | 屏清单 | report |
|---|---|---|---|
| **全形态** | 观察 node + 决策动作 node + report;决策动作 node 承载完整决策循环(重入裁决/分支刷新/确认链)——其中简报/BOSS 简报/位面过渡/等待 1-1/未达上限弹窗/简易武装箱六屏为**空决策变体**:决策动作 node = 重入裁决 + 固定推进,零策略器问询(与各屏文档「空决策形态」声明同义;归本型仅因两 node + report 齐备;其中等待 1-1 = §1.1 在册就绪等待变体,观察轮询口径与边界单一源见彼条) | CwScreenEncounter(刷新终结臂 + 选卡派发即终结;遭遇扩围批)、CwScreenSupplyNode、CwScreenInvestStrategy、CwScreenInvestEnv、CwScreenBriefing、CwScreenBossBriefing、CwScreenWaitOneOne、CwScreenDeployNotFull、CwScreenPlaneTransition(链观察)、CwScreenBoxPick、CwScreenArmoryBox | 11 屏全设;其中 WaitOneOne/DeployNotFull/ArmoryBox 现役零容器摄入面,接口为统一形态占位;BossBriefing 为真实写面(BOSS 类型直定;写语义单一源 = `kernel/cw_screen_report/boss_briefing.py`) |
...
| **推进型空决策** | 无选择面无容器域:观察 node = 门判定;决策动作 node = 单步推进 + 重入裁决;**无 report 接口** | CwScreenNextButton、CwScreenPlaneDetail、CwScreenConsumableOverlay、CwScreenEmblemDetailPopup、CwScreenItemDetailPopup、CwScreenInterruptDialog、CwScreenRefreshOddsPopup、CwScreenRoleDetailOverlay、`cw_screen_shop_card_detail.py::CwScreenShopCardDetailPopup`、CwScreenPrepLockedReturn、CwScreenAhaEquipPick(11) | 无(obs 类只记入口裁决;完备锁断言函数不在场) |
| **画面框 op**(用户裁定 2026-09-22 入表) | 不入常规分发(无 CwAction param,不经动作注册表与画面分发漏斗)、随宿主流程构造直驱(生产路径 = 宿主画面 op 委托 `execute()`);形态各自在册 | CwOpOpenShop(推进型只读/导航两 node 变体;直调形态 `open_shop` 的读屏例外 = §1.1 例外③)、CwOpSettleConfirm(单 node 驻留,宿主 = `CwScreenBattleWait` ②段委托;推进上报 = `report_node_advance(trigger='settle_confirm')`,节点推进上报族例外 = §2.1)。(CwOpCloseShop 编排壳已退役——关店执行位收编进动作 op `CwActionCloseShopOp` 真机械执行,见本节尾退役注 2。) | 无(OpenShop = obs 只记入口裁决;SettleConfirm 推进上报走 `report_node_advance`,不经 report 接口) |
...
| `cw_megastar_obs.py` | 巨星观察域:候选检测(弹窗标签带限定 OCR 定卡 1..N + 逐卡立绘 SIFT 交叉验证,xy = 该卡标签中心 + 在册卡身线随卡自带)+ 标准化失败裁决门(规范 = §1.1「观察标准化门」) |
