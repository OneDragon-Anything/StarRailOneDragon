...
|---|---|---|---|---|
| P1 早期(1-1~1-3) | 冲 Lv4 | saving | 否 | 纯升级;买低费过渡保血(过渡牌见 03 `transition_chars`,阵容参数非骨架);不主动 D;利息不重要(钱少),有余就升 |
| P1 中期(1-4~1-6) | Lv5-6 | interest | 是(A8 遭遇前) | 买 2-3费过渡;攒金到 50(**存 50 吃满 5息**是引擎);A8+ 战力不足+遭遇前 → 弃息 D 保血(保血 > 保经济) |
...
- `transition_chars`(早期打工牌)+ `shared_chars`(转型复用)。
