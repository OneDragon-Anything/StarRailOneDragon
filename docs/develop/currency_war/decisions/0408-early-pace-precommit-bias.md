...
- **Status 补记**:由「默认关、通道保留」翻转为**已删除**(删码留
  ADR;策略开关生命周期第 4 态兑现)。删除理由只引本 ADR 定谳:
  三窗 outcome 无一致正方向(配对出口 hp 差 −0.41/−0.26/+0.22 符号
  不一致),机理核 r7/boss EΔ 两臂几乎不动——偏置只顶进约束链、
  未改变 rung 成型路径,W248「投资前移→rung 上移→减伤」链在
  bias=5.0 单轮剂量下不成立;与 ADR-0305 goldrich 否决同构
  (成型加速可见,hp 不跟)。无操作化开臂判据,保留即悬置。
- **删除清单**:registry 五字段(`early_pace_enabled/min_round
  /max_round/bias/val_max`)+ scoring 偏置消费块与 `bd['early_pace']`
  触发依据记录 + 对应锁组(test_cw_w251 全文件移除;adr0293 字段面
  五条目随批收敛);清理后语义守卫见 test_cw_dead_arm_cleanup_locks。
- 复活条件:高损耗局分化假设获得新的机理证据(如 rung 路径对投资
  时序的响应实证),按本 ADR 预注册口径重新开臂,不复活加性偏置
  默认形态。
