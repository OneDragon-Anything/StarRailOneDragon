"""入口链难度确认屏的选职级缺省判据(策略内容迁移:判据单一源居 kernel)。

「选哪个难度」属策略选择(策略内容普查分界判据 §2.1「在多个候选间做选择」
明列「哪个难度」;普查裁决 F-D-OPS-01 = CONFIRMED,处置 = 判据迁 kernel、
行为保持):缺省策略 =「恒选最高职级」——难度确认屏「按钮-返回最高职级」
