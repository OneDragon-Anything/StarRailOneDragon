...
source: WebSearch 攻略 + `application/echo_of_war/` 代码 + `guide` / `challenge_mission` screen_info + check_power fixture
involves_screens: [星际和平指南, 挑战副本, 战斗画面]
---
...
- **待实拍画面 + vision**:`_use_power` 链路的历战 boss 选择 / 战斗 / 奖励 / 弹框实拍归档 ——
  周限 3 次 + 消耗体力,待用户配合切画面(到挑战按钮不点「开始挑战」可采部分态)。
