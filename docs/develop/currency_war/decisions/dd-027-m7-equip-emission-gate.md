...
## 5. 修订(2026-09-04:发射序回排,同帧「开店 ∧ 可穿件」漏发)

实机局 g_20260904_010335 首验发现门②合格但门①「漏发」:1-6/1-7 持可穿件
轮滑鞋未发 RunEquip,装备滞留 2 个备战期,1-8(该帧无开店意图)才首穿。
帧数据定谳**推翻「快照依赖环」假设**——1-6 决策帧(01:17:08)state.equips
已含轮滑鞋,快照经 overlay 确认链(PickBoxCard/ConfirmSupply 等,
cw_expected_state)及时推进,门①谓词正常开火。真根因 = **发射组织面与
帧稳定截断的相互作用**:M7 在执行序末位评估,发射落在同帧 M2/dominance/
M6 的开店意图(OpenShop=契约 §3.2 截断点)之后,truncate_frame_stable
其后必截,RunEquip 被静默丢弃;而门②闩在发射位已消耗 ⇒ 后续帧全部
equip_latch_skip,装备滞留整个备战期。与 R197 症1(EV 卖面被截断丢弃)
同类,根同发组织面:帧稳定契约要求发射序中可执行动作不落截断点之后。

修法:run_mandate 末尾发射序回排——RunEquip(可续类,装备 pass 画面零
迁移)插到首个截断点/终点之前,两动作均保留,执行序(先穿后开店)与
发射序一致;门①谓词、闩置位时机(发射=置闩)不变。回归锁新增
TestM7EmissionOrder 3 用例(发射序/截断后双动作存活/闩恰消耗一次)。
