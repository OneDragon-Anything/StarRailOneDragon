...
- **锁 A**(合格集语义,`test_cw_t88_r1_qualified_set`):混合集(可追+不可追)⇒ 账有限;三变异探针(全不可追/全 2★ 成型/成员集空)⇒ `(inf, 0)`;F7 成型先序探针封「重排判定回流」回归路。红证(修复前亲跑):真注册表体系对 15 名 lv3-6 得 `(inf, 21/42/78/78)`——账无穷同时卡费为正。
