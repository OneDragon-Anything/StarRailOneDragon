...
- **位面过渡:每个位面开始时出现一次**(用户确认 2026-09-02)。
- **位面简报出现时机(⚠️ 口述与实证冲突,待玩家确认)**:用户口述「位面简报只在货币战争入场时出现一次;接管未结束对局是否出现待验证」——但代码实锤(battle_loop 0a0b 分支,局54 哨兵 30 iter stall)**P2/P3 开局前出现过简报屏**(三 boss+词缀+「下一步」,与 P1 开局简报同建档「货币战争-简报」)。保守处理:白名单双锚保留(位面简报/位面过渡都列,见 05-battle),位面简报出现就走 BriefingOp;**待玩家实机确认 P2/P3 简报是否存在**,确认不存在则删该锚与 BriefingOp 的位面变体。
- 编排(循环分发式,对抗轮 1 终检修正——不写死顺序):结算完成交回循环 → 循环识别:位面简报[若出现]→ BriefingOp;位面过渡 → PlaneTransitionOp;2-1 备战商店自动开 → 交备战循环。打完 boss(1-6/2-6)后没有投资环境。
