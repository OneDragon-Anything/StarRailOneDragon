...
- **并行协调申报（joy-conditional-leg 批，attack3 F4 定稿事实）**：rider 已随 commit 14dd40977 提交（现两相门形态 + `test_cw_gain_chain.py` 10 处 evidence 调用）。收口序 = ①joy 批提交 ✓（已完成）→ ②本批 3.2 单相化时 rider 挂点随调用点逐位保持迁移、`test_cw_gain_chain.py` 仅做删 evidence kwarg 机械改写 → ③rider 腿行为锁仍归 joy 批验收面。
