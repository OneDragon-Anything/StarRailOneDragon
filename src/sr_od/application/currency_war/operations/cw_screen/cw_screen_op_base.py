...
- 并存纪律(§9.1):装配点(观察/动作端口模块槽 ``observation_source``/
  ...
  生产行为零变化;等价门(§9.1 主门)通过前双路径并存,旧路径退役归
...
#: 在册唯一成员 = 遭遇刷新计数 encounter_refresh_used(批次三件⑦):现役
#: 语义 = cw_screen_encounter 刷新链「发出点击即置位,防点偏未生效重入屏
#: 反复尝试」(验效失败帧仍 +1)。该屏 op 尚未迁移(§9.2 迁移序在事件屏族),
#: 先以申报面登记占位;迁移批接线时消费本表,禁静默新增发射型成员。
EMIT_TRIGGERED_DECLARED: dict[str, str] = {
    'encounter_refresh_used': (
        'CwScreenEncounter 刷新链(架构设计 §6.4 触发时点轴唯一在册成员;'
        '证据词 refresh_click,验效失败帧仍 +1)'),
}
