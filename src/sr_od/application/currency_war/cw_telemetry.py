...
def derive_xp_per_click(replay_dir: Path | str = DEFAULT_REPLAY_DIR,
    ...
    样本 = 同 (plane, round) 内「带 LevelUp 的行 → 本轮其后最近一行」:
    期望后态 = ``cw_state.xp_apply_clicks``(结转语义 ADR-0129)。对
    rate ∈ 1..max_rate 逐一试配,**全样本一致的交集恰为单值**才判 rate;
    无 LevelUp 的相邻行差(外生经验流)不进样本。跨级样本(点击触发升级
    结转)天然参与判别。
    ...
    for i, r in enumerate(rows):
        lu = [a for a in (r.get('actions') or [])
              if isinstance(a, dict) and (a.get('__type__') or '') == 'LevelUp']
        if not lu:
            continue
        st = r.get('state') or {}
        xp0 = st.get('xp_progress')
        lv0 = int(st.get('level') or 0)
        if not xp0 or lv0 <= 0:
            continue
        for j in range(i + 1, len(rows)):
            r2 = rows[j]
            if (int(r2.get('plane') or 0), int(r2.get('round_num') or 0)) \
                    != (int(r.get('plane') or 0), int(r.get('round_num') or 0)):
                break   # 出段仍未找到后读帧 → 该意图无同段读数,弃样本
            st2 = r2.get('state') or {}
            xp1 = st2.get('xp_progress')
            lv1 = int(st2.get('level') or 0)
            if not xp1 or lv1 <= 0:
                continue
            samples.append({'plane': r.get('plane'),
                            'round': r.get('round_num'),
                            'clicks': len(lu),
                            'before': [lv0, xp0[0], xp0[1]],
                            'after': [lv1, xp1[0], xp1[1]],
                            'rates': [rt for rt in range(1, max_rate + 1)
                                      if xp_apply_clicks(lv0, xp0[0], len(lu), rt)
                                      == (lv1, xp1[0])]})
            break
        # 同轮内 LevelUp 行其后只取最近一读(多次单元各成对;相邻 LU 行
        # 自然形成链式样本,不跨对重复计数)
    valid = [s for s in samples if s['rates']]
    inter: set[int] = set(valid[0]['rates']) if valid else set()
    for s in valid[1:]:
        inter &= set(s['rates'])
    rate = next(iter(inter)) if len(inter) == 1 else None
    return {'rate': rate,
            'matched': len(valid),
            'total': len(samples),
            'anomalies': [s for s in samples if not s['rates']],
            'samples': samples}
