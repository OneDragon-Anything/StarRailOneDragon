...
**引擎补完守卫**(ADR-0371,「拥有≥门槛却从未同时上场」修法,A/B 通道 `registry.evolve_engine_completion_enabled`):`evolution_step` 在常规提案枚举**之前**——pair 体系集(p1_pair ∪ transition_pair;希儿系=单卡判据)某体系 owned(bench∪deployed,全羁绊口径)≥ tier ∧ on-board(board_factions 口径)< tier 时,发补完事务(`_engine_completion_tx`):bench 该体系成员上场(同名已在场剔除),room 不足 undeploy 最弱**非保护**件(保护集=pair 成员 ∪ 引擎件 ∪ 锁定目标件 ∪ 种子窗;引擎件自 ADR-0375 起四体系全辖——含希儿系贡献件,`guard_seele_scope_enabled` 注入,**核心条件辖**(希儿恒保护;放大器件仅当希儿在手时保护),补完 undeploy 与 `execute_replacement` 保留序两面同辖),bench 容量不足 sell 最弱非保护件腾位;被拒退避(独立签名)与遭遇/boss 冻结轮纪律同既有;**末窗冻结豁免**(ADR-0363 件2 的同向例外):净效果 pair on-board 逐体系不减 ∧ 总引擎数不减(构造保证+事后复核 `_completion_freeze_exempt`)——补上不是拆。语义:守卫护「在场不被拆」(ADR-0363),本守卫补「拥有必上场」([20] 件上场才算配方;[13] 成型缺口=发令枪级)。
