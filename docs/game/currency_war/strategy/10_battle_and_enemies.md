...
- **"掉血=收益"web(更广)**:燃血 comp + 投资策略「星际和平保险/保险/先亏后盈」(按已损失生命给金/装备,../data/investment_strategies.md)—— 燃血队天生掉血,这些策略额外补益。

机制名跨版本稳;具体 boss/词缀属哪个机制随版本变(随 ../data/competitors.md 实机 OCR 更新)。比"boss 名→counter 阵营"名字查表更鲁棒。**数据需游戏**(实机 OCR 敌人机制/词缀;`enemy_affixes` 字段,见下)。
