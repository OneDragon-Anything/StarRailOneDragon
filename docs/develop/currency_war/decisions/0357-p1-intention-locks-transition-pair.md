...
- 关联:W143 主灶诊断(第二引擎获取链断裂)、0225/0243(cw_recipe 过渡配方一等公民)、
  ...
  0616(配方对方向判据四层谓词——本件决策 2 派生前提证伪与 [23] 冻结扩展的修订出处,
  见文末修订节)
...
## 修订(2026-09-10,ADR-0616 三同步清偿)

本件决策 2 的两处表述已被 ADR-0616(配方对方向判据四层谓词,落码 T-166 批1)修订。
旧文本按决策记录保真保留,现行语义以本节与 ADR-0616 为准:

1. **「体系对随资产重派生、支持度只增,非 pivot」前提证伪**:卖出降计数(T2)与
   合成降计数(T3)两形态下支持度真实下降(sim 实证,ADR-0616 §3);逐帧无记忆
   纯重派生即切线摇摆的根。派生语义由四层谓词取代:门槛维过滤先行(合格集)→
   在任优先单席易手(严格大于,平手保持)→ 排除谓词合成中性计票 → 可行性门
   (E≤R_rem 后置合取,门拒保持原对);落码 = kernel `cw_intention` 的
   `_derive_p1_pair`/`_p1_pair_eased`/`_p1_fuel_excluded`/`_p1_system_ordering`/
   `_p1_pair_gate`。这是谓词结构条件消环,不是滞回参数复活。
2. **[23] 冻结语义扩展**:原「冻结语义辖终局线不辖 P1 配方」扩展到配方对层——
   form_ok 达成即事件闩冻结方向(解冻闭集分域申报,ADR-0616 §2.2)。
3. **其余条款零变化**:P1 锁定产物=过渡配方体系对、囤货方向=体系对成员集、
   ①类直通资格通道、进 P2 清 `p1_pair`、P2+ 锁线机器照旧;门槛常量现行值以
   代码注册表(`P1_PAIR_LOCK_MIN_SUPPORT`)为准,本件行文数值不作消费依据。
