...
- 违反 token 效率 / progressive disclosure 原则(指令式 compact + 按需加载)。
...
## Considered Options
1. **保留单一大 SKILL.md(~190 行)**:简单,但 always-on token 重 + 结构无主脊。
...
- 跨阶段不变量(证据纪律 / 知识维护 / 调试纪律)留 SKILL.md always-on —— 它们**每次都用**,即使长也该内联(核心但长 → 留 SKILL.md,拆出去每次再读更费)。
