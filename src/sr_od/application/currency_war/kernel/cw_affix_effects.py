...
EQUIP_REWRITE_DECLARATIONS: dict[str, str] = {
    ...
    '精密拆装扳手': '金面:重复获得拆装扳手改+1金(确定性→逻辑写;写端=贡献算术'
                 ...
                 '既有写端(CwActionSellBenchParam 装备转移链,流向锚=cw_equip_env)',
    ...
    '员工投影仪': '单位面:拖拽→备战席该角色1星复制进席(确定性→逻辑写;官方前置'
               ...
               '拖拽回执时点窗口独占;现役原子通路消费真值归观察,写端接线候工具'
               '上报形态)',
    '完美投影仪': '单位面:同员工投影仪(无费用门;确定性→逻辑写,写端=桥 spawn_'
              'equip_bench_unit 无门形);现役原子通路消费真值归观察,写端接线候'
              '工具上报形态',
    ...
    '特权赋予卡': '装备面:拖拽后进阶装备变对应特权装备/角色已穿进阶装备随机一件变'
              ...
              '特权化桥 transform_worn_equip_to_privilege;现役原子通路消费真值'
              '归观察,写端接线候工具上报形态)',
