...
派子 agent 对着玩法自由发挥+**系统面清单驱动**(不只随机域:决策架构/数据链/结算层/供给层/观察层逐面过,主动想未出现的问题)。**交付完整性 = 指纹 + 可运行的检查项**(2026-08-23 用户定调「问题较大的应该留下回归」——压测官自己把检查项写进检查网,「设计归 worker」曾致 123 设计 94 积压的实证;测改分离边界=检查网可改,策略/sim 语义仓不可改)。**框架级症状纪律**:同类问题出现 ≥3 次(如振荡族/通道互踩族)= 必须上升为架构审查立案,不再点修。排程纪律:常设在飞,批间零空窗;心跳=点名制(进度树点名册节)。
