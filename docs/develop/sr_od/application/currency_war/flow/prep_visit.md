...
run()
 ...
 │    → _observe(heavy=True)（期望态重建的唯一读屏点 = 对账,决策 8）
 ...
 └─ ③④⑤单动作决策循环 for _vi in range(VISIT_ACTION_CAP=16)（循环内零读屏）：
      ...
      → 投影 _project_prep_obs（纯计算零读屏）:已建模面（OpenBox/OpenTome 腾席/
        ...
        RunEquip）→ None = 保守回退:本访问终结交回外循环重观察（重观察语境禁猜;验证阶梯 = 遗留节）
...
- （旧备战骨架的位面字面门 `round_num >= 9` 与息引擎门双源漂移两项 ⚠️ 随 迁移批死码清除消失——载体 `_is_boss_round`/`_levelup_engine_ok` 已删,见 §2.1。）
