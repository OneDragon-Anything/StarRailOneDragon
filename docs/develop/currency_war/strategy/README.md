...
| 位面 2 支出授权 | `decision_v2/p2_spend_auth` | P2 中段两通道支出授权谓词与预算带单一源:通道 A 定向授权(T1 线锁定[消费 P16 滞回后 v3 locked 位]∧T2 形态缺口[form_ok 锁定线分支取反]∧T4 g≥息线;T3 连败/hp 警戒带升格加急层)+通道 B 止血授权(T3∧T4∧帧资格即触发,与锁定位/形态缺口解耦;方向梯级=hoard→桥池→[31]① 空窗填充,投影失败帧禁整库保守域,纯散件恒不授权,bench 保留 ≥1 空槽不足降额);预算带对齐=常授权层无独立地板(与既有息账门同判「只开门不收门」),加急层破息至保留金下限;消费点=arbiter 金地板加急臂/息账加急豁免/非正分门核心必买,候选评分零触碰(P12 j=0 负例与升级三判据不动);交界=接管帧(分配器 v6 active)禁出手(末窗豁免收窄归此谓词)、economy 存息姿态让位序、血预算门(ADR-0448/0451)=discipline 层不受让位影响;拦断面普查=逐帧拦截原因枚举经 session 披露键进 telemetry 决策迹(协议 M0 分层消费);开关 `p2_spend_auth_enabled` 默认关,开臂=协议 V3 双钥匙(P25 证成∧M0a/M0b/M1-M3 达标护栏全绿,见 ADR 挂账) | [0480](../decisions/0480-p2-spend-authorization.md)/[0481](../decisions/0481-p2-spend-auth-v31-two-channel.md) |
