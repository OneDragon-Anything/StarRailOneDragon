...
class CurrencyWarRunLoop(SrOperation):
    ...
    #: 结算页1 掉血说明 tooltip 渲染延迟门:面板进页 ~2-4s 才出现(遥测验证批
    #: h_013/h_014 成对帧实证:进页 ~2s 无面板,~4s 完整出现;离线复跑同帧解析
    #: 全对 → 漏读纯系时序)。效率批「点空白加速」把页1 驻留压到 ~1-2s,读点帧
    ...
    #: 开局序列画面判定集(W971 P3b,01-opening §2.1 首帧分流门):
    #: (screen_info 画面名, 独有 id_mark/锚 area)。命中任一 = 首帧仍在开局
    #: 序列段 → 交 OpeningSequence;全不命中 = 常态画面/未知帧 → 跳过序列。
    OPENING_SEQUENCE_FRAMES: ClassVar[tuple[tuple[str, str], ...]] = (
        ('货币战争-简报', '标识-本场对局首领'),
        ('货币战争-位面过渡', '提示-点击空白继续'),
        ('货币战争-投资环境', '标识-投资环境'),
    )

    def _frame_in_opening_sequence(self, screen) -> bool:
    ...
    def _record_round_outcome(self, screen, telemetry_only: bool = False) -> None:
        ...
        **仅从分支3(按钮-继续挑战 = 已确认 round-end 结算屏)调用**,故无内部结算屏 gate —— 原 gate 查
        ...
        if self.ctx.cw_match is None:
            return
        # ADR-0250:见结算屏 → 战斗窗口关(watch 恢复)。telemetry_only 跳过
        ...
        # 循环状态,保持新路径零状态面;残留屏按定义是「胜结算续帧」,killed 门也会拦。
        ...
        try:
            ...
            # telemetry_only 的 killed 门:只落「显式败局」行(killed=False,由屏文本
            # 判定:挑战失败团灭 / 负进度轮败)。1f 的模板门理论上只挡败局页,但
            # 位面通关过渡页等同形屏误入时 killed=None/True → 不落行,防伪行进语料。
            ...
            self._settle_p1_ts = None   # 面板延迟门计时随本轮暂存一并清(下轮新值)
            if not telemetry_only:
            ...
            log.info('[cw-loop] on_round_end plane=%s round=%s hp_after=%s conf=%s comp=%s node=%s%s',
    ...
    def loop(self) -> OperationRoundResult:
        ...
        #     → PartnerOp(W971 P3b overlay 分发接管;委托现役 handler,详见 op)。
        ...
        #      机制见 docs/game/gameplay/currency_war.md 银狼策划事件节)→ PlannerEventOp
        ...
        if self.round_by_find_area(screen, '货币战争-骇入策划', '标识-我来当策划', crop_first=False).is_success:
            _r2 = PlannerEventOp(self.ctx).execute()
        ...
        #      策划系事件族:标题+三卡+Q详情+确认,布局同策划事件)→ FortunePickerOp
        ...
        if (self.round_by_find_area(screen, '货币战争-命运卜者强化', '标识-命运卜者', crop_first=False).is_success
            ...
            _r3 = FortunePickerOp(self.ctx).execute()
        ...
        if self.round_by_find_area(screen, '货币战争-盛会之星', '标识-盛会之星', crop_first=False).is_success:
            ...
            _r = MegastarOp(self.ctx).execute()  # 生命周期 owner:验证 overlay 消失,超预算 bail
            if _r is not None and getattr(_r, 'success', False):
        ...
        if self.round_by_find_area(screen, '货币战争-投资策略', '标识-请选择投资策略', crop_first=False).is_success:
            ...
            return self.round_wait(wait=2)
        # 开局投资环境段已退役(W971 P3b,01-opening §2):投资环境仅开场一次
        # (#11:开场 1-1 前弹,1-3 后局中只弹投资策略,两画面不同 handler),
        # 开局段由 OpeningSequence 统一编排( InvestEnvOp 步),主循环不再分发。
        ...
        # 0h. 祈愿试炼 overlay(节点级 quest 选择:选1试炼 → 完 objective 得奖励)→ WishTrialOp
        #     (W971 P3b overlay 分发接管;点第1卡 + 确认选择)。叠备战上挡备战分支 →
        #     必须在备战(1)前检测。2026-08-08 实跑发现:bot 卡此 overlay 68min(购买经验
        #     透出命中 → BattlePrepCycle 误派 → shop 被遮失败 → 死循环)。ESC 不关;
        ...
        if self.round_by_find_area(screen, '货币战争-祈愿试炼', '标识-祈愿试炼', crop_first=False).is_success:
            ...
            if _hook_r is not None:
                return _hook_r
            _rw = WishTrialOp(self.ctx).execute()
        ...
        #     开启后弹四选一星徽 → BookcardOp(W971 P3b overlay 分发接管,选卡读法按
        ...
        if _1f_tpl_hit and _1f_sign != 'pos':
            ...
            # 面板渲染延迟门(见 SETTLE_PANEL_WAIT_S 注;1f 是失败结算页主通道,
            # 过早补录 = damage 全 None——遥测验证批败轮行全 None 同根因)。只门
            # 页1(「点击空白加速」在场);后续「下一页/返回」页无面板,直接放行。
            _pnl = read_settle_damage_breakdown(self.ctx, screen)
            if (self.round_by_ocr(screen, '点击空白加速').is_success
                    and not _pnl['visible']):
                _now = time.monotonic()
                if getattr(self, '_settle_p1_ts', None) is None:
                    self._settle_p1_ts = _now
        ...
        # 开局编排接线(W971 P3b,01-opening §2/§2.1):run 首帧若仍在开局序列
        ...
        # 2. 点击空白加速(结算页1/强敌来袭横幅;位面过渡已上移 PlaneTransitionOp
        #    分支,不再与本分支混点)→ 点空白
        ...
        if _accel_hit.is_success:
            ...
            # 独立 try:progress OCR 异常不连带面板读取(原整块 except pass 是
            # 遥测验证批疑似漏读源之二)。
            try:
            ...
            # 面板渲染延迟门(SETTLE_PANEL_WAIT_S 注):只门结算页1(「点击空白加速」
            # 在场);横幅页(「点击空白处继续」)无面板,照旧立即推进。放行条件 =
            # 面板本帧 visible(提前放行)或自页1 首见超时(胜轮面板整块不出现的兜底)。
            if _accel_hit.is_success and not (_panel_now is not None and _panel_now['visible']):
                _now = time.monotonic()
                if getattr(self, '_settle_p1_ts', None) is None:
                    self._settle_p1_ts = _now
                if _now - self._settle_p1_ts < CurrencyWarRunLoop.SETTLE_PANEL_WAIT_S:
        ...
        if self.round_by_find_area(screen, '货币战争-结算', '按钮-继续挑战').is_success:
            ...
            # 「用户口述过场动画时序」#6/#25,2026-09-02):「继续挑战」按钮出现
            # 后金币奖励等结算数据才显示(初测 0.5s,用户复核裁定 **1.5s 更稳**)
            # ——锚命中帧立即读 = 打在空面板上(交接文件
            # REAL_MACHINE_EFFICIENCY_HANDOFF.md 遥测 damage 0/15 事故的时序
            # 根因;页1 有 SETTLE_PANEL_WAIT_S 渲染延迟门,本页此前无同款保护)。
            # 等 1.5s + 重截,读点落在数据已渲染帧;采集钩子仍抓首见帧(时序
            # 价值在动画帧)。
            time.sleep(1.5)
            screen = self.screenshot()
