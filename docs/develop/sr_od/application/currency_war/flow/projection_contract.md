...
| 层 | 载体 | 角色 | 关键契约 |
|---|---|---|---|
| 状态面板 | `kernel/cw_game_state.py::GameState` | 决策域局面模型（OCR 填充 + bot 跟踪）= **策略器唯一输入** | `bench` 定长 9 槽表(BenchView,BenchSlot.kind ∈ unit/supply_box/tome/bookcard/empty)、`deployed` 定长 10 槽表；卖出/下场置 None 不移位；晶矿域 OreSight.points 载点击坐标 |
