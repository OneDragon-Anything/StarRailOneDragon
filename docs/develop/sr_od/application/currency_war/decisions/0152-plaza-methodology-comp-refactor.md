...
- **加注(2026-08-25,ADR-0321)**:本 ADR Decision 七条不变、不标 ↺;但其方法论
  (逐篇精读→统计)的下游产物中 **form_tiers 类数据点**(如万敌条「夜2+燃2」实证)
  适用域由 [ADR-0321](0321-form-tiers-authority-ruling.md) 重划——设计判断域字段
  (成型判定下限)不再具有直接写入权,实证值仅用于校准上限/主流区间/行为期望。
