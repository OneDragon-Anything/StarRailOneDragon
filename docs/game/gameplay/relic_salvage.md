...
- **空跑场景(2026-09-15 实证)**:过滤后无可分解遗器时,「按钮-分解确认」不出现(op 重试未找到 ×4)→ 应用走「返回普通大世界」兜底正常收尾,画面停在 背包-遗器(带选中详情面板,fixture `screens/背包-遗器/选中详情.webp`)。此为正常空跑,非故障;当日该兜底路径撞上跨屏误匹配(角色信息提示单锚误命中,已修)才演变成停机,详见 bag.md「跨屏误匹配加固」。
