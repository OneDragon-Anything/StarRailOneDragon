...
> 分工分界:分发判定(两阶段身份分发)的单一源 = [../flow/outer_loop.md](../flow/outer_loop.md) §2(本目录各篇只写画面特有的排他/穿透形态与身份锚说明);画面 op 统一规范(两 node 形态/round_wait 循环/终结 op/期望态生命周期/对账边界/report 上报接口)= [op-layer.md](op-layer.md);逐屏形态分类总表(35 屏)= [op-layer.md](op-layer.md) §3;动作执行契约 = [../flow/action_exec.md](../flow/action_exec.md)。
...
| 领取类 | `CwActionCollectOreParam`/`CwActionOpenTomeParam` | 同名 `_action.py` op(`cw_open_tome_action.py` 等) | 非终结 |
| 领取类(开书册卡) | `CwActionOpenBookcardParam` | 同名 `_action.py` op(`cw_open_bookcard_action.py`) | **访问终结**(§6) |
...
| 点晶矿(区域-奖励) | 晶矿飞行动画 ≤2s(`research/screen_flow_timing.md` #16) | `CollectOre`(批式:一次全点→固定等待 2s→自上报交回;席满未点开的晶矿由下一帧观察回补;席满让路门 = `strategies/impl/mandate_v1/entry.py` 席满探针段) | 备战期 | 非终结 |
...
| 逐卡/整屏刷新重掷 | 刷新后 ~2s 稳定(时序 #13/#19/#23) | 投资策略逐槽(观察 obs 携带逐卡余量 + `safe_click` 留守臂)/ 遭遇(`cw_screen_encounter.py` 刷新终结臂,容器 `encounter_refresh_left` 剩余闸)/ 补给(`cw_screen_supply_node.py`「剩余次数」文本锚定,容器 `supply_refresh_left` 剩余闸);建议刷新 = `PickEvent.refresh`(`kernel/cw_events.py::decide_event`,P81;策略屏逐槽 = `PickEvent.refresh_slots`) | **刷新 = 终结为全域规范**(正本 = [op-layer.md](op-layer.md) §1.4):投资环境整组重掷、投资策略逐卡、遭遇、补给(点钮 + 2s 后 round_success 交回外循环重进 = 入口重建)全链终结语义,闸全域 = 剩余语义观察真值(`*_refresh_left`,禁已用推算);确认离开才画面终结 |
...
| 备战 OpenBookcard | **访问终结** | 开卡 = 专家邀请函弹窗在场 = 新事实,交回外循环按该画面分发选卡(`terminal_wait=1.8`;发射位 = 策略器 entry ① prep 实体面卡片臂) |
