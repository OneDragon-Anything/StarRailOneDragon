...
12. **停机钩子降级为留证采集钩子(W209i,run 27 停机事故第二层实证,
    用户指令「hook 根本没停下来,好好修复」)**:run 27 hook 停机后**画面
    没保住**——游戏自行从 2-7 备战推进到首领战败结算(14:09 停 → 14:16
    结算截图 20260826_141613)。根因 = **货币战争备战阶段实时倒计时,战斗
    自动开打——停 bot ≠ 停游戏**,「停机保画面待采集」对实时制游戏是虚假
    承诺(run 26 崩坏局的「画面停在现场」是因为用户热键手停恰逢非战斗态,
    不是 hook 的功劳);且误停代价(烧一局 + 世界状态不可控)>> 采集收益
    (7 格=钻石局可遇不可求)。修 = **整段降级**:n_raw=7 →
    `obs_conflict('back_7slots_collect')` 留证(带公式/CV/防抖序列 + 去重
    截图;verdict 如实声明画面可能推进,以截图为准)+ 帧态门保留辖留证;
    停机/flag 文件机制全删(源码级锁禁回流);7 格坐标由 CV 持续留证 +
    人工在场经 MCP 交互采集(暗框初测 → 拖角色逐位实锤 → upsert 登记)。
    帧态门复盘(run 27):触发帧确为备战态,门本身有效且判定素材 = 当前
    处理帧非缓存——失效的是「停机能保画面」的假设,不是门。
...
- **H.(件③)7 格无档继续 obs_conflict 留证不停机(维持首轮 C 的拒绝)**:
  ...
  **→ W209i 裁决翻转(记 I)**:run 27 实证推翻「停机=现场可采集」前提
  (实时制游戏停 bot 不停游戏)——H 的拒绝理由失效,采纳降级(决策 12);
- **I.(W209i)继续停机钩子 + 停机后等画面静止(结算弹窗出现/N 秒无变化
  确认)**:拒绝——等静止只能确认「世界走完了」,不能让它回退:结算弹出
  后备战帧已消失,采集窗口照旧不存在;且「停机+等待」仍烧掉整局。
  真要现场采集,正确工具 = 框架原生 PAUSE(方案 C,未暴露)/人工经 MCP
  驱动,不是 stop_running;
- **J.(W209i)停机时机再加约束(战斗中/boss 窗/结算前不停)**:拒绝——
  帧态门本就只在备战态触发(run 27 触发帧是备战态);问题是停机后世界
  继续走,加再多前置约束也保不住画面,治不了根。
...
- `cw_identity_obs.py`:read_deployed_chars 选档改双通道;件③:布局停机
  ...
  **W209i 再降级**:停机/flag 整段废弃,改 obs_conflict('back_7slots_
  collect')留证采集(带公式/CV/防抖序列,verdict 如实声明画面可能推进);
...
## 修订(2026-09-05,第四次停机治本批 da4f49f2)

决策「不一致采 CV 实测值」被占用一致性仲裁取代(前置条件):CV 端点探针在「N 档端点被占」帧被 occupied 立绘顶过阈值,结构性高估一格(ADR-0420 探针干扰在案的实证)——不一致帧采 CV 会选错档。现口径:已建档档位间冲突时,以「paddle 计数−前排占用=后排期望人数」对逐候选档比中心占用一致性,取唯一最小者;CV 失读/并列/缺档/未建档超集退采 CV 旧规。门序分层声明:防抖通过≠最终采信(门序=防抖→占用仲裁→留证)。验收:第四次停机留证帧 7/7 逐槽认出(留证帧 fixture 入测试仓)。
