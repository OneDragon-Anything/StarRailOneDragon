...
**范围**：球谓词席自由槽切换复用既有读口 `bench_free_slots`（零新立口，entry.py:141-146）、双源审计容器侧改 `deployed_count_of`（cw_screen_prep.py:819）。**不含**：黑板字段删除（批 5）、投影推进面（批 5 随投影退役）、新立读口（既有口已覆盖，见详设取舍-6）。
**设计依据**：`details/obs-retirement.md` §阶段 3.3。
**文件面**：`.../mandate_v1/entry.py`、`operations/cw_screen/cw_screen_prep.py`（审计消费点）、`sr-od-test/`。
**依赖**：3.1
**优先级建议**：5
**完成判据**：
- 行为对照 obs-retirement.md §阶段 3.3：球谓词席自由槽来自 `bench_free_slots`（含宝箱/典籍占席用例）；审计容器侧口径 = `deployed_count_of`。
- 通用工程门：同 3.1。
**验收凭据形式**：球谓词测试全绿（含箱/典籍占席用例）+ 审计口径对照。
