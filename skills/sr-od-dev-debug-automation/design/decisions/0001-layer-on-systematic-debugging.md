...
## Considered Options
1. **自含:把 Phase 1-4 重写进 SKILL.md**:最独立,但重复 superpowers 已有的;通用流程更新时要跟两处;SKILL.md 变长、信号被稀释。
2. **叠加:引用 superpowers:systematic-debugging 作基座,SKILL.md 只写项目专属判据**(选中):无重复;用**完整命名空间标识符**引用(`superpowers:systematic-debugging`)。
...
- **follow-up**:若团队决定全面去 superpowers 依赖,本 skill 需重评估:要么把 Phase 1-4 吸收进自身,要么改引其它基座。本 skill 与 superpowers:systematic-debugging **无立场冲突**(后者非强制 RED 的 Iron Law 类),故当前不必跟随去依赖决策。
