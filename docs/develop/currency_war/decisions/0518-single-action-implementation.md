...
- **对抗修复批补登(2026-09-06 对抗审查裁决,实施批修复两轮)**:①EV 买面席位门(`shop.py` EV pass 补 check_seats 与 dominance 同款;拒因分键 `shop_ev_bench_wait`)——满栏非合并买入在 simulate 走 bench_full 整动作 no-op 分支,无门则同帧重复提案不收敛;席位门后买面全 gated(M4 腾席/M2 break/M2b continue/dominance/M6/EV 各有门),**满栏 §2.5 执行侧机制与双账满栏豁免在商店生产路径不可达,保留为防御纵深**;②执行侧防御帧帽 `SHOP_SEGMENT_ACTION_CAP=16`(与备战 VISIT_ACTION_CAP 同款;超帽 RuntimeError + `plan_visit_action_cap` 分键,禁静默);③计数键改名 `shop_wave_idle_gold`→`shop_visit_idle_gold`(visit 语义;跨结构不可直接对拍,旧键全仓零消费);④prep SellBench 投影补 state 侧金账(`sell_refund` 与 simulate 同式;此前半投影致同访问后续帧读偏低金=过量卖出风险);⑤OpenBox/OpenTome 投影按 action.slot 摘;⑥guard_proposal_vs_expected 补 BuyCard 名断言。
- **终结 op 退出补登(2026-09-06 终结 op 语义 review V1/V2 修复批,as-built 勘误)**:上文本节与 `shop_visit.md` §2 曾写「RefreshShop 终结 = 本段结束」——该表述在修复前是**规格意图而非代码现状**:决策循环对 `_aop.terminal` 零消费,RefreshShop execute 后循环回顶,黑板保持刷前旧牌面,策略器对旧牌面重发 RefreshShop 连发至硬墙(P35 实证 plan=Refresh×4),旧牌面 BuyCard 提案亦可点在新牌面槽位(错买)。修复 = execute 后按 `_aop.terminal` 统一 break(RefreshShop/CompTransaction 同路径;CloseShop 保留 execute 前落点——其动作 op 为 no-op,行为等价,且保持「CloseShop 终结不入行」的 decisions 行形态契约),新事实由段循环下一次迭代入口观察重建;连带修复 `ledger.did_refresh` 未段级复位的残留(首段刷新后后续段被陈旧 True 钉住,段循环跑满不收工,修复前被「段内连刷」形态掩盖)。decisions 行粒度切换申报:**每刷一行**(刷新终结后独立成段),消费端按 `__type__` 计数不受影响;硬墙语义重推:封顶对象从「段内连刷」变为「跨段刷新提案」,计数器与拦截点不变。review 报告 = `.debug/temp/currency_war/terminal_op_review/报告.md`(V1/V2 违规,V3 本条勘误,V4 备战席满破墙段波批残留挂收敛批不动)。
