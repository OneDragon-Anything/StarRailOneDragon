"""货币战争 通用插件注册表(判断层,手编;C4 契约)。

**建库基准(leader 裁决,防名单迭代残留)**:单卡名单取
「组件元素与插件」深度研读笔记 的
**三B 节(单卡插件名单定稿,用户裁定纯效果序)**;小羁绊名单取
**三C 节(小羁绊名单定稿)**——即文档内**最晚定稿节**。
勿从旧节(三/三A/插件池数据定级版)取名单——旧节名单已被后续定稿
推翻(椒丘/巡海游侠1 出池即例)。
...
消费方:decision_v2 层1 插件买门、C3 fill_gap 空位规则
...
_UNIT_PLUGINS: tuple[PluginEntry, ...] = (
    ...
    #   买门按名匹配永不命中(PluginEntry docstring 承诺单卡=角色规范名)。
...
# 注:名单共 15 个,与三C T3 定稿名单对齐(测试锁 smalls==15)。
_SMALL_FACTION_PLUGINS: tuple[PluginEntry, ...] = (
    ...
    # 圣杯2(三C T3 定稿成员):效果=命运圣杯 2 档开祈愿试炼任务链
    # (产出 Archer;comp_elements 三·4 圣杯任务系统)。
...
# (恰 2 线过半且全局 ≥15%,研读报告 A2 边界组;开拓者·欢愉两线同族 → 1 键)。
...
def cross_line_skeleton() -> tuple[str, ...]:
    """跨线骨架名单派生(strategy_v4 点0 弱意向囤货集)。

    规则(研读报告 A2 口径,数据源 = W16_MAJORITY_LINES):
...
# 单卡插件的 majority_lines 与过半线统计表**程序同步**
# (单一写入口——过半线刷新只改 W16_MAJORITY_LINES,上方的字面量
# 被全集覆盖;小羁绊插件的 majority_lines 仍手注[三C 口径,无单卡
# 统计])。锁测试对拍两侧一致。
