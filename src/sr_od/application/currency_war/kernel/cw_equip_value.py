...
- ``EQUIP_GENERIC_VALUE`` / ``equip_generic_value``:通用输出先验表
  (自 cw_events._EQUIP_VALUE 逐值平移,值零变化;
  审计链随表迁移);
