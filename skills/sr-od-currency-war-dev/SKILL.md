...
2. **sim 批量**(秒级):`cw_sim.py` 的 P1 批量入口,A/B 对照同参数分布(hp≥验收线占比/均值/方向建立率)。sim 有粒度边界(见下),差异必须解释。**sim 是可深度挖掘的基建**(真代码层同源 + 校准层可注入 + 实机 Δ 池重放):遥测回放对拍/稀有态扫描/参数敏感度/校准层逐维升级——细则见 verification.md「深度挖掘方向」。
...
6. **子代理对抗**(ADR-0010):分析类方案/归因结论在采信前起子代理攻击(攻击型指令+实测数据+逐条裁决实验);大改动 review 同理。
