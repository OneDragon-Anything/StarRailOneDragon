...
> - **权重纪律(r5,防手调不收敛)**:eval 已膨胀到 20+ 维权重全标"待校准" → 20 维手调 = 随机游走。**改**:① research meta 先验权重**固定**(标"V4.4 先验",不标"待校准",版本更新才改);② 阶段 6 手调**只调最敏感 3-5 维**(~~hp_safe_threshold~~(已删,ADR-0204:阈值迁 cw_state 难度表)/ comp_viability 观测权重 schedule / `MAX_REFRESH_PER_ROUND` / α(t) 的 r_open/r_close / 连败 fold 阈值);③ ML side door 给**明确触发点**:攒够 N=50 局 replay 后可启用离线逻辑回归拟合(非模糊"以后再说")。详 06 阶段6。

## A3 阶段键控 eval(✅ 已实现 + review agent 经济学校准)
evaluate = `_phase_weights(plane, hp)` 加权的(羁绊+经济+角色质量)。**2026-08-03 修正(review agent + 用户)**:前期 economy **不该压低**(利息越早到 50 越好、滚雪球)。原"plane1 压低 economy"把"前期"和"保血"混淆。现:**HP 危险(hp<HP_DANGER)→保血**(战力/角色加权、经济降权,任何位面);**plane3→锁血**(全力战力/星级);**其余健康→平衡**(economy 不压、snowball 到 50)。阶段键控权重的**具体值见 `_phase_weights` 代码(单一源,不在文档写,避免两边漂;阶段 6 实机校准)**。economy_score 利息项由 level_plan 硬 gate 主导(D-14;~~economy_mode~~ 已删[ADR-0204:与 level_plan 打架的死配置],spend_mode 单一源);synergy 有高 ceiling 潜力项;char_quality 计 bench+deployed。**待校准**:HP_DANGER(A8 高难升阶;D-32 difficulty→threshold 派生**已落地**,detection 待接线)+ win_streak(连胜保连胜>吃息,待 read_streak)+ phase 权重值实机(阶段 6)。
**hp 阈值(⚠️ 2026-08-17 起 config.hp_safe_threshold 已删,ADR-0204;现行单一源 = cw_state 难度表 `effective_hp_threshold` 派生)**+ 偏移系数(转型 0.75×、死局 0.5×、连败 fold 1.5×)。`_phase_weights(plane,hp,hp_threshold=HP_DANGER)`、`_refresh_cap(state,hp_threshold)`、`maybe_pivot`(`0.75×threshold`)签名带参数(**默认=HP_DANGER 保行为不变**)。**D-32**:evaluate/plan/maybe_pivot 经 `effective_hp_threshold(state, config)`(`cw_state`)取阈值 —— `state.difficulty`(A1..A8,难度确认屏检测)检测到且 `config.difficulty_hp_override`(`DEFAULT_DIFFICULTY_HP` 常量,A1-A4 不变 / A5+ 升阶)有对应键 → 覆盖 hp_safe_threshold;否则回退 hp_safe_threshold(**向后兼容**:detection 未接线时零行为变化)。死局 0.5× / 连败 1.5× 待 is_run_dead / losing_streak 实现时补。

**经济统一论(2026-08-03 用户框架,接战略层)**:D 牌/买牌/买经验都是**花钱的一环**。方法论:维持 ≥50 金(息引擎)→ **超出 50 的钱免费该花**,花在哪由 **`target_comp.level_plan[当前等级]`** 导向(level_up / roll 找几费 / stable),tempo 例外(streak/HP 危/断档)可破息。**接法已落地**(2026-08-04):`plan()` level_plan **硬 gate**[D-14](level_up + afford → 直接执行,非纯贪心 eval delta)+ `select_comp`/`maybe_pivot`[cw_comps] + shop.py 接线。`economy_score` 封顶 5 档=金>50 隐式 free。level_plan 曲线:comp 自带优先,无则通用曲线 `_DEFAULT_LEVEL_GOAL` 兜底。
...
- **牌池计数(⚠️ 2026-08-18 对齐 V3.7 guide 权威值,旧「统一 9 张推测」作废)**:别套 TFT 的 29/22/18(多人池;货币战争**单人 vs AI**)。**精确副本数:1/2 费 = 27 张/种(可合四星),3/4/5 费 = 9 张/种**(guide 必修二实机论证,详 `../../game/currency_war/guides/V3.7_公共_经济运营.md`);低费池 3 倍深 → 同名浓度摊薄,压缩买/集星的池操纵期望按此建模。bot buy 减池、sell 回池与否待实机核(按不回保守)。
- **1 星买卖无损 → 免费操纵牌池(guide 实证确认,= economy_research §2)**:买 1 星(1 金)再卖(返 1 金)= 净 0、牌池 −1 → "买目标卡**同费用**的其他 1 星卡 → 卖掉"可零成本消耗该费用池,目标牌刷新概率 ↑。已编码于压缩买链(r61-r66 `_compress_release`+扫尾);cost≥2 的 2星合成再卖有 1 金手续费(§6/ADR-0121)。
- **采样顺序(r1 正确性-3)**:`_sample_shop` 改为**先按等级采费用(概率表)→ 再在该费用池里按角色均匀采**(角色→阵营反查,../../../game/currency_war/data/characters.md 74 角色可用)。当前"先阵营后费用"高估小阵营。
...
- **P1-1 optionality_score / 灵活性(high)**:A8 是方差生存战,eval 只奖励 commit(target_progress 向单一 target 推进),不奖励**保持灵活性**。加 `optionality_score` —— bench 角色同时属于 ≥2 可行 comp 的数量(用 COMP_LIBRARY.shared_chars 反查);holding transition_chars 给正分;过早卖 shared_chars 扣分。与 target_progress 平衡(承诺 vs 期权)。这是 auto-chess 高手核心直觉。
  - **状态(2026-08-04,D-21)**:`optionality_score(state)` + `alpha_t(state)` **纯函数已实现 + 2 测试绿**(`cw_decisions.py`);**evaluate 集成延后**(α·target_progress + (1-α)·optionality 混合,改核心 eval 需 P0 游戏验证才稳)。R_OPEN/R_CLOSE/OPTIONALITY_WEIGHT 值在代码。
...
- **F-1 off-policy 覆盖缺口(CRITICAL-for-convergence)**:校准链 replay(旧权重跑)→ 拟合新权重 是 off-policy 迭代,可收敛**前提是探索覆盖足够**。占位权重下 bot 只访问低质/单一 comp 状态 → 拟合权重对"好权重才到的强状态"(成型邪道/转型中段/plane3 高经济)零覆盖 → 外推失真。**修**:① 显式探索 preset(强制 K 套不同 comp 强绑 + 各 spend_mode 姿态各跑 N 局(旧 economy_mode/aggression 已删,ADR-0204));② 覆盖度指标(replay 记 synergy_vector×plane×round 经验分布,拟合前断言各 comp×plane×阶段样本≥阈值);③ importance weighting / 分位回归(off-policy 修正);④ active learning(优先跑不确定性最高的状态)。
