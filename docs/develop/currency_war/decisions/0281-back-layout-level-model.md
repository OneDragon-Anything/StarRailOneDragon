...
> **版本界碑(2026-09-04 ADR 存量 review;且本件结论已被 ADR-0385(后排选档双通道对账)推翻)**:本 ADR 裁决的对象已亡——decision/ 整包(含 decision_v2 各模块)已随 b94e9cfb(dd-038)删除,现行唯一策略载体 = mandate_v1(kernel 吸收下沉判据)。本件仅存史料价值(记录设计 why/翻案史);**文内一切「后续应做 X/须换成 Y」类前瞻指令一律视为已亡,勿执行**。

- 状态: accepted(**选档 level 驱动已被 [ADR-0385](0385-back-layout-cap-diff-formula.md)
  勘误推翻**——run 26 实证 + 用户口述「后台格数=6+(cap−level)」;本篇的
  幻影档清除、件3 系统单位恒最右自检、件4 不可拖剔除继续有效,且「cap9/10/11
  全落 8 格」由 cap 差公式自洽解释)
- 日期: 2026-08-24
