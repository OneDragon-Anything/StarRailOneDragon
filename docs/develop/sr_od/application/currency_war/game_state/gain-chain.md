...
- **不管**:溢出善后(腾位/卖牌归策略溢出面,链只如实记账);效果数据表的内容建模
  (数据表 = 游戏数据映射,见 §3);「获得投资策略」原语(策略屏落地相未迁链,见 §8)。

## 2. 三原语契约

三个公开入口(kernel/cw_gain_chain.py),全部带 `rand: bool` 必带入参(纪律见 §5)
与 `sig: ChannelSig`(链内所有容器写共享链入口 sig,同组遥测;evidence 加
`#place`/`#mergeN`/`#equip` 后缀区分写步)。出参统一 = `GainOutcome`(见 §2.5)。
...
1. **注册**:`gs.active_env` 逻辑写(写端署名 = `producer`,缺省
   `GAIN_CHAIN_PRODUCER`)。选择事实产生在动作落地处理时(画面 op 只观察选项与
   机械派发;原「点卡前写」语义已退役,用户裁定 2026-09-21)。
...
3. **回调返回后,单级 3 合 1 升星判断**(合成池 = 备战 ∪ 上阵 ∪ 溢出):恰有同名
   同星 3 只 → 消耗三只、载体升星(单级合成规则见 §2.4)。**回调先于升星**的理由 =
...
(栏未观察 = 零写留证,§6)→ `on_equipment_gained` 回调(§3)。装备无升星判断。

### 2.4 单级合成 merge_step_once
...
`GainOutcome`(frozen dataclass,三原语共用):`placed`(容器写主步是否发生)/
...
三原语与三回调均带 `rand: bool` 必带入参,逐函数入参、递归透传:

- 链入口按自身性质定初值(投资环境确认 = 确定性入口,False);
...
best-effort 边界只收在**非容器写的观测/登记腿**内(§2.1 登记腿、§3 advisor 申报
行/条件腿标记面):失败 log + 缺陷留证、不阻塞。
...
pick_supply 等旧通道与策略屏效果腿(`kernel/cw_action_report/pick_invest.py`)。
...
现役唯一接入 = **投资环境落地相**:`kernel/cw_action_report/pick_invest.py::
report_action_pick_invest_param` 的 `source='portal'` 支整支走 `gain_invest_env`
(rand=False;session 形参显式传入)。发射相与策略屏(`source='strategy'`)支不消费
本链;「获得投资策略」原语随策略屏迁移批另立。
