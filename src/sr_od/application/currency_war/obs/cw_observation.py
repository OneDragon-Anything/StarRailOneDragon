...
def read_deployed_count(ctx: SrContext, screen: MatLike) -> int | None:
    ...
    现役消费面 = 备战观察装配环(``cw_screen_prep.py`` cap 段)与后排
    布局选档(``obs/cw_back_layout.py``):d(cap_remaining)+ e(offset)
    依赖它;读不到 → fallback
...
def read_deploy_cap(ctx: SrContext, screen: MatLike,
    ...
    现役消费面 = 备战观察装配环 cap 段应用本 Y 非 level 估 cap_remaining。读不到 → 退 level 估(fallback;cap=level 故 fallback 仍准)。
...
def read_game_state(ctx: SrContext, screen: MatLike,
    ...
    部署 = DeployMove 原子序(备战决策环动作));席位域观察写端 = 备战装配环(通道声明见下
    容器直写块注),本漏斗不写。
    """
