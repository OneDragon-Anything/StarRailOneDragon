...
- **不管**:PR 收尾(处理 review comment / 推可合并 / checks)→ `sr-od-dev-pr-finishing`;单条 review comment 处理 → `superpowers:receiving-code-review`;画面建模细节 → `sr-od-dev-screen-onboarding`。
- 引用上述 skill 而非重复。
...
- 前缀:`sr-od-dev-`(开发流程类)。
- junction:`.claude/skills/sr-od-dev-pr-review` → 根 skills/(本地,不提交)。
- 结构:`SKILL.md`(方法论入口)+ `design.md`(本文件)。

## 与现有 skill 的关系

- `sr-od-dev-pr-finishing`:PR 收尾(已开 PR 推可合并)。本 skill 是**收尾前的审查验证**;审完给结论,收尾走 pr-finishing。
- `sr-od-dev-screen-onboarding`:画面建档。本 skill L4 live 时"顺路建档"引用它。
- `superpowers:writing-skills`:通用 skill 结构/frontmatter。本 skill 的项目特定部分(4 硬规范 + 落点)由 `sr-od-dev-skill-guide` 管。
