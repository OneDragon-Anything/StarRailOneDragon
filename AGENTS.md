...
- **sr-od-test 纪律（`test_context` 是 session 级共享）**：改其属性（`run_context`/`controller` 等）一律 `monkeypatch.setattr`（自动还原；裸赋值会被 conftest 守卫警告并还原）；测试不写真实 `.debug/` 路径（用 `tmp_path` 或重定向 `_DIR` 常量）；动过的外部状态 `finally` 还原。
