...
跨局静态知识,策略 `import` 查询,不进 `GameState`。**单一源 = 代码注册表**(AGENTS.local.md「工程化/单一真相源」+ 2026-08-06「游戏数据代码单一源」):游戏数据(name/effect/category/faction/source)一旦在代码注册表全量建模,对应 `data/` doc 即冗余 → **删除**(doc 与代码双源 = 漂移);doc 只在「代码未全量」时作过渡源。
