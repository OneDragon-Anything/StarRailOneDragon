...
- 失败记忆归分发层:op 侧不自持熔断计数,同签名零推进由外循环环级无进展守卫接管([../flow/guards.md](../flow/guards.md) §1)。
- cap 误差不对称取舍(降级链 = guards.md §7):低读阻塞上阵(贵)> 高读白拖一次(便宜)——paddle 权威直用、失读 max 兜底、全源失读不设板满门(拖到游戏拒即真值)。
