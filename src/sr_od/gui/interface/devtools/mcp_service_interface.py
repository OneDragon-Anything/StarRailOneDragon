...
本页面只管理本机 ``sr_od.backend.entry.server`` 子进程，不把 MCP server
...
def _server_log_path() -> Path:
    ...
    return _project_root() / '.debug' / 'sr_od_mcp' / 'main_server.log'
...
def _probe_server(port: int) -> tuple[bool, str]:
    ...
    try:
        with urllib.request.urlopen(_health_url(port), timeout=2) as resp:
            ...
            if resp.status == 200 and 'sr_od' in body:
                return True, f'已启动: {_server_url(port)}'
            return False, f'端口 {port} 有响应,但不是 sr_od MCP 服务'
...
class McpServiceInterface(VerticalScrollInterface):
    ...
    def __init__(self, ctx: SrContext, parent: QWidget | None = None) -> None:
    ...
    def get_content_widget(self) -> QWidget:
        ...
        self.action_card = MultiPushSettingCard(
            ...
            title='服务控制',
            content='按当前端口管理本机 sr_od.backend.entry.server 进程',
