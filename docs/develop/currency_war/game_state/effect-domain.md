...
> 逐效果规格),即候裁 8([r5-migration-plan.md](r5-migration-plan.md) §7 条 8「效果域
...
- 效果域载体 = `BoardState.effects`,类型 `ActiveEffectInventory`(kernel/cw_board_state.py
...
W4 键级三分(游戏效果键→效果域/策略行为键→决策行/无消费键删;r5-migration-plan.md
§2 W4)在效果域侧的落点:
...
|---|---|---|---|
| 节点推进 | 备战分支 `advance_node`(operations/cw_loop.py:2131);同节点去重与「登记当节点不推进」守卫在 inventory 内(kernel/cw_effect_inventory.py:240-254) | PLANE_START / NODE_ENTER / CONDITIONAL-N_NODES 型 | 节点类 counter +1,每节点恰一次(推进有效位为闸门) |
