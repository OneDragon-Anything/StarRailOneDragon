...
| 实体 | 增量属性(人维护) | 消费点 |
|---|---|---|
| **投资策略/环境**(`cw_investments`) | `EconomyEffect` 27 字段(给金/免费刷/利息覆写/难度Δ/连胜倍率/合成触发族…,ADR-0131/0142/0205/0211 四轮);`PICK_VALUE`/`ENV_PICK_VALUE` 选卡分(旧低血生存选卡分已退役为常数 0 钩子删除,勿再增录);`ENV_CATEGORY` 七类+`ENV_FACTION` 阵营绑定 | decide_event 选卡/`_refresh_cost`/`_refresh_cap`/利息引擎/难度账本/env 亲和 |
