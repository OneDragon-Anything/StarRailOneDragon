...
def p2_form_key(gs: GameState, calib: P2CombatCalib) -> float:
    ...
    """
    star_depth = deployed_star_depth(gs)
    return (float(_settle_rung(gs)) + calib.form_level_weight * (
        level_of(gs) - calib.form_level_base)
        + calib.form_star_weight * star_depth)


def p2_win_p(gs: GameState, node: str, round_num: int,
...
def p2_combat_delta(gs: GameState, node: str, round_num: int,
                    rng: random.Random,
    ...
    """
    wp = p2_win_p(gs, node, round_num, calib)
    if rng.random() < wp:
...
def boss_settle_delta(gs: GameState, dir_round: int,
    ...
    """
    if rng.random() < node_win_p('boss', round_num_of(gs)):
...
def _deployable_depth(gs: GameState) -> int:
    ...
    **辖域(v11 后)**:reward/supply 桶键与观测面(depth_trail/账本);
    ...
    板深维不可兑换,rung 维可辨)。
    """
    return sum((gs.board.value or {}).values())
