...
| 冶金炉变异产物是否触发获得后果 | 实机采证点;**本批建采证钩子**(§2.2 采证行:采样命中后果表成员即留证),推翻「变异不触发」即升格 `on_equipment_gained` |
...
|---|---|---|---|
| 冶金炉·拖装备 | **单笔 `write_logic_rand`**:equips = 移除工具 ∧ 目标件 → 同类别采样替换 | rand | 否(变异面) |
| 冶金炉·拖角色 | ①equips **单笔 `write_logic_rand`**:移除工具 ∧ 移除穿戴件 ∧ 追加各穿戴件的同类别采样替换;②目标角色行域 `write_logic`(穿戴清空) | ①rand ②logic | 否(变异面) |
