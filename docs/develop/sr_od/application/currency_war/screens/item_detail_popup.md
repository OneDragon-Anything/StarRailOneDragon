...
- 外循环分支 0e3:OCR「聘用书」(lcs 0.8)∧ **祈愿排他**(`货币战争-祈愿试炼.标识-祈愿试炼` 不命中)。排他双写于外循环分支与本 op `entry_ok`(两处同参):祈愿试炼选项名含「聘用书」,无排他即截胡祈愿分支死循环——排他是本分支判据承重件,非装饰。建档 = `currency_war_item_detail.yml`,**有档案但无 id_mark**(仅「按钮-关闭」定位区)→ 入口维持 OCR 回退;档案补 id_mark 后可切 area 锚(切换属实施批内部对齐,ADR-0584 §2.2)。
