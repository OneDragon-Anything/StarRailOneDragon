...
§2.4,本表行集 = 统一词表全集);事件线 pick 族批4 收编待入(§2.5)。
注册完备锁(批4)自此以本表为机械约束对象。(原 CompTransaction 行已随
unified-action-factory 批2b R3 删除——整档替换宏动作全链退役。)

**同名动作双域行更替申报(批3)**:词表归一(批2b)后 ``SellBench``/
``LevelUp`` 两类为商店/备战共用词表,单一表一键一行——两行自批1 商店
...
_REGISTRY: dict[type, type[ActionOp]] = {
    ...
    # 备战族(批3 收编;SellBench/LevelUp 两行自批1 商店行更替,见模块头
    # 「同名动作双域行更替申报」节;LevelUpShop is-a LevelUp 兜底本行)
    SellBench: PrepSellBenchOp,
    LevelUp: PrepLevelUpOp,
    DeployMove: DeployMoveOp,
    SellDeployed: SellDeployedOp,
    WearEquip: WearEquipOp,
    ClickSpheres: ClickSpheresOp,
    OpenBox: OpenBoxOp,
    OpenTome: OpenTomeOp,
    OpenBookcard: OpenBookcardOp,
    FurnaceUse: ToolUseOp,
    PrivilegeCardUse: ToolUseOp,
    WrenchUse: ToolUseOp,
    PrecisionWrenchUse: ToolUseOp,
    StaffProjectorUse: ToolUseOp,
