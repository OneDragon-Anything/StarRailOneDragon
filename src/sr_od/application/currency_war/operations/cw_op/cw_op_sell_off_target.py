...
sell off-target(阵营 ∌ target)→ 清板 → concentrate target。互补 (防**新** off-target deploy;
...
**坐标(实机观测,待 screen_info 化)**:deployed 槽 click = 前排槽(x,~450)/后排槽(x,~680)
...
class CwOpSellOffTarget(SrOperation):
    ...
    SELL_BTN: Point = Point(1746, 901)
    FACTION_RECT: Rect = Rect(1395, 285, 1605, 330)
    ...
    def clean(self) -> OperationRoundResult:
        ...
        for slot in self.FRONT_SLOTS + self.BACK_SLOTS:
            ...
            if _do_sell:
                self.ctx.controller.click(self.SELL_BTN)
                time.sleep(0.8)
                sold += 1
            else:
                kept += 1
