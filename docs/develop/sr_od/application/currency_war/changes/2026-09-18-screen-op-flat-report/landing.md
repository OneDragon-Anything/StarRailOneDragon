...
**范围**：`screens/` 全部画面文档旧架构表述清零（基类/五段/适配器/装配点分流/段迹/旧观察类名 → 两 node + report 现役形态，逐篇对照现役代码）；`architecture.md`/`flow/README.md`/`game_state/fields.md`/`node-derivation.md` 同类残留；`flow/guards.md` §7 失真守卫行修正；`flow/outer_loop.md` changes/ 引用清理。零代码改动。
**设计依据**：design.md §2.2/§2.3；正本 = screens/op-layer.md（T-8 重写版）。
**文件面**：上述 .md 文档（不含 `changes/` 目录）。
**依赖**：T-8。
**优先级建议**：6。
**完成判据**：全域 grep 归零（排除 `changes/`）；零代码改动；游戏机制内容零失真（逐篇对照现役代码）。
**验收凭据形式**：grep 归零输出 + 文档抽查对照。
