...
# after_operation_done 的 result 注解在类定义期求值,OperationResult
...
def readiness_admission_report(state, comp) -> dict:
    ...
    **帧对齐说明**(预估源 = 容器单例 game_state_of,T-146 装配源迁移
    ...
    return launch_admission_report(game_state_of(state), comp,
...
def _dispatch_pos(ctx: Any) -> tuple[int, int]:
    """[cw-op] 行位置键:容器节点读口现读(缺省镜像 kernel 读口 1 形态;
    ctx 无对局/session(局外/测试桩)退 (0, 0))。

    (原 telemetry.op_journal._op_journal_pos_of 随其模块退役,判定体
    迁此消依赖;dispatch 包装与仲裁段两类日志行共用。)
    """
    sess = getattr(getattr(ctx, 'cw_match', None), 'session', None)
    if sess is None:
        return (0, 0)
    from sr_od.application.currency_war.kernel.cw_game_state import (
        game_state_of,
    ...
    _gs = game_state_of(sess)
    return (int(plane_of(_gs) or 0), int(round_num_of(_gs) or 0))


def _log_cw_op(op_name: str, pos: tuple[int, int], dur_s: float,
               outcome: str, detail: str = '') -> None:
    """落一行主日志 [cw-op] 行(只落 exit 行;op 调用流的日志载体)。

    dur = 分派进入→出口的单调钟差(enter 行不落——「当前正在执行哪个
    op」的实时现态由画面巡检 + state 流尾行 sig.actor 承接);op 序列/
    时长画像/三载体商店访问计数的消费协议 = skill runtime-ops「效率剖析
    与巡检阈值回填」与 match-review「op 边界重建规则」。
    """
    msg = (f'[cw-op] op={op_name} plane={pos[0]} round={pos[1]} '
           f'dur={dur_s:.2f}s outcome={outcome}')
    if detail:
        msg += f' detail={detail}'
    log.info(msg)
...
def _launch_frame_arbitration(op) -> dict:
    ...
    **预算闸闭包读金口径** = 期望态容器现读(``game_state_of(session)``,
    ...
    try:
        ...
        _sess_hp = session
        if not in_launch_spend_zone(_arb_gold_of(_arb_gs_of(session)), _sess_hp):
...
class CwLoop(SrOperation):
    ...
    阶段三 特殊规则(无独立画面档/身份不可表达:道具详情/消耗品浮层/
    阿哈装备/BOSS 误读兜底/位面过渡误读兜底;互斥,顺序不承重);
    阶段二 备战表面默认分支(双锚 + 横幅中间态守卫)→ CwScreenPrep
    备战单轮(观察→对账→决策→期望态→执行)→ 战斗窗(闩+帧锚)→
    「下一步」→ 未知兜底。
    """
    ...
    CW_DISPATCH_SCREENS: ClassVar[tuple[str, ...]] = (
        ...
        '货币战争-商店刷新概率表', '货币战争-祈愿试炼', '货币战争-星徽秘典弹窗',
        '货币战争-备战-专家邀请函', '货币战争-商店卡牌详情', '货币战争-备战-策略锁定',
        '货币战争-备战-遭遇锁定', '货币战争-备战-开商店', '货币战争-提示-前台无角色',
        '货币战争-BOSS简报', '货币战争-位面过渡', '货币战争-简报', '货币战争-投资环境',
        '货币战争-备战-角色详情', '货币战争-星徽详情', '货币战争-中断挑战弹窗',
        '货币战争-大厅',
    )

    def _dispatch_anchor_precheck(self) -> None:
        """iter1 分发画面预检(防建档/merged 漂移):分发清单内任一画面不存在
        或无 id_mark 锚 → log.error 逐条点名。不中止运行(缺画面退化为「该
        画面不识别」,其余分支照常推进;中止会造成无对局可跑)。异常吞掉不
        阻塞(预检失败不能比事故本身更贵)。"""
        try:
            problems: list[str] = []
            for _screen_name in self.CW_DISPATCH_SCREENS:
                _si = self.ctx.screen_loader.get_screen(_screen_name)
                if _si is None:
                    problems.append(f'{_screen_name}: 画面档不存在')
                elif not any(_a.id_mark for _a in _si.area_list):
                    problems.append(f'{_screen_name}: 无 id_mark 锚')
            if problems:
                log.error('[cw!][loop] 分发画面预检:%d 个画面异常 → 对应画面'
                          '每帧静默跳过(历史事故形态):%s',
                          len(problems), '; '.join(problems))
        except Exception as e:   # noqa: BLE001  预检失败不阻塞对局
            log.debug('[cw-loop] 分发画面预检失败(不阻塞): %s', e)
    ...
    def __init__(self, ctx: SrContext, max_rounds: int | None = None,
        ...
        self._iter: int = 0
        # run 收口:中止/卡死/停机局不走 3c 回大厅,收口由
        # ``after_operation_done`` 钩子补置(成功/失败/停止全路径必达,
        # 检查未收口 → 补收口)。收口 = close_run(零落盘,置跨局 run_id 重铸位)。
        ...
        # ——cw_match 若已建立则不再当新局(保守方向:漏标不误标)。

    def handle_init(self) -> None:
        """run 级状态初始化(框架钩子:每次 execute() 开头由
        ``_init_before_execute`` 调用;见类注 R4-1 迁移说明)。"""
        # run 启动时刻/新局标记在下方 _is_new_match 求值后注入;首见结算屏标记
        # (relaunch 残留结算判据)现役宿主 = CwScreenBattleWait(SettlementState,
        # W971 05-battle §1)。
    ...
    def _cw4_counters_snapshot(self, session: Any) -> dict[str, int] | None:
        ...
        局终级全键聚合现归宿 = 局终域行载荷 ``MatchFinal.cw4_counters``
        (两收口路径在 ``write_match_final`` 调用点现读传入)。取值 = 策略 state
    ...
    def after_operation_done(self, result: '_OperationResult') -> None:
        """局终 runs summary 收口(收口挂本钩子而非 loop 顶:``operation.execute()``
        每轮前(operation.py:408)先查 ``is_context_stop``,stop 到达后 ``loop()``
        不再被调——loop 顶检查不可达)。
    ...
    # (局终归档现役归宿 = W2 已落位(gs_prov=快照来源注记/局终速查=
    #  match_final 行,retirement.md §2);存量数据文件归档只读
    #  (不删不写,裸读考古,r5-migration-plan §4-6)。)
    ...
    def _write_terminal_summary_if_needed(self) -> None:
        ...
        换源 = ``gs.node.value is None``,观察漏斗每帧刷新节点);只要
        观察过对局态或有过任一结算行,就是真局,必走收口。

        本收口现役面 = match_final 收口行(含计数聚合)+ run 收口位
        (跨局 run_id 重铸承接口)+ 档案装配。
        """
        ...
        try:
            ...
            _gs_hp = (_gs.hp.value if _gs is not None else None)
            _final_hp = self._last_true_hp(_gs_hp if _gs_hp is not None else 0)
            ...
            try:
                ...
                _mf_type = resolve_final_type(
                    ...
                    plane_reached=(plane_of(_gs) if _observed else 1),
                    rounds_played=_has_outcome)
                if _gs is not None and _mf_type is not None:
                    write_match_final(
                        _gs, final_type=_mf_type,
                        plane=(plane_of(_gs) if _observed else 1),
                        round_num=(round_num_of(_gs) if _observed else 1),
                        hp=(int(_final_hp) if _final_hp else None),
                        gold=(_gs.gold.value if _observed else None),
                        streak=(_gs.streak.value if _observed else None),
            ...
            state.close_run(
                ...
                plane_reached=plane_of(_gs) if _observed else 1,
                rounds_survived=round_num_of(_gs) if _observed else 1,
            ...
            log.info('[cw][loop] 局终 summary 收口:%s p%s-r%s hp=%s',
                     ...
                     plane_of(_gs) if _observed else 1,
                     round_num_of(_gs) if _observed else 1, _final_hp)
    ...
    def _dispatch_identity_screen(self, name: str, screen) -> OperationRoundResult:
        ...
        if name in ('货币战争-备战-策略锁定', '货币战争-备战-遭遇锁定'):
            _lock_screen = name
            _lock_area = ('按钮-返回投资策略选择' if name == '货币战争-备战-策略锁定'
                          else '按钮-返回遭遇选择')
        ...
        if name == '货币战争-备战-开商店':
            ...
            # visit_open_shop 返回 (ok, detail) 元组 → 包装可调用形经 _FnResult 适配
            return self._dispatch_screen_op(
        ...
        if name == '货币战争-位面过渡':
            ...
            return self._dispatch_screen_op(
                ...
                on_result=_on_plane_transition_identity)
        ...
        if name == '货币战争-星徽详情':
            # 双锚判定已由建档组合承载;点X 关迁入 op(「不用 ESC」理由随迁 op)
            return self._dispatch_screen_op(
        ...
        if name == '货币战争-大厅':
            return self._lobby_return_collect(screen)

        # 在册画面暂无处理器(建档先于接线的过渡态)→ 等待,不落备战默认
        log.warning('[cw-loop] 身份命中但无处理器(建档/接线过渡态): %s → 等待', name)
        return self.round_wait(wait=1.0)

    def _lobby_return_collect(self, screen) -> OperationRoundResult:
        """3c. 回到大厅(对局结束)→ 对局收口(telemetry 红线:runs summary/
        分配器/存档写端保留在本 loop,不随分发形态迁移)。"""
        def _lobby_return_step() -> OperationRoundResult:
            # r10 假局守卫:本 loop 从未记过 round_outcome(未打过任何一回合)却见大厅
            # = 开局失败/中断(第四局实证)。不记 summary、不喂分配器,仅清理 match。
            ...
            if self.ctx.cw_match is not None:
                ...
                _gs = game_state_of(self.ctx.cw_match.session)
                _observed = _gs.node.value is not None
                # ⚠️ 假 win 守卫(2026-08-17 M70 事故):要求 plane==3 精确值
                # (值域守卫已在上游拒 8,此处双保险);死局不判 win。
                _died_this_run = self._settle.saw_defeat_settlement
                _final_plane = plane_of(_gs) if _observed else 1
                _final_round = round_num_of(_gs) if _observed else 1
                _gs_hp = _gs.hp.value
                _outcome = MatchOutcome(
                    won=(_final_plane == 3 and not _died_this_run),
                    final_plane=_final_plane,
                    final_round=_final_round,
                    final_hp=(_gs_hp if _gs_hp is not None else 0),
                )
    ...
    def loop(self) -> OperationRoundResult:
        self._iter += 1
        # stop 路径 runs summary 收口不挂 loop 顶:operation.execute() 每轮前
        # (operation.py:408)先查 stop,stop 到达后 loop() 不再被调——
        # loop 顶检查不可达;收口钩子 after_operation_done 对全路径必达
        # (operation.py:492),见类注。
        ...
        # [停机钩子纪律] 钩子停机的前提是该屏**偶发**出现;建档完成后立即删除,
        # 别留到「下次遇到」(实证:曾致每局必停、误判外部会话拦截,排查一整晚)。
        ...
        if (self.round_by_ocr(screen, '聘用书', lcs_percent=0.8).is_success
            ...
            def _on_item_detail(ok: bool, _res: Any) -> None:
                log.info('[cw-loop] 道具详情弹窗(聘用书)→ 点× 关闭(ok=%s)', ok)
        ...
        # 0g. 投资策略「阿哈大悦」装备选择 overlay(为阿哈选1件简易装备;无独立画面档,
        #     锚挂备战档「标识-简易装备」)→ 点首件自动关回(固定策略申报见 op)。
