...
## L3 消歧(52 轮 sim 收账「L3 独立发射判据」对齐)

「L3 独立发射判据」= **L3 档的触发条件独立**(金量级达标
`in_must_spend_zone` 即入选择序,血线不构成判据输入)——不是独立
发射位:消费位仍是 M3 的 LevelUpShop 单动作(§3.5 合并纪律「禁第二
发射位」不因本批破例),触发源 = M3 三臂 ∨ 必花域,`auth_basis`
分键可归因。判据独立、发射位合一,两表述不冲突。

## 挂账知悉(登记面,本批不动)

- 激活率门(死亡线武装后个位数百分比内)实测收账随首批武装局;
- 装备 P61 期限到期核销——均为编排者跟踪项。
