...
关联:ADR-0517(画面 op 规范,裁定 1 来源)/ADR-0563(策略器状态工厂与 B4 收缩,create_state 保留依据)/ADR-0579(候选评分遥测供数,键守卫段内平移)/`docs/develop/sr_od/application/currency_war/flow/README.md`(四身份分离/黑板单一写端,§2 同步重写);方案 = `.debug/temp/currency_war/t119_strategy_contract/方案.md`(v2,零阻断)+ 方案审 v2(0 阻断 + 3 低随批)
