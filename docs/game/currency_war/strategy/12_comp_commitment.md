...
当前 select_comp 按 comp_score(含 form_progress)选,但 gap 小 → flit。**commit 机制**:一旦某 comp 的 form_progress 越过 commit 阈值(如 0.4),target 强粘(只更强信号才转)—— 不让微小的 score 波动切走已积累的 comp。语义:「你已经在堆 X 了,继续堆 X,别因 shop 这轮没刷 X 就放弃」。
