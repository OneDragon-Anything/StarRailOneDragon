...
| [0592-p1-upgrade-realize-predicate-alignment.md](0592-p1-upgrade-realize-predicate-alignment.md) | ADR-0592: P1 升级视界支A 兑现链谓词收敛(T-139;命题族 P82 单篇候证明批)——支A 从「板满∧bench 2★」收敛「板满∧配方等待件(arm1_existence ② 板面域不限星级)+配方缺口成员+边际贡献>0」(P82-c 双向差分:放宽 1★ 缺口件/收窄线外 2★ 与纯冗余件;[33] 裁定 2026-09-01 vs ADR-0576 落码 2026-09-07=落码违裁定性 P82-e)+谓词单一源下沉 kernel/cw_waiting_piece(ADR-0516 先例,arm1_existence 实现迁入 statefn 重定向零改,kernel ①臂/ΔV_pop 指示项/criteria 委托/引擎披露/检查器镜像五面同源)+RECIPE_WAITING_MODE 显式参数缺省口径 B(候裁行=账本 T-139,翻常量即切 gap/ignition/board)+char_id∉deployed_cids 轻量合取先行(has_deployable 完整接线候后续批)+引擎披露键 dec_recipe_waiting(ADR-0589 模式,检查器三级优先)+prereg 判据节(r6-r8 lv≥5 占比≥60%∧上行≥25pp 双列,LevelUpShop token 口径,历史值 14/24 标 cal1 窗/cal2=10/24)+ALL IN 语义照 P72 §2.5 消费不预支 P81 行候裁收窄面(与 T-143 互引划界);闸本体与奖励帧辖域零改 | 已实施(方案 v3 三轮对抗零阻断放行;拟号 0591 撞 T-141 改 0592 让位,编排者裁决;commit 候编排者统一门) |
| [0594-endgame-liquidation-p81.md](0594-endgame-liquidation-p81.md) | ADR-0594: 域①终局清算落码(P81;T-143 S2′)——双落点(编排者裁决#2):落点一=ADR-0566 仲裁域①豁免行三件套(带内 range(0) 解除[in_launch_spend_zone zone 判定单一源释放支,分键 launch_arbitrage_endgame_frames 随释放同源计数]+launch_arbitration_gate 地板降 0[P70 边界 1 的域①豁免]+段帽处置[生产 MAX_REFRESH 硬墙不动,sim 豁免行旁路与哨兵扩展随载体声明批挂账])+落点二=决策层清算臂(域①判定函数 endgame_liquidation_frame=plane_last_battle∧is_final_plane 独立判定,run_plane_count 载体声明生产=3 结构真值/sim=1,不动 schedule_of;循环序步 0-5=升级续批[辖域判据独立于 arm1,三跑 N2 实证闸开≠臂发]→腾位卖出→补位买入→单帧合成→寻件刷新→金尽终止;花什么全走既有单一源,域①帧既有选择序让位[M6/②(b)/囤积族花费对象=未来价值不获 P81 授权])+卖出腿=P78 通道对价段「终局清算」豁免行(ADR-0585 §4 N4 预留位接入,对价=域①资产价值清零腿②无损,线内/骨架禁卖维持,reason=endgame_liquidation_clear 枚举闭集第 6 通道值);mandate 备战栈 M3 批位同触发(auth_basis 分键 endgame_liquidation);P68 收窄关系申报(域②③权威由 P81 承接,撤并候独立批);生产 P3 r9 域①发射面实时生效/sim 载体声明前零漂移(申报含哨兵扩展+段帽放宽+runner 声明三项接线义务) | 已实施(方案 v2 二轮对抗审零阻断放行;编排者裁决#2/#4/#6 固定;新锁 16+登记门 2 更新;快速层 3268 全绿;commit 候编排者统一门) |
