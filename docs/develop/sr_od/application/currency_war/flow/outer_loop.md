...
阶段三 特殊规则（无独立画面档/身份不可表达;互相锚互斥,顺序不承重）：
        ...
        位面过渡误读兜底（本档'提示-点击空白继续'区命中 ∧ 非 boss 帧;共享
        文案按本屏 rect 判定——boss 帧同文案在 y≈770 波段,本屏 y≈930,
        全帧裸文本判定会被 boss 帧击穿误派,第五局 1-9 实锤路径）
...
| 0q-backstop | 位面过渡误读兜底 | 本档'提示-点击空白继续'区命中 ∧ 非 boss 帧 |
| 0n | 商店访问（备战分支内子分支） | 开商店态三 id_mark 锚；[cw-op] 行 op='商店访问' |
| 1 | 备战阶段默认分支 | 双锚；CwScreenPrep |
| 3c | 回大厅收口（局终正常收口） | 身份行'标识-创业指南' |
| 5 | 下一步 | OCR；CwScreenNextButton |

> **0p/0q 退役挂账（2026-09-16 调查结论）**：两个兜底的存在理由 = 「阶段一身份 miss 的真实帧」的**唯一推进路径**（删了只能落未知兜底停机待建档，等建档无人推进）。逐个判据：
> - **0p 具备退役条件，待实证后删**。原始触发事故（boss 标题「强敌来袭」持续误读）已随锚换装阵营徽记模板（2026-09-16）在身份层关账；剩余覆盖面 = 徽记跨局失配 / 提示文本同帧误读（低概率，且瞬态形态下一轮自愈）。退役条件 = 徽记跨局实证（实机连续数局横幅帧阶段一锚命中）→ 达成后删，残余形态落停机留证可接受。运行成本 ≈ 0（`read_ocr_texts` 同帧 OCR 缓存）。
> - **0q 暂不可删**。位面过渡身份锚仍是纯 OCR 小字双锚（「位面」×2，48×26px，conf ~0.95），节点小字持续 miss 可达；0q 与过渡 op 同用提示 rect，派发即可点空白推进，是该形态**唯一推进路径**——删了 = 位面过渡可能反复停机。长期解 = 位面过渡身份锚模板化（节点盘图形），锚硬化后同法评估退役。
...
2. 备战画面 → 派发 `CwScreenPrep`（备战单轮五段，[../screens/prep.md](../screens/prep.md)；环入口清场收编于观察段）。交回契约：op 携带 `launch_fired` 出战事实（通道载体与观测面）；战斗窗置位策略 = 外循环既有口径（ADR-0250：备战环 success → 窗开驻留闩）；
