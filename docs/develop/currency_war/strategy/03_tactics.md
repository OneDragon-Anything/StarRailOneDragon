...
**濒死带支出收窄(C3;ADR-0426 濒死带节)**:应急深带内「再输一场即死」的帧(`filters.dying_band_active` 判定,registry `dying_band_account_enabled`,默认关)命中时,`filter_candidates` 濒死段按 **Δp_board 符号判定**收窄支出——Δp_board = 本轮支出后、下一战开打前的**上场位增加数**,板面不变则胜率不变,删除集支出对任意后续价值期望非正,是可证明零期望的符号判定(纯收窄,只删不增):BuyCard 无空位可上则删(`hoard_buy`——含 final 目标件「买而不上」,濒死帧来不及兑现,设计内意图);LevelUp 升完不能立刻多上 1 件则删(`levelup_no_deploy`——可部署性谓词,非动作类型黑名单,ADR-0302/0303 滤死升级反模式的结构性防复发);RefreshShop 店内无「可本轮买+上」的名集件则删(`blind_refresh`);卖(变现)/部署非支出不辖。名集 = 目标件 ∪ 高费强件(费用 ≥ `dying_band_high_cost_floor`),**只作刷新存在性判据的评分先验,非授权边界**(买谁由 EV 层定价)。辖 plane≥2;与 release FLIP 辖区(hp>`emergency_hp`)零交集。
