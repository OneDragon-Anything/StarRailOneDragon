...
> **修订记录**:r1 对抗回炉(审查 2b362e40,发现清单 = `.debug/temp/currency_war/session_design_attack/发现清单.md`)——A1 补动态属性清册段(§2.6,24 项)并消解遥测承诺矛盾;A2 五字段退役改判迁出(v2_state/locked_line/bridge_id→§2.3,v2_round_*→§2.4),退役降为 4 项全部带前置动作;A3 sim 前提修正(§3.4/§5.6,撤回「零改动」);B1 megastar 落点定案局容器;B2 六处漏消费点补入 §5;B3 恢复局两行评级升「中」+6.2-4 判读义务;B4 create_state 非 abstract 兼容条款;建议 C1/C2/C3 全部采纳,已就地标注。
