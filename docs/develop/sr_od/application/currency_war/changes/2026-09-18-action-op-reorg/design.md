...
| `CollectOreOp`(cw_collect_ore_action.py) | `CwActionCollectOreOp` | collect_ore(session) |
...
> 执行注记(2026-09-18 收口):③a/③b/③c 合并为单个提交(主仓 `d87c2fa87`)——换壳后旧构造签名立即失效,a/b/c 分解无法在不引入过渡垫片的前提下保持每提交全量绿,按「每子步全量绿」硬门合并;双记防线第三处(cw_screen_prep 决策环两处 `apply_prep_action_logic` 直写调用)在执行中确认并同批删除(spec §1.3 未列,属 §1.3.2 同构面)。执行器/落地门分派改**节点直调** `op.run()` 而非 `op.execute()`:单节点动作 op 的框架循环机制(重试/超时/停机查)归包络与交回面所有,动作层零重试语义(§0.7 同源),节点异常原样上抛保持执行异常通道不变。
