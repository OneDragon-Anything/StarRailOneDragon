...
- 现状症状:
  ...
  2. 存在**第二画面 op 基类** `CwProgressionScreenOp(SrOperation)`(`_progression_base.py`,空决策骨架)带 **11 个子类**——与 `CwScreenOpBase` 并存,段迹/端口装配位/on_outcome 注册表只在 `CwScreenOpBase` 一侧,两套基类的机制面永久分叉。
...
- 根因归层:**流程层**(iteration-design §2.1 归层集 = 表示/约定/流程/语义,「结构层」不在集内)——不是语义错,是既定迁移序列(架构设计 §9.2 步骤 4「通过后按同式迁下一画面」)未走完,且变体基类(2026-09-07)与统一基类(架构设计,2026-09-10 定稿)先后立项后收敛动作未执行;两半都是「既定流程未走完」,同归流程层。
...
- 明确不解决(防范围外溢):
  ...
  4. 策略判据、BoardState 字段语义、`cw_loop` 外循环分派逻辑——分别归策略器契约/记录模型设计/正本「基类化不动外循环」裁决(§5.2)。
