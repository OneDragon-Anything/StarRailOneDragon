...
- **hp/掉血类指标校准前禁作 A/B 判据(2026-09-09 在册裁定,与「金类指标校准前禁作A/B判据」条目同族（本节直接条目）)**:sim 的 hp/掉血读数(出口 hp、hp≤20 低尾占比、单轮掉血等)依赖战斗结算校准,该维有已知失真(实证:出口 hp≤20 占比跨批同量级 12.7-14.7%,但 R9 单轮掉血中位 −34,且按板深四分位分组组间零敏感 = ±2 深度桶测不出)。校准补齐前这类指标只作报警线/观察面,禁当 A/B 判据;涉 hp 的结论以实机为准。
