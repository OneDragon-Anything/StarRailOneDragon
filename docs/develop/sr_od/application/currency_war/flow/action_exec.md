...
> T-201 拆分后现位：各动作 op 住 `operations/cw_op/cw_<action>_action.py`（一 op 一文件，聚合注册与分发 = `cw_shop_actions.py::shop_action_op_for`）；守卫断言与执行侧账本面仍住 `cw_shop_action_ops.py`。
