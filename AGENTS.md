...
运行口径：串行为准（`uv run pytest sr-od-test/`），`-n 8` 仅本机可选不作规范。
2026-08-29 实测（5950X/16C，OCR 磁盘缓存关）：全量串行 8:00，`-n 8` 仅 4:28（1.79×），且存在串行/并行红差集（顺序污染未清）与 OCR 跨 worker 重复推理成本——在 ①顺序污染清零并双口径同绿同红复验、②OCR memo 跨 worker 共享两事完成前，`-n 8` 不升级为默认。慢桶机制：单条实测 ≥2s 的用例入 `sr-od-test/slow_marks.txt`（conftest 钩子注入 `slow` marker），L1/L2 带 `-m "not slow"` 跳过慢桶，L3 全量不过滤（慢桶多为行为锁不可丢）；名单由 `--durations` 巡检维护，详见验证分层协议 §5。
