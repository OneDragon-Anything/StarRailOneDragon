...
## Decision
选 B,分三批:
1. **批次1(基建)**:`wait_stable_frame`/`observe_full` 落地;PrepDirector
   环入口与 battle_loop 稳定门改用它(并行期旧路径保留);
2. **批次2(op 降级)**:shop.py 的 HP/gold 识别段、deploy_bench 的
   自读段上收;组合 op 变纯执行器;
3. **批次3(清理)**:删 4 层旧等待实现;钩子守卫统一走 gate。
