...
| [0629-sim-second-engine-caliber-unification.md](0629-sim-second-engine-caliber-unification.md) | ADR-0629: T-212 sim 次引擎期限检查口径单一源化——局终(全账本)口径(T-211 归因候选①;诊断=「never 21 超带」假警报根因:检查器语义声明「至局终」而实现吃 ADR-0362 P1 段切片账本,名实分裂由调用方喂入面静默制造)——run_batch_level_checks 增 full_ledgers 可选参(simulate_p1_batch 直传 results 原账本,缺省回退 ledgers=纯 P1 批零漂移)+检查器内 _game_round_of_row 跨段统一轮轴(单一源=账本行 ts 写入端「跨位面累计,P1 段==rn」;P2 round_num 段内重计直接做差在位面边界回卷,负 gap 并进限期窗=轴混用伪影,T-211 收尾脚本 delayed=34 即此,自洽值 45)+输出 caliber_note 自描述;否决键名标注方案(名字防不了比较错误/planes=1 批名实不符/破坏 T-206 键名连续性);只换本检查输入的辖域申报=其余批级检查判据轮域 P1 锚定(r≥6/r≥7/rn==6),P2 行 rn 1..7 会别名撞进同数值 P1 轮域,P1 视图喂入对其承重;冻结实证=九批全局面 never 序列 2/0/1/11/12/12/15+P1 截断 3/0/1/22/24/20/21 与 T-211 §2 直算逐位一致+s8350 冻结↔收窄重放配对局终口径下零翻转保持(12→12/45→45);验证=新锁 2(语义+接线双钉)先红后绿+受影响面 114 绿 | 已实施(commit 候编排者统一门) |
| [0630-unified-state-journal.md](0630-unified-state-journal.md) | ADR-0630: 统一 state 状态流水(BoardState 收编升级+三渠道写入口+自足快照变更账)——旧 12 流退役(理由=流散乱+写点不全+无渠道签名+无统一 state,非查询/重放问题)后的统一状态记录面:①容器=BoardState 收编升级(write_seq 升格版本 id/写入 API 全带渠道签名/GameState 收编零补域,唯一新增=派生域+上下文域四字段(修订 2026-09-10:双专名 node_inferred/node_observed → top_bar_raw 观察层+node_ord 逻辑层)+receipts 域;另起容器否决双源)②三渠道写入口(obs/logic_action/logic_hook 封闭集+ChannelSig 签名+actor 登记面+域准入白名单+禁旁路 grep 锁)③自足快照变更账(state/journal.jsonl 两行型,行=改了什么+签名+版本 id+写入后完整 state 快照,行行自足零重放零锚零对账;v2 增量账+快照锚+回放自检被用户根本性纠正作废)④派生规则四腿终版(备战/弹窗/0q 位面过渡/0p BOSS简报 专用腿+类型派生,跃迁去重键=(run_id,effective_ord);修订 2026-09-10:守卫族终版 4 员={结算窗,简报,投资环境,等待1-1},0p/0q 出族——「守卫族含 BOSS简报」中段前驱 E12 勘误作废;节点域单字段双值+字段层次终极版,详见 ADR 修订节)⑤receipts 回执域(滚动窗 8,发出即簿记;出战/商店两条点击后重读路径=H1 辐射面勘误在案,v3.3 折入硬义务)⑥影子双写缺省关(旧 12 流零触碰,消费切换挂迁移批次二);实现=R1(T-229)+R1.1(T-232)+R2(T-230)三批同车一笔(T-167 共享文件先例);D3 kind_inherited 保留申报/D4 报告 26→28 锁勘误/基线 3132→3134 勘误在案 | 已实施(验收=三份落地审 accept;commit 随本 ADR 同窗) |
| [0631-match-archive-v12-journal-slice.md](0631-match-archive-v12-journal-slice.md) | ADR-0631: match_archive 装配器 v12——统一 state 新账切片+相对路径物化+旧档案自动重装配补键(统一 state 消费方迁移批 R3-1 落地审 F6「v12 bump 无 ADR」补档,SCHEMA_VERSION 11→12,先例链 v10=ADR-0605/v11=ADR-0615)——①切片 state/journal.jsonl 入 _SLICE_FILES(行带 run_id 同一过滤口径;影子开关缺省关,文件缺席=空切片判「未开」非「丢数据」)②materialize_slice 按切片键同名相对路径落盘+父目录自动建,--match 物化后与 --run 走同一套 query 实现(单一源)③旧档案经 load_archive 版本检查默认就地重装配一次补键,水位线推进同口径自愈(直读 JSON 的离线消费端不吃欠列档案);Considered 否决=第二文件(策略侧决策行)同批加切片(候其落地批,防空引用)/文件缺席告警化(影子期缺省关=合法常态)/不 bump 靠文件探测(版本号=重装配触发器);加法面旧 12 流零触碰;验证=test_cw_state_journal_reader.py 19 锁(v12 切片按 run 过滤+物化还原+v11 旧档案自动重装配补键+CLI 配对拒绝+缺省旧路径零变化)+L1 3199/L3 3725 亲跑 | 已实施(R3-1 批入库 commit f55631c84;本 ADR=T-228 补档) |
