...
| P1 档位推进目标函数(**已删**) | `decision_v2/tier_push` | 旧方案存量目标型杠杆(伞 `p1_tier_push_enabled`+三子旗标默认关)——判据挂账未跑即悬置,经清查零新方案引用,已随清退批(dd-009)按开关生命周期第 4 态整批删除(模块/评分补项/散装门/预算承诺臂/刷新超几何项,registry 位置留墓碑注释);A/B 协议与负结果语义见 ADR-0494 与 prereg/w803 归档 | [0494](../decisions/0494-p1-tier-push-objective.md)+[dd-009](../redesign/decisions/dd-009-legacy-mechanism-eviction.md) |
...
| 执行 | `prep_director`/`prep_actions`/`operations/`(battle_loop+prep+handlers+run_nodes) | 备战决策环、原子动作执行器、op 层;商店链 = `operations/prep/shop.py`(编排壳:前置守卫/HP 关帧读链/关店后重估与对拍)+ `open_shop`/`buy_cards`/`close_shop` 三原子 op(开店幂等/波循环决策执行/关店验证,壳以宿主直调保读屏次序) | [03](03_tactics.md) |
