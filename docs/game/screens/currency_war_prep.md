...
- 「备战栏-1..9」:底部,持有角色(拖拽源)。
- 「前排 / 后排」:舞台部署槽(拖拽目标),舞台带「前台区域/后台区域」文字标签(~906,288 / ~906,564)。**槽位数可变(团队规模机制;用户确认 + equipment.md:211)**:**前排固定 4**(不变);**后排基准 6,财富宝钻(装备,无论是否穿戴)+1 团队规模上限**,诅咒·宝石剑泽尔里奇对应 -1。screen_info 基线 前排×4 / 后排×6,实测见过后排 7(对上财富宝钻 +1)。→ 后排槽位**不可硬编码 6**,deploy 须按运行时实测槽位数 + 布局动态推导(见 gameplay doc「团队规模」)。**⚠️ 布局机制(2026-08-19 狸猫局暗框检测核实)**:槽位增减时**旧槽物理位置不动、新槽加在两端、编号平移**(8 槽 = [464,606,748,889,1031,1174,1316,1458];位2-7 与 6 槽基线位1-6 同位;早前"整体重排"结论系 grounding 换算错误已纠正)。screen_info 已建 ``后排8槽-1..8``(狸猫局实拍);7 槽待实拍(upsert ``后排7槽-1..7`` + ``cw_back_layout._LAYOUT_PREFIX`` 登记)。选档 = ``cw_back_layout`` 按 deploy_cap 路由。**狸猫兄弟(投资策略「龙虎兄弟狸」)**:狸小虎(蓝,弟弟,带【随便骰子】)/狸小龙(红,哥哥)为**固定单位不可拖动**,模板已建(portrait_plaza/狸小虎,狸小龙);identify_slots 对其不报(固定单位无占用边框,slot_occupied 前置门跳过——已知口径非 bug)。
- 「出战」(按钮-出战):右,进自动战斗。
