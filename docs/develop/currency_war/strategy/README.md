...
| P1→P2 接口机制 | `decision_v2/p1_iface` | plane==1 接口五项单一源(与 p2_spend_auth plane==2 正交):①锁线 v2(摇摆代价门=降格事件/连败达阈且未锁→FALLBACK 评估,候选对板面一致性验证交叠≥阈值才锁全不达不锁,消费=update_target 后处理 latch 压制未锁分支重派生;换线=pair 变更事件,降格不计)+②carry 装备(硬节点帧意向核心裸装必分配 duty 豁免 equip_all 过渡 hold;禁无接收者卸装)+③遭遇备战授权(hp−L_node<emergency_hp 只开门,花后金≥rebirth_floor)+④连败金流(连败≥2∧金>息线∧form 缺口→当场可上场体系件授权,纯散件不买,目标件照囤)+⑤血线三带(预警 40 禁纯囤件[压库豁免]/应急带当轮转化豁免[压库不豁免=显式取舍],boss/ALL IN 窗豁免);横切=同帧优先序⑤>④>③、末窗降格独占帧③④不触发、①板面一致性门②③④输入前置;收门统一走约束 `p1_iface_gate`,拦截枚举经 session 披露键进 telemetry(R 族协议面);五开关 `p1_iface_*_enabled` 默认关,开臂=W774 协议 v2(M1-M3+R+G1-G4) | [0484](../decisions/0484-p1-iface-v2-impl.md) |
