...
1. **点候选**(仅当 `StrategyState.megastar_clicked` 为 False,经 kernel `strategy_state_of` 通道读写,状态缺席不冷建):`decide_megastar` 选 idx → 点候选位(「候选-左」/「候选-右」area center,兜底常量 (822,333)/(1061,333);名位置 = 卡身选中区)→ mouse_move + click → 置位选中标记(局级,跨 re-dispatch 持久)→ `chosen_megastar` 写(session + `game_state_of(session).write_logic`,候选选中时点)→ 0.6s。
