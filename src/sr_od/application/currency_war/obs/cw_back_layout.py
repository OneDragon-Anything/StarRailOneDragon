...
**后台 9 格档(已交互建档)**:用户口述真值(2026-09-07/09-11)确认 9 格
可达(宝钻/召唤物扩展封顶 9),e4972b43(cap13 lv8)实拍后台 **9 格**,
几何与居中重排族自洽(暗框/占位 std 剖面峰落 392..1528、pitch 142)。
坐标档 = screen_info ``后排9槽-1..9``(rect = ``[321+142*(i-1),600,
463+142*(i-1),739]``,i=1..9),登记凭据 = 2026-09-12 实机交互实锤
(位面 1-3 lv4/cap7 局:拖拽落位与预测 rect 逐位吻合 + 槽 9 点击开
浮窗——布局/坐标建档唯一终审;实拍帧入库 sr-od-test
``screens/货币战争-备战/后排9槽-cap7lv4*``,4 组 = 首帧/拖拽验证态/
恢复静止态/浮窗遮挡态)。CV 通道 9 格判别 = 外缘探针独有几何签名 +
内缘端点复核(标定分布见 :data:`_CV_OUTER_XS` 注)。
...
#: 9 档已交互建档直读(见 :data:`_LAYOUT_PREFIX` 注)。
_CAP_DIFF_MAX: int = 3
...
#: 探针整窗擦线保守带上界(整窗 ∈ [6.0,12.0) → 不可判退公式)。
...
#: **外缘探针(x=357/1564)——9 格档独有几何签名**(窗 ±71 同 :data:`_CV_HALF`;
#: 标定 = sr-od-test ``screens/货币战争-备战/后排9槽-cap7lv4*`` 4 帧实拍 ×
#: 既有 6/7/8 档 fixture 全量 + 开商店族 10 帧负例纯读扫描,2026-09-12
#: 交互实锤帧族)。
#:
#: 机理(居中重排族带随档对称扩展,格带缘 6/7/8/9 = 604..1316/463..1457/
#: 393..1529/321..1599):357 窗=[286,428]——6/7 格全排外背景(none);
#: 8 格左半窗全排外(l=2.4-2.5 恒定)、右半盖 1 号格左缘 35px(r≈35-47
#: 暗框/立绘)→ slice 恒定;9 格左半窗含 1 号格 [321,428] 共 107px 格带
#: (l=23.5-23.7)→ full(4 实拍帧 asym 2.4-2.5)。1564 窗=[1493,1635]
#: 对称同构:8 格右半恒纯平(r=2.3)→ slice;9 格含 9 号格 [1493,1599]
#: 106px(l=46.1/r=42.1 → full;浮窗遮挡态 l=20.7/r=18.0 仍对称 full)。
#: **8 格误判 9 由几何排除**:8 格下两窗各有一半窗全排外(std≈2.4 恒定),
#: 永不 full。
#:
#: 致命假阳与复核门:**开商店族 10 帧**(shop_open/刷新/锁定等备战画面带
#: 右侧面板扩展)外缘同样 (full,full) 且锚位门拦不住(锚 std 15.5-41.4
#: 全 PASS)——外缘单凭不可判 9。分离凭据 = 内缘端点(464/1458):9 格
#: 4 真帧全部至少一端 full(槽 8/9 或 1/2 相邻占用签名;实测首帧
#: (full,full)/拖拽态 (None,full)/静止态与浮窗态 (slice,full));开商店
#: 族内缘全无 full(8 帧 (None,None) + preview 帧 (None,slice))。故
#: **判 9 = 外缘双 full ∧ 内缘任一 full,复核不过 → 弃权 None 退公式**
#: (误判方向单向封死:弃权只可能少判,公式通道 diff≥3 封顶 9 兜底)。
#:
#: 边界申报(占用态覆盖面):4 实拍帧中 -b(拖拽态)槽 1 空置 = **左 1 空
#: 置有实测**——357 窗整窗 std 12.2(空槽暗框弱签名,紧贴 12 擦线弃权带
#: 上沿)、半窗 (12.5, 6.4) 对称 → full 判据仍成立,但余量极薄;空左格帧
#: 若渲染再淡一点整窗即落 [6,12) 弃权带 → 不判 9 退公式(保守方向,公式
#: 通道 diff≥3 封顶 9 兜底,无误判)。**右 9 空置无实拍样本**:外推依据 =
#: 空槽暗框为对称结构(1564 窗格带侧半窗 106px 远宽于 357 窗左半的 36px,
#: 整窗余量预计显著高于 357 侧),full 判据预计不变;实机空右格 9 格帧
#: 到手后补标。8 格误判 9 由几何排除(其外缘半窗恒纯平,永不 full)。
_CV_OUTER_XS: tuple[int, ...] = (357, 1564)
...
def cv_back_slots(screen) -> int | None:
    ...
    标定注)→ 内缘探针(464/1458,即 8 格档 1/8 号格中心),各探针按
    :func:`_probe_state` 判 full/slice/none 三态,判定序:

    1. 锚位任一无槽签名 → 整帧不可判(overlay 遮挡/非备战态/非 1080p)
       → None(调用方退公式通道);
    2. 外缘 (full, full) → 9 格独有几何候选,内缘任一 full(板面结构
       复核,开商店族假阳内缘全无 full)→ **9**;复核不过 → None
       弃权退公式(不猜);
    3. 其余(外缘非双 full,6/7/8 档全部如此)按内缘组合:
       ...
       - (slice, slice) → 7:两端都是**切片签名** = 7 格档端点格被占(左端
         ...
         漏认 4/6 后排;
       - (none, none) → 6:两端纯背景;
       - 其余组合(full+none / full+slice / slice+none)= 未覆盖形态(如
         端点空槽的暗框半窗分布未标定),保守不可判返 None 退公式(不猜)。

    判定序 9 先于 8:9 格满占用帧内缘 (full,full) 与 8 格同签名(端探针
    在 9 格居中重排下饱和于 8 的已知限制),不先查外缘会把 9 格读成 8。
    ...
    四档几何(居中重排,排中心恒 960,互不共享列位)见模块 docstring。
    ...
    try:
        ...
        if any(a is None or a < _CV_SLOT_STD_MIN for a in anchors):
            return None
        inner = (_probe_state(screen, 464), _probe_state(screen, 1458))
        outer = tuple(_probe_state(screen, x) for x in _CV_OUTER_XS)
        if outer == (_PROBE_FULL, _PROBE_FULL):
            # 9 格档(独有几何;标定与假阳复核门见 _CV_OUTER_XS 注):
            # 内缘任一 full = 真板面结构(相邻端点占用),否则开商店族假阳。
            if _PROBE_FULL in inner:
                return 9
            return None   # 外缘假阳(开商店族形态):弃权退公式
        if inner == (_PROBE_FULL, _PROBE_FULL):
            return 8
        if inner == (_PROBE_SLICE, _PROBE_SLICE):
            return 7
        if inner == (_PROBE_NONE, _PROBE_NONE):
            return 6
...
def back_slots_from_cap_diff(diff: int) -> int:
    ...
    **裁决值域 {6..9}**(上限 9 = 用户口述真值 2026-09-11,机制正本
    ...
    - 裁决值即坐标档:9 档已交互建档(2026-09-12 实机实锤,见
      ...
      (>9),由调用侧 :func:`resolve_back_slots`(运行值 ``n``:未建档 →
      8 格超集)退 **8 格超集**(扩展带读全不丢系统单位;拖到不存在的位被
...
def _observe_back_layout_to_container(ctx, n: int | None,
    ...
    - 值 = 运行档 ``n``(已建档 6/7/8/9 直读;未来新档 >9 → 8 格超集
      运行)——**裁决值与坐标档分离的残余面 = 未建档新档**:容器记运行值,
      evidence 补 ``superset`` 标记(标记判据 = 裁决 raw 值未建档,与留证
      采集钩子同一条判据;设计正本 §3.2.7「域外一律按 8 格超集读全扩展带」
      同口径,防超集近似被当精确值消费;9 档已建档,直读无标记);
...
def resolve_back_slots(ctx: SrContext, screen: MatLike | None,
    ...
    **防抖(W209h/决策 11)**:CV 读出**未建档**新格数(≠公式 且 ∉ 已建档
    档 {6,7,8,9})单帧不行动——重读 2 次三次一致才采 CV 值;任一不一致 =
    ...
    if level_trusted is False:
        ...
        d = 0 if diff < 0 else min(diff, _CAP_DIFF_MAX)
        formula_raw = _BACK_SLOTS_BASE + d            # 裁决真值(值域 6..9;9 = 已建档档)
        formula_n = back_slots_from_cap_diff(diff)    # 同上(残余分离面 = 未来新档 >9:未建档 → 8 格超集)
