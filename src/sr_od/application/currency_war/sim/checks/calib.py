...
# **双锚制**:已申报 sim 策略批(出口金方向性变化链,各批报告
# 均已申报出口金方向性变化)累计把 sim 出口金从 69.35 抬到 ~124,
# 对 45.1 比值 1.54→2.75,本检查自止超发批时代起常红、检测力失效——
...
# 可归因、无未知残差):基座臂 69.35(sim_20260913_050118,卖出口恢复批
# 报告 §④)→ 88.93(修复臂 sim_20260915_033341)→ 99.96
# (刷新止损修前 sim_20260915_030128)→ 124.0(刷新止损修后
# sim_20260915_052726;双窗 123.57/124.05 独立种子复现)→
...
def check_sim_endgold_calib(ledgers: list[list[dict]]) -> dict:
    """末金校准历史锚(ADR-0276/0285 定标;双锚制现役锚见锚注册表)。
...
# sim P1 金分布基线带(双锚制的违规判定带,与 endgold 双锚同批):
# 中心 = 换锚批实测 P1 帧金均值 61.6(n=2700;批目录见
# SIM_ENDGOLD_BASELINE 注释),±5 绝对带宽沿 M1「30±5」结构。
# 实机带(上)降级纯披露:策略省金行为(刷新止损等已申报批)使
...
def check_gold_dist_calib(ledgers: list[list[dict]]) -> dict:
    """`w493_income_calib/` 对拍项:P1 备战帧金分布(双锚制)。
...
# 旧锚(0bf6c0d6,ADR-0362 批)已失效:重锚批(2026-09-09,n=300,
# seed 300-599 与旧锚域 0-299 不相交,planes=1,池=snapshot 快照
# 0e091d4d9d0b10ac+eqg1,批=sim_20260908_235159000_n300_s300_0e091d4d)
# 换锚。**复合漂移如实声明(单变量 bisect 拆解已做,三段归因)**:旧锚→
# 新锚总漂移 = ①池语料前移(0bf6c0d6→0e091d4d,局终自动再生管线持续
# 再生)+ ②工作树在飞策略批(污染声明域:修复前引擎
# 周期/成型率判读不可信)+ ③本批执行面(sim
# 缺板满换血执行面:M1″ 计划非空帧卖 victim+残余补部署,sim 换血行为
# 可见)。③的单变量贡献 = 同 seed 同池 A/B(执行面桩掉对照臂,300 局
# 配对,双臂同树并验策略文件 mtime 签名防并行编辑污染):engines2
# +0.2500 / avg_final_hp +8.7333 / hp_ge_60 +0.0833 / battle_losses_
# le_2 +0.0433 / recipe5_by_r6 +0.1100 / avg_refreshes −1.4267(hp 双
# 指标均带外[hp_ge_60 带宽 ±0.02 / avg_hp ±1.6] = 设计内行为变化非
# 噪声,方向 = 板面更深更成型,与 P24 零支出补部署支配预期同向)。
# 【留痕申报(落地审 F1)】③ 的 B 臂生成方式 = 运行期桩点
# (monkeypatch ``engine_p1.m1p_swap_execute`` 返回 (st, False) 后跑
# simulate_p1_batch(300, seed_base=300, 同池 planes=1, ledger=False),
# 双臂同树以策略文件 mtime 签名前后核验);**B 臂产物未存档**(无批
# 目录、驱动脚本未入库)——③ 六数为单次测量,独立复算须按上述桩点
# 重跑,禁作长期对照基线引用。
# ①+②合计 = 总漂移减 ③,不再单拆(池与策略在飞面持续移动,拆解值
# 不稳);新锚只作「当前树+本批执行面」状态的基线,在飞策略批
# 落地须按本锚再申报。avg_refreshes 25.4 = 现策略泄金阶梯/必花域口径的
# 真实刷新分布(旧锚 0.0 系 08-28 V_D 口径时代策略,锚如实记不折旧)。
# 旧锚(0e091d4d,重锚批)已失效:sim-实机校准批
# (2026-09-15)换锚。换锚批 = sim_20260915_081247000_n300_s77200_
# 460e6031(n=300,seed 77200-77499,与旧锚域 300-599 及卖出口恢复/刷新止损
# 批域 77000-77199 均不相交;planes=1,池=snapshot 快照
# 460e6031e2f4ae06+eqg1)。**复合漂移如实声明**:旧锚→新锚 =
# 前锚后至本批的已申报策略批累计(执行面批 → 止超发批 →
# 六处卖出口恢复 → 刷新止损),单批贡献
# 不再单拆(各批报告已申报);avg_refreshes 25.4→0 主因 = 刷新
# 止损(修前批 sim_20260915_030128 为 14.61/局且全部超账放行 →
# 修后整批 0)。**六指标一体换锚**:登记制语义 = n=300 基线可整体
# 引用,只换单项 = 混代基线(校准设计只点名 avg_refreshes,
# 落地按登记制语义扩展为六项,扩展依据录校准报告)。
ANCHOR_REGISTRY_N300: dict = {
    'pool_fingerprint_prefix': '460e6031',
    'recorded': '2026-09-15(校准批换锚:n=300,seed 77200-77499'
                '[与旧锚域 300-599 及 77000-77199 不相交],planes=1,'
                '池=snapshot 快照 460e6031e2f4ae06+eqg1,'
                '批=sim_20260915_081247000_n300_s77200_460e6031)',
    'metrics': {
        'engines2_by_r6': 0.51,       # 旧 0.85(前锚;复合漂移见上注)
        ...
        'avg_refreshes': 0.0,         # 旧 25.4(刷新止损后 P1-only 载体零付费刷新,见上注)
    },
}
...
# 本口径为基准)。**臂口径注记(投资选卡双臂批前)**:本锚录制于旧频次注入臂
# (现 invest_arm='freq');'sink' 基线臂(真实判据选卡,现 invest=True 缺省)
# 的选卡分布不同,重锚须按臂分别录并写明臂标——跨臂数字不可比。
# ⚠️ 池指纹 1762b1cf 与 `w157_p2/` 锚 0bf6c0d6 不同——快照
