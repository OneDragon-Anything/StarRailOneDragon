...
> 计数/判定统一在 `_dispatch_screen_op`（hook 早退位次之后，hook 仍可短路）。链形透传分支（3c）不经本网（各有自身预算）。
...
> 位面过渡的误分发根修在识别层：BOSS简报锚换装徽记模板（OCR 误读免疫），阶段一位面过渡身份臂挂 boss 判别排他（接管派发 boss op）。
...
触发 = 备战栏槽位占用但 SIFT 未识别（非角色非已知箱/典籍；金币说明 overlay 开着则跳过本帧判定——overlay 盖住备战栏右端会制造假「占用未识别」，局 69 实证）。处置 = `stop_running(save_screenshot=True)` 框架留证。**不能降级**——未建档物品被当空槽/普通占用乱操作比停机贵（用户 2026-08-20 纠偏）。
...
触发 = 效果清单含候逐条确定的名单效果（`_SPEC_INVEST_WATCH_NAMES`，挂点 = visit_open_shop 入口）→ 直接停机（`stop_running(save_screenshot=True)`），**人工采集**——停机后画面原样保持（店开着）：人工看画面 / MCP 截图补帧，对照效果注册表逐条确定该组合下商店改写/计数/经验行为，结论回填效果规格 verdict/notes。同组合进程内只停一次（重启清零 = 允许重停）。采够后删整段（常量+方法+挂点两行），不留开关。
...
|---|---|---|
| gold 失读（shop 关态） | 备战入口 heavy 观察重试 | 宁可重观察,不造值 |
| 卖出执行守卫 | 守卫断言唯一 = `guard_proposal_vs_expected`(`cw_shop_action_ops.py`;提案动作的对象在期望态中存在且未被消费,非法 = 策略器 bug 响亮炸出;零读屏)。卖前双账对拍 `guard_expected_vs_tracked` 已退役——对账唯一合法时点 = 观察边界(观察 vs 逻辑双态比对,kernel `cw_reconcile` 执行),op 层零双态比对 | 不卖错件(卖出不可逆);逻辑态建模 bug 检出归观察边界 reconcile 纠漂显影(观察赢),详见 `../screens/op-layer.md` §1.3/`action_exec.md` §4 |
| 部署 cap 失读 | 单调链 max 兜底；全源失读 → 不设板满门（拖到游戏拒即真值） | 低读阻塞上阵（贵）> 高读白拖一次（便宜） |
| 空部署计划 | 部署候选单一源 = `cw_deploy_logic.select_deployments`,空候选不提案 | 空计划 ✓ = 假成功形态(候选单一源空不提案为唯一防线;停顿监测归哨兵) |
