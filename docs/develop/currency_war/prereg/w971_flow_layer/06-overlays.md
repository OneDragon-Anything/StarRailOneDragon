...
③ 原子 op 执行 → 完成承诺 = **等备战商店开画面出现**(场景①判稳),交回循环
```

- 原子 op:RefreshSupplyCard(次数约束 P8)/ SelectSupplyCard / ConfirmSupply(末位契约)。
...
④ 原子 op 执行 → 完成承诺 = **等备战商店开画面出现**(场景①判稳;遭遇选择 =
...
最简画面 op:前置 = 循环识别 BOSS 简报(「标识-强敌来袭」,#26 建档);动作 = **点击空白**(推进);完成承诺 = **等备战商店开画面出现**(场景①判稳,与节点后 overlay 同口径;交回循环重识别兜底)。
...
| overlay 族 | 确认后 | 完成承诺 |
|---|---|---|
| **节点后四类**(投资策略/补给/遭遇/BOSS 简报) | 回备战 → 商店自动开 | **等备战商店开画面出现**(场景①判稳) |
