...
1. **池派生住 sim,kernel 采样链够不着**:`sim/cw_sim_pool.py` 四函数
   ...
   (口述·权威 2026-09-20):**随机获取角色效果的采样空间 = 剩余池对应费用档桶**
...
| 项 | 排除理由 |
|---|---|
| 装备池统一(`furnace_reroll` × `tool_use.py` 双实现) | 对拍锁已护;角色池先行验证升格形态,装备池另批候选 |
| 奖励球是否池驱动采证 | 效果批辖 |
| 策略估值族档传参(`refresh_prob` 等) | 精度面,另行挂账(银狼升星记账批 §2.6 已列) |
...
| 装备池统一 | 不进本批 | 顺手统一 `furnace_reroll` 双实现 | 角色池先行验证升格形态;装备池对拍锁已护,另批候选 |
