...
(``SrOperation.__init__`` 会构造 ``OpenAndEnterGame`` 等有副作用的依赖)。
供 ``list_operations`` / ``describe_operation`` / ``run_operation`` 复用。

详见设计 spec §4.4(扫描根 operations/、三重过滤、op_id 拆分、纯反射)。
"""
import importlib
import inspect
import json
...
def _annotation_coercible(annotation: 'Any') -> bool:
    ...
    (如未来 SR 的 ``@dataclass``+``from_dict`` 计划项)。非 dataclass / 无 ``from_dict`` → False(仍需走 application)。
    """
...
def scan_operations(ctx: 'SrContext', refresh: bool = False) -> OperationListResult:
    ...
    Args:
        ctx: SrContext(扫描纯反射,占位;保持与其它 backend 接口一致)。
...
def resolve_op_class(op_id: str) -> type:
    ...
    Args:
        op_id: 定位标识,如 ``sr_od.operations.move.move_directly.MoveDirectly``。
...
def describe_operation(ctx: 'SrContext', op_id: str) -> dict:
