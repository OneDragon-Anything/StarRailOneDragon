...
| invest_cards | 字段收编候选+流退役 | 候选卡面+效果原文→画面 payload 域 `strategy_offer`(离屏即 None);注册表 ground truth 回流用途不变;逐局原文语料断供已定谳 = 接受断供(写入端已随删除波 1 退役),定谳与恢复路径 = ADR-0643 |
...
| 10 | 策略侧决策行(现役 DecisionTrace) | 瘦身演进落文+接线;行面钉字段 = state_ref 单字段(过渡钉面标记 pin_scope 已退役删除——候裁 10 终裁 C 案,历史档案判读知识见 [决策行文件schema设计.md](../design/决策行文件schema设计.md) §8-2) | 候 | 接线 = W6 后另批(决策行文件落地批,T-5 验收绿后立卡,定谳 C6) |
...
| 2026-09-11 | §2 invest_cards 行「停写前须定谳」兑现落档:定谳 = 接受断供,恢复路径 = strategy_offer 域建模(挂 W5 评估);同批落 obs 缺陷 refs 指冻结档案的判读指针处置(ADR-0641 后果 2/3 两件收口) | ADR-0643(定谳)/ ADR-0641(删除波 1——写入端先于定谳退役的波序事实如实申报) |
| 2026-09-12 | §7 #10 行修订:迁移动作列「消费切换子集落地前 state_ref 带 pin_scope 标记」随字段退役删除作废——行面钉字段 = state_ref 单字段,历史档案判读知识指 schema 正本 §8-2;批次列改「接线 = W6 后另批(定谳 C6)」 | 候裁 10 终裁 C 案(定谳记录 = changes/2026-09-11-unified-state/定谳记录-候裁7910.md);字段退役执行 = T-99(commit b0775d8b) |
