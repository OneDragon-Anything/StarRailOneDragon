...
**范围**:`CwProgressionScreenOp` 改继承 `CwScreenOpBase` + 变体五段钩子缺省实现(ADR-0584 空决策合同逐字保留)+ `handle` 顶部分流;11 子类零改动随之收敛。边界:不改任何子类文件;不改变体骨架语义(预算 2/重入裁决/免锚形态)。
...
- ADR-0584 既有行为锁原样保绿(锁红先按详设「测试与锁面」节重推锁语义,修锁在报告对照表申报);
- 写入流对拍:本阶段无适用面(ADR-0584 空决策合同零写端,如实申报;design.md §2.2-4);
- §12 通用工程门(同上)。
**验收凭据形式**:变体结构锁 + ADR-0584 相关锁 + L1 快速集输出。
