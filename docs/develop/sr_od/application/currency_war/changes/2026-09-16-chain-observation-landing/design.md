...
2. **备战帧现行链写端（prep_row）——挂点 = 备战入口 heavy 观察的节点行读点**：
   ...
   - **逐格映射**（对抗 F1-2：current 直读恒 None 是探针 Hu 匹配器的现实，入口写端按以下取值链）：past → `channel='dim'`/token=None（已通过段不由本帧负责，链正本 §3）；current → token=**左移推断值**（载体 = 链自身上一帧——上一帧该位为 upcoming 的 Hu 读数即本轮类型，`channel='hu'`，自锚定免新增 anchor 状态；上一帧无该位 Hu 值（首帧）→ `channel='none'`/None，与现役台账 fail-open 等价；草案期「OCR 标签兜底」随实现收窄省略——slots current 直读恒 None，兜底为无效分支）；upcoming → `channel='hu'`/token=Hu 匹配，`hu_dist>HU_DIST_UNRECOGNIZED` → `channel='none'`/token=None；boss 末槽 → `channel='sift'`，**SIFT miss → none/None，禁回落 Hu 判定**（boss 槽 Hu 距离系统性不可靠，cw_node_reader:218-222 在案）。左移推断的载体 = 链自身上一帧（自锚定），非 session anchor 字段（验收④措辞对齐：功能等价，免与探针双写竞争）。
...
4. **过渡屏行归属用高亮晶矿识别 vs 位面序号推导**：放弃晶矿识别——高亮晶矿跟随「已到达位面」而行内容是「刚离开位面」，两者错位；「到达位面-1（下限 1）」推导零识别成本且三情形（开局/1→2/2→3）统一（对抗 F10 的接管残余风险申报见 §2.1-3）。
