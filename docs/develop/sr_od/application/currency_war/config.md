...
> 本文回答「配置面该有什么、不该有什么」。决策依据 = 用户画像定调 + 单一职责原则。
> 字段实现 = `currency_war_config.py`
> (行为与当前值在代码,本文只记语义与归属)。
...
违反即双源或错位(反面案例:原 `dot_punish_envs` 把游戏客观数据放进了配置,与注册表双源,且注释把词缀标成「投资环境」——已删)。
...
> 原 `event_whitelist`(恒最高 boost)已删:「指定具体分值」是引擎调参非用户偏好,
...
| 字段 | 定位 | 语义 |
|---|---|---|
| `strategy_id` | **用户面** | 选策略插件(`default` = 不配置即内置打法;现行生产 v2 = `decision_v2`;旧 `line_v2` 已删) |
...
> 已出清字段(`economy_mode`/`event_whitelist` 删、`hp_safe_threshold`/`difficulty_hp_override` 迁代码常量 `cw_state.HP_SAFE_THRESHOLD`/`DIFFICULTY_HP_TABLE`、gate_* 4 个 yml-only flag 删)的**why 与过程 → git 历史**;阵营轴保留与必含轴确认的用户裁定 → 用户裁定档案(已随裁定档体系退役)。

## 5. 待用户定的事项

无待定项;后续配置演进(GUI setting card / 预设)按 §3 目标态实施。历史裁定(阵营轴保留/必含轴/§4 出清)的 why 归设计文档动机段与 git 历史,变更过程不再在此记流水。
