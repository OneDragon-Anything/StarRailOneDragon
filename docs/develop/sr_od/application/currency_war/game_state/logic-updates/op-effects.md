...
工具族七件的逐类写端申报专篇 = [tools.md](tools.md)(机械半 + 逐类写端表 + 判据准入;工具七类 = 容器直写(`gs.equips` 域合并单笔),黑板帧面随 `gs.prep_obs` 退役删除)。本总述只留机制锚:上报写端 = `kernel/cw_action_report/tool_use.py` 七具名上报函数(`_REPORT_BY_CLASS` 按 param 类型解析),写端申报单一源 = `EQUIP_WRITE_SIDES`(§3),桥语义 = §4。

## 6. 动作侧计数面(效果账计数)

- `CounterKey.BUY`:买牌上报函数获取计算完时点回调 `gs.effects.on_buy` 推进(`kernel/cw_action_report/buy_card.py`;对全部声明 `duties.track` 的在册条目推进,消费按 (spec_id, key) 隔离读;满栏一击多买按实际张数 k 计 k 次,live/sim 同源)——返利族「每购 3 张 5 费」计数面;
- `CounterKey.REFRESH`:刷新上报函数统一触发(`report_action_refresh_shop_param` → `gs.effects.record_refresh`:refresh_total 恒 +1、付费帧 refresh_paid +1、同帧 bump)——采购专员族「每刷 7/5 次」触发面 + 选卡评估全量计数(refresh_total/refresh_paid);
...
注册表 pick 族 5 行逐类专篇:[pick-encounter.md](pick-encounter.md) / [pick-supply.md](pick-supply.md) / [pick-megastar.md](pick-megastar.md) / [pick-partner.md](pick-partner.md) / [pick-planner.md](pick-planner.md)。本总述只留族共通机制锚:op 类 = `operations/cw_op/cw_pick_xxx_action.py` 五类各一文件(`CwActionPickEncounterOp`/`CwActionPickSupplyOp`/`CwActionPickMegastarOp`/`CwActionPickPartnerOp`/`CwActionPickPlannerOp`,域 env = `cw_overlay_pick_env.py::OverlayPickExecEnv`,机械参数由各画面 op 决策半现算传入,op 类体内零决策);容器 GameState 零写(事件线选择非逻辑态通道),写边只有确认到账 grant(CwActionPickSupplyOp 一类,经 `register_confirm_arrival` → `apply_confirm_effect` dict 分支)与 chosen_* 观察写端(画面 op 类名作 actor 标注;策划选择落地域 `chosen_hack` 现役无写入端);机械确认链纪律正本 = [flow/action_ops.md](../../flow/action_ops.md) §2.3/§4.4。
