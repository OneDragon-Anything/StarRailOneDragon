...
- **C 类:处置裁决本身**(DISPOSITION 文件里的「接受/驳回/部分采纳」理由,尤其 DESIGN_FINAL_ATTACK 阻断-1(A/B 过线才删旧包的时序裁决)与阻断-2(轮岗语义勘误的用户方法论裁定)——这两条是**活的约束**,01/02/03 都在引用)。→ **晋升**:将 DISPOSITION 报告晋升为持久文档,建议 `docs/develop/currency_war/decisions/`(与 ADR 体系衔接:阻断-1/-2 各一条决策记录,或并入现有 ADR 序列)。晋升后正文标记改为指向该持久路径。
...
| N2 | PHASE2_PACKAGE_REPORT.md | 进度/变更叙述入 docs | 整文件是「本次实际改动清单+自检结果」= 实现进度+变更史,规范「实现进度不进任何文档,归进度追踪;变更史归 ADR」 | 迁至 `.debug/progress/` 当前迭代目录或 decisions/ 存档;其 §4 G2(毕业判据挂账)需先确认已落 01 §8(已落,G2 条款在 01 §8 ① 可查)再迁 |
...
**修(按优先级)**:
1. §1.2 C 类:DESIGN_FINAL_ATTACK 阻断-1/-2 处置报告晋升为持久决策记录(decisions/),01/02/03 引用改指持久路径——Phase 3 执行门不能挂在 temp 上。
