...
| 位面 2 支出授权 | `decision_v2/p2_spend_auth` | P2 中段定向支出授权谓词与预算带单一源:四条件合取(线锁定[消费 P16 滞回后 v3 locked 位]∧形态缺口[form_ok 锁定线分支取反]∧g≥息线)+加急层升级开关(连败/hp 警戒带报警面);两层预算带(常授权=花完仍≥息线,贴线带 P25 待证占位保守不买;加急=保留金下限破息);消费点=arbiter 金地板授权臂/息账加急豁免/非正分门核心必买;接管帧(分配器 v6 active)禁出手,末窗与覆盖纪律态归既有族,升级与 P12 负例零触碰;开关 `p2_spend_auth_enabled` 默认关,开臂=P25 证成∧A/B 协议达标(双钥匙,见 ADR 挂账) | [0480](../decisions/0480-p2-spend-authorization.md) |
