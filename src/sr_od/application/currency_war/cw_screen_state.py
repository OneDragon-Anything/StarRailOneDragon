...
# 大厅态白名单(非对局屏,必须显式排除):
# - 大厅/模式选择/攻略系/弹窗系/阵容编辑:入口导航层画面,不是对局中态。
# - 列车补给每日弹窗:盖在大世界之上、早于 CW 入口首段导航的**非对局屏**。
#   必须显式排除——in_match_screen_names 按 货币战争- 前缀自动收屏
#   (match2 实锤 2026-08-31:弹窗帧被误判「已在对局中」→ 跳过 enter/start
#   直交 loop → 未知态钩子 33s 停机)。
# - 星琼详情/星徽详情/积分奖励(ADR-0574):入口链路径上在场即误判的同类非对局屏
#   ——星琼详情为模态弹窗(2026-09-07 实机卡死事故主体,守卫=入口链共享助手),
#   星徽详情为同族「标题+X」详情弹窗(已建档,处理分支属同族泛化待办),积分奖励
#   为入口链 3b 分支处理的局末奖励页(cw_entry_start 承接);三者屏名带前缀会被
#   自动收屏,在场即误判「已在对局中」跳过 enter,卡死换姿势复发。
