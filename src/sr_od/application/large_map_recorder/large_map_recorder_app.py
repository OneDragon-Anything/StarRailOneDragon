...
from sr_od.application.large_map_recorder import large_map_recorder_utils
...
class LargeMapRecorder(SrApplication):
    ...
    RUN_MODE_ALL: str = 'all'
    RUN_MODE_SCREENSHOT: str = 'screenshot'
    RUN_MODE_MERGE: str = 'merge'
    RUN_MODE_ADJUST_SIZE: str = 'adjust_size'
    RUN_MODE_SAVE: str = 'save'

    STATUS_FULL_FLOW: str = '完整流程'
    STATUS_STOP_AFTER_MERGE: str = '合并后结束'
    STATUS_STOP_AFTER_ADJUST: str = '调整后结束'
    ...
    def __init__(
            ...
            debug: bool = False,
            run_mode: str = RUN_MODE_ALL,
    ):
        SrApplication.__init__(self, ctx, large_map_recorder_const.APP_ID, op_name='大地图录制 %s' % region.cn)
    ...
    @operation_node(name='准备执行', is_start_node=True)
    def prepare_run_mode(self) -> OperationRoundResult:
        if self.run_mode == self.RUN_MODE_ALL:
            return self.round_success(self.STATUS_FULL_FLOW)
        if self.run_mode == self.RUN_MODE_SCREENSHOT:
            return self.round_success(self.RUN_MODE_SCREENSHOT)
        if self.run_mode == self.RUN_MODE_MERGE:
            return self.round_success(self.RUN_MODE_MERGE)
        if self.run_mode == self.RUN_MODE_ADJUST_SIZE:
            return self.round_success(self.RUN_MODE_ADJUST_SIZE)
        if self.run_mode == self.RUN_MODE_SAVE:
            return self.round_success(self.RUN_MODE_SAVE)
        return self.round_fail(f'未知运行模式 {self.run_mode}')

    @node_from(from_name='准备执行', status='完整流程')
    @node_from(from_name='准备执行', status='screenshot')
    ...
    @node_from(from_name='准备执行', status='merge')
    ...
    def merge_screenshot(self) -> OperationRoundResult:
        ...
        if self.run_mode == self.RUN_MODE_ALL:
            return self.round_success(self.STATUS_FULL_FLOW)
        return self.round_success(self.STATUS_STOP_AFTER_MERGE)

    @node_from(from_name='准备执行', status='adjust_size')
    @node_from(from_name='合并大地图', status='完整流程')
    ...
    def adjust_size(self) -> OperationRoundResult:
        ...
        if self.run_mode == self.RUN_MODE_ALL:
            return self.round_success(self.STATUS_FULL_FLOW)
        return self.round_success(self.STATUS_STOP_AFTER_ADJUST)

    @node_from(from_name='准备执行', status='save')
    @node_from(from_name='调整地图大小', status='完整流程')
