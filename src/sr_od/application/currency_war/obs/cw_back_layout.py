...
#:
#: 边界申报:9 格 4 实拍帧边格(1/9 号)全被占用,「边格空置」形态无实拍
#: 样本——空置时外缘窗内格带=空槽暗框(同档空槽中心窗整窗 std 实测
#: ~27,8 格档空槽满窗 38.8),暗框为对称结构预计 full 判据不变;风险位 =
#: 357 窗左半仅含 36px 暗框,若其 std 走低致不对称比破 2.5 → slice →
#: 弃权退公式(保守方向,无误判);实机空边格 9 格帧到手后补标。
