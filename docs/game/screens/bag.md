...
## 识别快照(analyze_screen 实测,2026-09-15,遗器态选中详情)

- 匹配画面:`背包-遗器`(bag_relic)`is_precise=true`(三锚:一级标题-背包 + 二级标题-遗器 + 按钮-分解 全命中)。
- 画面态:遗器列表 + 右侧遗器详情面板(选中 +15 遗器:套装名 / 强化等级 / 属性 / 二件套说明 / 装备中标记 / 详情 / 重掷 按钮)。
- fixture:`screens/背包-遗器/选中详情.webp`(源帧 = 2026-09-15 退局停机钩子留证,见下「跨屏误匹配加固」)。
...
- **跨屏误匹配加固(2026-09-15)**:背包-遗器详情区说明文本含「装备者」类词,曾使单锚屏「货币战争-备战-角色信息提示」(锚 text「携带装备」lcs 0.5,「装备」二字按序出现即达标)在本页误命中 → 对局中误判停机(返回大世界兜底触发)。修复 = 该锚 lcs 收紧 0.75(本页 OCR 实测「二件套:使装备者的能量恢复效率提高」对「携带装备」lcs=0.5 不再命中;归档帧 4/4 全等仍命中)。教训:异系单锚低阈值屏对含高频词(装备/角色/强化)的背包详情文本天然脆弱——新增跨玩法单锚 id_mark 时须对背包详情文本做撞车检查。
