...
## 装备数据(已抓,../data/equipment.md ~130 件)
...
## 数据需求(游戏边界)
- 装备图鉴:../data/equipment.md(已抓,🟢)。**非游戏**(meta 数据)。
...
## round 3 补充(P1)
- **P1-4 装备合成树(recipe graph,high)**:07 加了 ComposeEquip 但**无合成树数据**(哪 2 简易→哪个进阶)。从 ../data/equipment.md 提 `EQUIP_RECIPES: dict[adv_name, (simple_a, simple_b)]`;ComposeEquip 查表(当前"有 2 同类简易→合成"太模糊,无具体配方无法决策合成什么)。建模**机会成本**(合成消耗 2 槽 vs 等更好 T0 drop)。
