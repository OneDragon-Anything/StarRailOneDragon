...
# ⚠️ 数据复核修正(用户抓的统计错误):入包双条件 = **Early 出现率 ≥8%** AND
...
# 本副本仅为 sim/旧判据未迁消费点保留,随处死计划批 2 随文件删除;勿新增消费。
TRANSITION_PACK: dict[str, tuple[str, str]] = {
    ...
    '符玄': ('量子', 'partial'),        # 4费(Lv5 起 2%——审计修正:非 2 费;
...
# 移除记录(数据复核):瓦尔特(5费 Early 0.2%=终局 core,非过渡件;列车终局 comp
# 已有)/娜塔莎(0.7%)/佩拉(1.1%)/丹恒·腾荒(6.8%)——Early 资格不足。
# 花火入列车框架后不动(她双 flow:列车阵营+量子;框架计数按阵营,量子侧靠
...
FRAMEWORK_FACTIONS: dict[str, tuple[str, ...]] = {
    ...
    # 框架映射随桥删除(迁移审计判定,出处见 git 历史:已封存线不再有框架豁免通道);
...
def pick_framework(bench, deployed, shop=None, current: str = '', portal: str = '') -> str:
    ...
    **开局先有鸡还是先有蛋修正**(局29 实证):旧逻辑 shop 半权只在框架
    ...
    **预囤修正**(蒙特卡洛 2000 局实证:仅「合并权启动判定」不够):单框架/商店期望仅
    ...
    **0.5%**——死锁只是缓解未破。破法=**未定框架期「见框架件就囤」**(买最便宜
    ...
    # 审计修正:平局按 dict 序偏仙舟(FRAMEWORKS 首位)——主流先验(32% vs 29%),
    # 有意为之:同计数时选数据上更主流的框架。
    # 启动门槛从「持有 ≥2」放宽为「合并权 ≥2」(持有 1+在售 2 即启动);
    # 预囤在位后启动门 1.5(纯 shop 1.0 仍不够)。
    # 审计修正A:**选定与保持解耦**——合并权 <1.5 时,现任持有权 ≥1 仍保持
...
#: 阈值取 5.0(旧值 3.0 下 词缀1.5+策略2.0=3.5 即 ready → r1-r2 必定型,
...
def transition_score(char_id: str, faction: str, framework: str = '') -> float:
    ...
    审计修正B:预囤只对 **carry/partial**(囤了围绕它走);drop 档返 0
    (应急件,囤了 P1 末就卖 = 浪费金,与 recipe 追买口径一致)。
    阵营兜底(局38 复盘终判「列车件池只有 5 人?」→ 实 8 人:配方羁绊
    ...
    if ent is None:
        ...
        if not framework and tier == 'drop':
            return 0.0   # 预囤模式不囤 drop(审计修正:预囤只囤 carry/partial)
        if fw == framework:
            base += 0.3   # 同框架集中
        elif fw == '通用':
            base += 0.15  # 通用插件次之
