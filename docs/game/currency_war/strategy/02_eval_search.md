...
**hp 阈值统一走 config(r5,代码漂移修;DEFERRED D-18)**:当前 `_phase_weights` 硬编码 `hp < 40`(`cw_decisions.py`),方案各处 hp<30(转型)/hp<50(连败)。**阶段 2 改**:所有 hp 阈值走 `config.hp_safe_threshold` + 偏移系数(转型 0.75×、死局 0.5×、连败 fold 1.5×),删硬编码;`_phase_weights` 签名加 threshold 参数。**当前仍硬编码 `HP_DANGER=40` + `maybe_pivot hp<30`**(触及签名+测试,单列任务,非漂移;D-18 deferred)。
