...
- 「出战」(按钮-出战):右,进自动战斗。
- **角色详情面板(子态)**:点舞台**已部署角色** → 右侧弹详情面板(~x1400-1840,y370-780):角色名 / 位置标签(前台·后台)/ 阵营图标 / 星级 / 描述 / 携带装备(特殊角色标「装备无法拆除」)。**= deployed 身份 ground truth 源**(task 角色身份感知)。出口:**点面板外空白处关闭**(re-click 同一角色不 toggle;⚠️ 勿 ESC = bug#2 中断挑战)。fixture:`screens/货币战争-备战/char_detail.webp`(2026-08-09 实测佩佩:双阵营 击破+夜之半神、3★)。
...
- **装备区(区域-道具装备 x1252-1918)= 装饰球体 + 扳手(①-a 2026-08-09 live 实测推翻 D-18 旧判)**:r1-8 详情面板关时 VLM 实测 —— 装备区**只有顶部扳手(y90-120)+ 装饰球体(y130-710,蓝/灰球)**,**无 owned 装备 icon / 装备槽**。原「装备槽右列 x1805-1885」是 D-18 crop VLM 误判(球体当装备槽),①-a 实测推翻。**owned 装备不在备战装备区**。
- **角色装备槽 = 详情面板**(点 deployed 角色 → 右侧面板):3 槽横向,终结技区下 / 装备推荐上(y~700-800;**精确坐标待 crop 交叉** —— VLM grounding 与 OCR「装备推荐」@1465,802 位置冲突,单次 grounding 不可信)。r1-8 藿藿 **3 槽全空(裸装,深色 □)**。**owned 装备源(从哪 drag 到槽)待探**(点槽弹 owned 列表?装备推荐弹推荐列表 D-18)。装备机制 = drag(D-18)。`equip_all`(x1800-1918 亮度检测)整个建在错认知上(装备区是装饰球体非装备),待①-a 完整建档(精确槽坐标 + owned 源 + 三态)后重写。详 D-23。
