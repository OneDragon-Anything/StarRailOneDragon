...
> 本篇为新写薄篇：pick 族 = CwStrategy 的 9 个选项决策钩子（`strategies/impl/cw_strategy.py` + `strategies/impl/flow.py:298-487` 实现反向 + `kernel/cw_events`/`kernel/cw_comps` 判据单源）。事件面目录（E1-E18）重排自原 `03_strategy_layer.md`(已删除) §4.9（E16-E18 = r104 后补三钩子，本批补录）；数学判据逐项状态登记沿用 [08_events.md](08_events.md)（该篇管"目录全量/数学全空"的落差，不动件）。
...
| `decide_planner(options)` | E16（银狼策划事件：升费卡二选一，r104） | 委托 `cw_events.decide_planner`（升费卡打分含银狼线/在场判定；target_comp 决定银狼线加成） | **有规格**（知识判据；r104 用户定调） |
| `decide_star_tome(options)` | E17（星徽秘典四选一，r104） | 打分：①target_comp.all_factions 命中 +`PICK_BIAS.tome_target_faction`；②board 已有该阵营（板计数 ×`PICK_BIAS.tome_board_hit`）；③当前配方框架阵营命中 +`PICK_BIAS.tome_framework_faction`；无命中 fallback idx=0 | **有规格**（结构打分，非数学门） |
...
| `decide_box_card(names)` | E18（武装箱/节点弹窗四选一装备卡，r104） | 打分：①target.key_equips 命中 +`PICK_BIAS.box_key_equip`（成型加速压倒）；②合成材料通用性（`cw_prep_expect.material_value` 配方数）；③key_equips 的合成材料（两跳，读 EQUIPMENTS.recipes）命中 +`PICK_BIAS.box_key_material`；fallback idx=0 | **有规格**（结构打分） |

## 2. 事件面目录 E1-E18（原 03 §4.9 收编 + r104 三钩子补录 E16-E18；判据状态详见 [08_events.md](08_events.md)）
...
| E16 | 银狼策划事件（升费卡二选一） | 策划节点（r104） | 升费卡打分含银狼线/在场判定；target_comp 决定银狼线加成（§1 `decide_planner` 行） | r104 用户定调；cw_events |
| E17 | 星徽秘典四选一 | 星徽秘典节点（r104） | 结构打分：target 阵营命中/板面已有阵营/配方框架阵营命中三层 × PICK_BIAS（§1 `decide_star_tome` 行）。与 E8（虚数件驾驶员指派）同属星徽域但是不同决策点——E8 定「谁驾驶」、E17 定「选哪张秘典」，不归并 | r104；pick_bias |
| E18 | 武装箱/节点弹窗四选一装备卡 | 武箱子节点（r104） | 结构打分：key_equips 命中优先/合成材料通用性/key_equips 材料两跳命中（§1 `decide_box_card` 行） | r104；cw_prep_expect |
