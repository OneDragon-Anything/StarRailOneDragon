...
- **存量档案(O2)**:v8 落地后任一读取触发即把 v7 档案重装配,loss_nodes 数值原地从净额变战斗腿(如 g_20260906_182456 p2r4 −19→−33)。符合「能修复就修复」数据治理判据;历史判读记录(复盘文档/抽查对照)按当时口径成立不改——p72 F11 与 sim-design P72-6 两处口径注已加 v8 勘误指针,防「文档 −19、档案 −33」被判数据伪造。
  - ⚠ **勘误前向指针(v9,ADR-0577 §4)**:上例 −33 经取证证实为合成行鬼值(补给行 hp=45 为陈旧快照非真实回血,帧序 18:53:49→18:57:42→18:58:00 亲算在案),v9 起合成行一律退出步进链,182456 重装配直读 **−19**(鬼值消除;另消 p1r6 幻影 −2);p72/sim-design 两处口径注已随 v9 二次更新;T1/T2 夹具按锁的存在性纪律重推(依据=ADR-0577 §4)。本节立案叙述与 O2 示例保留作勘误链存档,禁按 −33 判读现档案。
