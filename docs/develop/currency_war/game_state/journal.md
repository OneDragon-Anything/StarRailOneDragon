...
> 命名对应注)。本文 = 流程侧遥测正本的记录机制面;设计裁定与 why 的持久锚 = ADR-0630。
...
段界三窗(完整契约见 ADR-0630 引用的遥测正本 §3.2.2 规则 6):局间旧 id 驻留窗
...
- **效果域捕获声明**:效果账本域(effects)保持就地可变 inventory 方法域,不设专用
  ...
  生命周期/规格声明)= 独立效果域设计件(候成文),本文只管捕获面。
- 预期簿记不单独产行:挂起面随快照可见,产生/清账由相邻行差分人读可见。

## 5. 落盘与查询

- **落盘形态**:内存追加+规范化序列化;磁盘批量 flush(阈值随实现批定),同步关键
  路径零 open/write。体积=单行全量快照所致显著大于旧单流,实测口径(单行写耗时
  p50/p99/备战链增量/单局体积)挂影子局实测候裁;压缩候选须保行自足。
...
- **查询模型**:判读/装配/策略回溯=按行直接读(单行自足);跨行对照=行间差分(人读
  ...
  派生,不设系统机制。
- **读向隔离**:决策输入禁读状态流水(ADR-0577);运行时控制面读口=显式豁免类
...
| 审条目 | 落点 | 修正 |
|---|---|---|
| F4 | §6 write_logic 豁免枚举 | 补「动作发射写点 receipts」,与遥测正本 §3.2.5 写点口径对齐 |
| F5 | §6 豁免枚举 | 「事件屏 chosen 族」首现加释义(事件屏选择结果字段族 `chosen_*`,容器正本 §3.4) |
