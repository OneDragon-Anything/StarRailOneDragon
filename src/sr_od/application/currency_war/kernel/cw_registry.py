...
class DecisionV2Registry:
    ...
    # ===== 位面 2 支出授权(W757 v3.1 两通道版落码;决策 why=ADR-0480/0481)=====
    #: 语义(v3.1):plane==2 中段的**两通道**支出授权——通道 A 定向授权
    #: (T1 锁定∧T2 缺口∧T4 g≥息线;T3 升格加急层)+ 通道 B 止血授权
    #: (T3∧T4∧帧资格,与锁定位/形态缺口解耦;方向梯级 = hoard→桥池→
    #: [31]① 空窗填充,纯散件恒不授权,bench 保留 ≥1 空槽)。预算带对齐
    #: (「只开门不收门」):常授权层无独立地板(与既有息账门同判),加急
    #: 层破息至保留金下限;末窗豁免收窄归分配器接管帧谓词(非接管末窗
    #: 帧授权可达);血预算门(ADR-0448/0451)=discipline 层不受姿态让位
    #: 影响。授权只改「门开不开+预算上界」,候选/评分公式不动(P12 j=0
    #: 负例/升级三合法判据不动)。
    #: **v3.2 增量(W772 分支 2 后出手面修正;ADR-0483)**:通道 B 方向
    #: 梯级重排 L1 高价值定向(核心卡/2★ 凑档件/线内 3-5 费件)→ L2
    #: 定向 D(P12 不变)→ L3 低价兜底;保留金下限公式化(见下两字段)。
    #: 触发判据/豁免结构不动(M0a 98.4% 实证有效)。
    #: **v3.3 增量(W779 净支出可行性审计后 sink 供给+判据口径双修)**:
    #: ①单帧净支出闸废除→窗级水位不升出手计划(W779 审计证单帧净>0
    #: 在 P2 中后段结构性难达 rn5-7≈0-2%);②L1×L2 耦合(D 后当帧
    #: 复评 L1,刷后 re-decide 段链承载);③XP sink 授权化(hp>停升级线
    #: ∧非 P21 濒死带帧买经验至 carry 概率窗,ev.levelup_ev_basis 臂④,
    #: 血预算门 AND 不动);④L3 量级诚实化(bench 容量封顶,只辅助)。
    #: 总开关:False=现行为逐位一致(零漂移锚,A/B 对照臂)。
    #: **开臂判据挂账(事前写死,协议 V4 双钥匙缺一不开;策略开关生命
    #: 周期第 1 态=W779 M0b 再败后 sink+判据双修重挂,准入理由=D 解绑/
    #: XP sink/窗级水位目标为新机制未经合规 A/B+保留金公式参数仍属
    #: 标定层+P25 Δp 未标定)**:
    #: ① P25 证成(Δp 标定回填,g 分带——全带/贴线带逐点/破息带——各给
    #:    符号结论;单篇=docs/game/currency_war/research/proofs/
    #:    p25-p2-corecard-purchase-ev.md;采集窗口死线=自 v3.3 落码起
    #:    60 局实机或 1 次 sim 注入批先到,到期未证成=按证伪分支当场
    #:    兑换);
    #: ② sim A/B 预注册协议 V4 主判据达标且护栏全绿(协议=.debug/temp/
    #:    currency_war/w757_p2_spend_auth/SIM_AB_PREREGISTRATION_V4.md,
    #:    n=500/臂配对 McNemar;主判据 = M0a 判定触达≥70% ∧ **M0b 授权
    #:    窗水位不升(窗终金≤窗起点金)≥50%**(v3.3 换口径)∧ M1-M3;
    #:    护栏 G1-G6 沿用,G1-B① 阈值随公式下限;三分支归因规则写死:
    #:    M0a 败=覆盖失败/M0b 败=sink 解锁失败→本设计层,M0 双达而
    #:    M1-M3 败→上游)。
    #: 验证不过的出口=删码留 ADR-0480/0481(生命周期第 4 态),禁悬置默认关。
    #: 开臂翻默认时必须盘点「断言授权关闭行为」的锁组语义(第 3 态义务)。
    p2_spend_auth_enabled: bool = False
    #: T3 连败阈值 N_fail(**占位值,不做生产决策**——开关默认关;出处:
    #: 口述 [18]+combat §3「连续失败」口径;标定网格待开臂前定,判据=
    #: W757 v2 §二「未标定量显式标占位」)。消费口径=带符号 streak
    #: (cw_state.GameState.streak 单一源,负=连败)取连败幅度。
    p2_spend_auth_fail_streak_n: int = 2
    #: 加急层保留金**绝对异常地板**(v3.2 起语义:仅防公式输入异常
    #: (负值/日程表缺)的回落值,不再参与常态判定——常态下限由
    #: ``p2_spend_auth_reserve_rounds_cap`` 参与的公式化下限给出
    #: (5×min(剩余备战轮数,cap),p2_spend_auth._reserve_floor 单一
    #: 源)。取值锚= rebirth_floor 同值([18] 应急保留重生基数),非
    #: 同一判据——标定后独立演进,禁回指 rebirth_floor 造双源耦合。
    p2_spend_auth_reserve_floor: int = 20
    #: 保留金公式化封顶轮数(v3.2;W772 修订方向②:**占位值,不做生产
    #: 决策**——开关默认关)。下限=5×min(本位面剩余备战轮数,cap),
    #: P13 息流截断(每轮息 5 金、位面末守息无意义封顶 cap 轮),取值
    #: 域 [5,5×cap];出处=W757 v3.2 设计 §3.1 保留金带公式化+权衡账
    #: (T3 帧息收益 ≤5/轮 vs 败局 15.39)。G1-B① 护栏阈值随本公式
    #: 重标(协议判读脚本参数,判据红线语义不动)。
    p2_spend_auth_reserve_rounds_cap: int = 3
    #: 贴线带下界(P25 单篇 §边界点与分带核验 分带定义:花完 ∈
    #: [45,50) 为贴线带,<45 为破息带;[22]④/[11] 贴档线凑息账口径)。
    #: v3 预算带对齐后「贴线带证成前保守=不买」子句已废除(授权层与
    #: 既有息账门同判,无独立判定消费本常量);保留作 P25 分带核验的
    #: 定义值(非判据输出,零决策消费点)。
