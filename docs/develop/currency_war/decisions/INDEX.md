...
| [0630-unified-state-journal.md](0630-unified-state-journal.md) | ADR-0630: 统一 state 状态流水(BoardState 收编升级+三渠道写入口+自足快照变更账)——旧 12 流退役(理由=流散乱+写点不全+无渠道签名+无统一 state,非查询/重放问题)后的统一状态记录面:①容器=BoardState 收编升级(write_seq 升格版本 id/写入 API 全带渠道签名/GameState 收编零补域,唯一新增=派生域+上下文域四字段+receipts 域;另起容器否决双源)②三渠道写入口(obs/logic_action/logic_hook 封闭集+ChannelSig 签名+actor 登记面+域准入白名单+禁旁路 grep 锁)③自足快照变更账(state/journal.jsonl 两行型,行=改了什么+签名+版本 id+写入后完整 state 快照,行行自足零重放零锚零对账;v2 增量账+快照锚+回放自检被用户根本性纠正作废)④派生规则双腿(备战腿顶栏权威+弹窗腿守卫族,跃迁去重键=(run_id,effective_ord),守卫族含 BOSS简报 中段前驱 E12 勘误)⑤receipts 回执域(滚动窗 8,发出即簿记;出战/商店两条点击后重读路径=H1 辐射面勘误在案,v3.3 折入硬义务)⑥影子双写缺省关(旧 12 流零触碰,消费切换挂迁移批次二);实现=R1(T-229)+R1.1(T-232)+R2(T-230)三批同车一笔(T-167 共享文件先例);D3 kind_inherited 保留申报/D4 报告 26→28 锁勘误/基线 3132→3134 勘误在案 | 已实施(验收=三份落地审 accept;commit 随本 ADR 同窗) |
