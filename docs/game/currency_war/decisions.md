...
## D-69 (2026-08-11)【发现·match-end 流程地图 + A8 对局客观指标(积分9600/晋升170满/总经济103/阵容价值8)】挑战失败 → 对局评价 p1(对局未完成2-1/标准博弈A8/对手财富造物主40/积分9600/晋升170/阵容/投资环境策略)→ 下一页 → p2(小队生命值0/总经济103/阵容价值8/伤害12.4万+3.0万)→ 返回货币战争 → 大厅(晋升170已满级/积分9600-18000)。**match-end 流程地图完整**。A8 对局客观指标(败,位面2-1 团灭)

- **match-end 流程**:挑战失败(HP0 团灭,下一步)→ 对局评价 p1(积分/难度/晋升/阵容/投资)→ 下一页 → p2(总经济/阵容价值/伤害)→ 返回货币战争 → 大厅。
- **A8 对局客观指标(本期 A8 第一局)**:标准博弈 **A8** / 积分 **9600** / 晋升等级 **170(已满级)** / 总经济 **103** / 阵容价值 **8** / 伤害 12.4万+3.0万 / 对手 财富造物主40 / **位面2-1 团灭(败)**。
- **大厅状态**:晋升170 已满级 / 积分 9600/18000(本期)/ 开始「货币战争」(新局)。
...
## D-79 (2026-08-11)【里程碑·第二场完整 A8 对局(equip_all,过位面1 boss,位面2-1 团灭,伤害~14× 提)】CurrencyWarRunLoop 第二场完整 A8 对局(含 D-77 equip_all):24min(retry 0)→过**位面1 boss**(装备助力)→位面2-1 **挑战失败**(HP0 团灭)→对局评价(总经济107,伤害 ≈8567万[6912+1569+61+25],vs 上局 ≈492万 **~14× 提**)→大厅。**equip_all 明显提 bot 实力**(伤害 14× + 生存更久 24 vs 19min),但**位面2-1 wall 仍在**(两场都团灭,comp/策略待提)

- **第二场完整 A8 对局**(equip_all D-77):CurrencyWarRunLoop 24min(retry 0)→ 执行成功「对局结束,回大厅」。
- **进展(vs 第一场 D-76)**:① 位面1 boss **过**(定点爆破+装备助力);② 位面2-1 **挑战失败**(HP0 团灭,同第一场位);③ **总经济 107**(vs 103)+ **伤害 ≈8567万**(6912+1569+61+25,vs 上局 ≈492万,**~14× 提**)+ **24min**(vs 19min 生存更久)。**equip_all 明显提 bot 实力**(伤害暴涨,生存更久)。
...
## D-83 (2026-08-11)【里程碑·match#4 首过位面2-1 wall → 死位面2-2(墙在软化,HP 管理是关键)】match#4(~26min)比 match#1-2 进一步:**首次过位面2-1**(HP 12→1,保险机制+12 救)→ 死**位面2-2**(HP 0)。墙在「软化」(保险/运气可过位面2-1),但 HP 管理(进位面2 时 12,r1 后 1)+ comp 强度不足 → 位面2 仍死。HP 轨迹直证 HP 管理是关键杠杆(① 后策略)

- **match#4 轨迹**:HP 100→48(r8 挑战成功)→12(位面1 boss r9,存活)→进位面2(HP12)→**1(位面2-1,保险机制+12 救)**→**0(位面2-2 死)**。comp 巡击青雀(p1)→列车同行(p2);board 8 阵营散(仙舟3/能量2/击破1/昼之半神1/欢愉1/列车同行1/战技点1/治疗1,集中度低);equip 仅轮滑鞋(equip_all no-op,supply 无穿戴)。
- **vs match#1-2**:1-2 都死**位面2-1**;match#4 死**位面2-2**(进一步 1 回合)。**墙在软化** —— 位面2-1 可过(保险/运气),但非稳健(HP1 擦过)。bot 在变强(equip_all + 经验),但位面2 仍 HP 枯竭死。
- **关键杠杆(HP 管理,① 后策略)**:HP 全程下滑(100→12 进位面2),位面2 靠保险机制苟活 → 位面2-2 死。**根因 = HP 管理 + comp 强度**(tier-3 未成型,8 阵营散;equip 不足)。① 解锁后策略调优重点:① **HP 管理**(减少位面1 boss 损血 / 保留 HP 进位面2)② **comp 聚焦**(8 阵营散 → tier-3 成型)③ equip 供给(补给节点穿戴)。
...
## D-94 (2026-08-11)【架构·/goal 三轴重设计稿 15 与 14 重复 → 净贡献合并入 14 + 删 15(单一源,治双源漂移)】/goal「重新设计策略(灵活阵容 A + 阶段目标 B + 经济驱动 C)」产出 15_node_economy_flex_strategy.md,经核对发现 **15 与 14 ~70% 重复**:三轴 = 14 §1-§2(B 阶段)/ §3(C 经济)/ §4-§5(A 灵活);15 自己也写「15 替代 14 §2」但**新建文档而非改 14 → 两份并存 = 双源**(CLAUDE.md 明禁)。且双源已实际漂移:14 §7 副本数=18,已被 D-93 推翻为 9。

- **15 净贡献(合并入 14,非重复部分)**:① `NodeGoal` dataclass 形式化(target_level/spend_mode/action_focus/danger_d + 字段消费者);② `get_node_goal(plane, round)` 查询原语(区间键 + 单点 + fallback);③ `spend_mode` → economy 档位映射(触达 economy_score + _maybe_sell_for_interest;与 config.economy_mode 主辅分层);④ HP 区行为(>50 fold / 40-50 buffer / <40 保血,消除真空带,对齐 HP_DANGER=40);⑤ streak 接线顺序(先胜负语义核再消费,防反向);⑥ 7 杠杆落地状态表(✅/🟡);⑦ §8 实施路线;⑧ target_level gate vs _expected_level 责任分工。A 轴方向(多羁绊/optionality/level_plan/ENV/transition)14 §4-§5 已有,只补「深度改动拆后做(P0 + D-17 前置)」。
- **备选(为什么没选)**:① **保留 15 作「实施级」、14 作「设计级」** → 同三轴拆两文档 = 人为分裂,invite 漂移(已发生:副本数 18 vs 9),违背单一源。② **保留 15 删 14 §2/§3** → 14 是调研落地(D-21)有 web 来源/证据等级,15 无来源;且 14 §1/§4-§7 不重复不该删。③ **15 改成只留净贡献 + 重指针到 14** → 仍两处维护。三都不如合并。
- **修法(选)**:15 净贡献 8 点合并入 14(§2 升级为 NodeGoal 结构 + §2.2 spend_mode 表 + §3 加 HP 区/streak 接线/7 杠杆状态 + §4 加 A 轴拆后做 + §7 修副本数 18→9 + 新 §8 实施路线 + 头注 D-94);删 15(未 commit 无历史损失);14 头注说明原委。14 升级为「设计 + 实施形式化」一体单一源。
- **漂移已修**:14 §7 副本数 18→9(D-93);§1 概率表权威源 `SHOP_REFRESH_TABLE` → `cw_shop_odds.REFRESH_PROB`(D-91 实机 OCR)。
- **关联**:D-91(REFRESH_PROB 权威)/ D-93(副本数 9)/ A4 进度(理论牌池模型,本文 §8 实施路线 1-4 纯逻辑可做)/ 14(被升级的单源)/ 15(已删)。
- **方法论沉淀(避免再犯)**:重设计前**先 grep 现有文档**核对是否已覆盖(三轴重设计前应先发现 14 已覆盖 B/C/A);review 别只审新稿内部一致性,要带「是否与既有重复」问题。新稿与既有覆盖同一主题 → 合并入既有 + 删新稿(单一源),非新建并行。
- **状态**:**已合并 + 15 已删(单一源恢复)**。`· 14(单源,升级为设计+实施)/ A4 进度(实施路线对接)`。

## D-95 (2026-08-11)【策略·方案定型轮1 玩法修正(限时AV / 掉血归因 / 过渡基础设施 / commit渐进 / COMP扩充)+ review HIGH 折进设计】策略方案定型(做法一⑤工件 `4_策略设计/`)第一轮。两路输入:**review agent 揪 5 HIGH**(① COMMIT_ROUND=2 vs α(t) R_OPEN=2 早锁矛盾 / ② optionality 数据空 shared/transition 稀疏 / ③ comp_viability 死函数 maybe_pivot 不调 / ④ spend_mode 0 次出现 economy_score 无 phase 钩子 / ⑤ 8/9 comp 无 level_plan)+ **用户审查给玩法认知修正**(限时=行动值 AV 非"非限时" / 掉血归因按成型度三分 / 转型只用于凑不齐 / 前期尽快50金少D / 过渡阵容+通用角色是基础设施 / commit 渐进非死堆 / COMP 流派远不止 9)。折进设计文档:

- **03**:加「掉血归因」框架(成型中→继续组建 / 成型后→补强不转 / 凑不齐→转;COMP_LIBRARY 收录即强度可信,观测判投资方向不推翻阵容)+ 修转型信号3(成型后掉血不转,仅未成型+濒死才保命转)。
- **14**:P1 前期改"尽快 50 金、少 D 除非过不去、过渡/输出核心稳血存钱" + §4 把 transition_chars/shared_chars/通用过渡角色标为「灵活+存钱+组建支撑」基础设施(当前数据空,待调研补)。
- **12**:commit 语义改「定方向 ≠ 死堆,组建渐进 + 过渡支撑」+ 防散放宽(D-106 prefilter 组建期放过过渡/通用辅助,只拒别的成型方向)。
- **currency_war.md**:限时"非限时"误记 →「有限行动值(AV)」(米游社 content/6564,双源漂移:`data/gameplay.md:13` 记对了)。
- **COMP 扩充调研**(落 `4_策略设计/流派扩充调研.md`):9 新流派(欢愉系3 全新羁绊 / 减益黄泉应从 DoT 拆 / 龙丹战技点 / 双王群攻 / 纯追击飞霄 / 遐蝶燃血 / 火花量子)+ 通用角色清单(符玄/知更鸟/花火/爻光/缇宝/桑博/3仙舟…)。🟢 先入,🟡(N4/5/8/9)待米游社逐角色攻略直读(chrome-devtools)升核。
- **限时下输出低 = 超时扣血** →「输出能力」观测降为 diagnostic(hp_trend 已隐含主信号);扣血公式 + 结算字段🔴待实机核。

- **备选(为什么没选)**:① 直接改代码(optionality 接线 / economy_score 加 spend_mode 钩子 / comp_viability 接 maybe_pivot)—— 用户要先**定型设计**(配置从设计派生,代码是实现后续),故本轮只改设计文档 + 调研,代码改动排队(清单记)。② 保留"成型后掉血→转"—— 用户纠正(阵容收录即强度可信,成型后掉血 = 装备/星级不够 → 补强,非转)。两都没选。
- **关联**:D-94(14 合并单一源)/ review round1(5 HIGH)/ `4_策略设计/策略需求清单.md` §6 + `流派扩充调研.md`。
- **状态**:**方案定型轮1 完成**(设计文档自洽 + 玩法认知修正);**轮2 review 待跑**(确认折进后无新 HIGH)。`· 14/03/12(改)/ currency_war.md(限时修)/ 清单§6 + 流派扩充调研(4_策略设计)/ HIGH-3/4 + COMP扩充/level_plan(HIGH-2/5)待实现排队`。
