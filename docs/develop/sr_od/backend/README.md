...
> `docs/develop/sr_od/backend/` 的入口。星穹铁道一条龙的运行层（`SrContext`）之上的**传输无关**后端，把游戏感知 / 操作能力经 MCP 与 HTTP 对外暴露。本文为「总」，各「分」文档见下方索引。

## 是什么

一个 **headless 服务进程**：自己持有 `SrContext`，通过 `SrBackendContext` 收敛成传输无关方法，再由 MCP、HTTP 两个适配器并行对外暴露。GUI 是另一个独立入口，不经过本层。

三层：

| 层 | 内容 |
|---|---|
| Layer 0 | `SrContext`（截图 / OCR / YOLO / 控制器 / 执行引擎），不改动 |
...
flowchart LR
    ...
    Entry --> Backend["SrBackendContext"]
    Backend --> SrContext["SrContext"]
...
```shell
uv run --env-file .env python -m sr_od.backend.entry.server --port 24001
```

如果项目根目录没有 `.env`，可省略 `--env-file .env`：

```shell
uv run python -m sr_od.backend.entry.server --port 24001
```

GUI 启动：打开「开发工具 -> MCP 服务」，使用同一条入口命令在本机启动 `sr_od.backend.entry.server` 子进程。

启动后：

- MCP：`http://127.0.0.1:24001/mcp`
- HTTP 探测：`http://127.0.0.1:24001/health`
