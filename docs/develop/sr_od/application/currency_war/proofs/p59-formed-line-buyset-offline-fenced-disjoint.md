...
> 对象:换阵卖出义务臂——`cw_op_deploy.offtarget_sell_allowed`
...
> 依据:(含 2026-09-05 原裁定档修订「P18 型结构命题」立项挂账)、
> 原裁定档修订节(已退役)、`flow.py:224-264`(target_comp = get_comp(locked_comp)
...
(W209 振荡事故即「引擎件躺板」形态),线后续锁向黄泉减益、
fp 达 1.00、板满——与 第七局死锁帧同构(旧线件在板 + 新线
...
- 「fp 与 locked_comp 不一致帧」不是本案根因:`flow.py:245/264` 中
  ...
  两面同一 comp——**预设的修法「触发门改 locked_comp 单一源」
...
3. **预设修法作废登记**:「触发门改 locked_comp 单一源」
