...
- **`docs/game/<玩法>/research/`**:我们提炼或核实过的知识(社区帖统计提炼、机制核实、用户口述、确认过的打法知识),**活文档**,随游戏版本与证据原地更新。
- **代码注册表**(如 `src/sr_od/application/currency_war/cw_*.py`):游戏**数据**(角色/装备/羁绊/概率表的值)的唯一源;注册表全量建模后对应 data 文档删除(双源即漂移)。
- **gameplay 总览 doc(`docs/game/gameplay/<玩法>.md`)只写结构事实**:玩法怎么运转/分几个阶段/与画面的关系——打法纪律(该怎么打的判断)**不复述进总览**,单一源在 research,总览只放指针;否则口径更新时总览成为被遗忘的旧口径传播源(实证:CW 停手口径修正时总览差点漏改)。
...
- **证据分级(两层权威序,ADR-0482)**:每条结论标来源与等级——**机制事实层**(游戏怎么运转,只能观测确立):`实机实测/图鉴实锤 > [口述]`(证词;未实测标「待实测」挂验证账)`> [米游社] > [社区]`;**策略命题层**(什么打法更优,可证明/可实证):`数学证明 + sim/实机分布实证 > [口述]`(直觉假设来源,立命题待证)`> [米游社] > [社区]`。存疑/未找到标 🔴。旧单层序(口述最高)已废;历史文档 `[口述·权威]` 标签=来源出处标记,解释权归本序,不批量改写。
