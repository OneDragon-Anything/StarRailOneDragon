...
## 5. 效果感知(ADR-0202 v2)与缓存

- `solve(ledger)`:持有效果改变世界规则时(息 cap/单击价/连胜乘子/节点日程)按
  台账重解;`None` = 基线(零漂移锚,测试锁行为);
- `EffectLedger` 的字段语义见 `cw_effect_ledger.py`(四象限路由);**纯时点金
  (instant_gold 类,overlay 73 条中 52 条)不进台账 → 指纹不变 → 不重解**;
- 缓存(v6/v7):**解级内存缓存**(`solve_cached` 按台账指纹 memo,同指纹二次调用
  0ms;冷解向量化 ~0.3s,盘缓存层已移除);查询路径 `posture()` ~2.1µs(slots+动作码
  预表;曾试查询级 memo,冷路径 3× 恶化已撤——重复查询场景少,得不偿失)。
...
| 效果解切流 | 47 号发布层灰度 | 生产从基线解 → 定制解(v6 后每指纹冷解 ~0.3s,成本已不构成切流障碍) |
| 内环 numpy 向量化 | 若冷解需 <5s | 当前 13.7s + 缓存已够用,性价比低,搁置 |

## 7. 维护红线

- **零漂移锚不可破**:无 ledger 解必须与历史基线逐 posture 一致(v6 向量化对拍锚:
  ACT 全等 + VAL max|diff|=0,集成测试锁定),它是 0181/0190 两份 A/B 报告的可比性根基;
- 值/动作表是**紧凑数组不是 dict**:`policy`/`value` property 是惰性物化(旧测试
  兼容),生产代码**只用 `posture()`/`value_at()`**;
- 求解为 numpy 向量化实现(布局 [t,Li,gi,hi,rbi] flat):改常量后直接重跑即可
  (0.3s),无任何缓存文件需要管理。
