...
class BuyShopCards(SrOperation):
    ...
    def buy(self) -> OperationRoundResult:
        ...
        for _scr, _area, _evt in (
            ...
            ('货币战争-遭遇节点', '标识-遭遇节点', '遭遇'),   # T#103:原全屏 OCR「遭遇其一」(lcs=0.9 卡标题截断 miss 前科,cw_loop 0c 同源 anchor)
            ('货币战争-列车同行', '标识-选择伙伴', '选择伙伴'),   # T#103:原全屏 OCR「选择伙伴/确认选择」
            ('货币战争-盛会之星', '标识-盛会之星', '盛会之星'),   # T#103:「确认选择」事件里唯一有画面档的巨星 overlay 改走标题锚(cw_loop 0b 同源)
        ):
        ...
        return self.round_success(finalize_buy_phase(
            self, match, outcome, hp_value, _hp_readable, _hp_trusted))
