...
### 商店刷新概率表 p(level, cost) ★ `level_plan` 硬输入(🟢 bwiki 2026-08-09)

> economy_research 原只有「7级3费=0.4」一个点;本表补全 Lv1-10。落代码 `SHOP_REFRESH_TABLE`(02 A4)。
...
> ⚠️ 本表 = **设计参考快照**(bwiki 🟢,与米游社 `77124902` 锚点「Lv7 3费=40%」交叉一致,反而增强可信);**权威值以代码 `SHOP_REFRESH_TABLE`(02 A4)为准**,版本更新只改代码 + 同步本表(双源防漂移)。

## §2 节点×等级×动作骨架(阵容无关)

| 阶段(位面-节点) | 等级目标 | 动作(骨架) | 经济 |
|---|---|---|---|
| P1 早期(1-1~1-3) | 冲 Lv4 | 纯升级;买低费过渡保血(具体过渡牌见 03 `transition_chars`,阵容参数非骨架);不主动 D | 利息不重要(钱少),有余就升 |
| P1 中期(1-4~1-6) | Lv4→5-6 | 买 2-3费过渡;攒金到 50;A8+ 战力不足+遭遇前 → 弃息 D 保血 | **存 50 吃满 5息**是引擎;A8 遭遇前保血 > 保经济 |
| P1 后期/首领(1-7~1-9) | Lv6 | 锁血过 P1 首领;观察发牌定 target | 维持 50,超额买同费稀释牌池 |
| P2(2-1~2-7) | Lv7→**Lv8** | **2-7 左右升 8 搜核心**;放宽 D 上限 4-6;commit target | 攒 50→升 8→用息 D;连胜保连胜 > 吃息 |
| P2 后期/首领 | Lv8 | 成型;补质量(追 2星核心) | 血健康吃息;血危花光提质量 |
| P3(3-1~3-5) | Lv9→10 | 上 9-10 找 5费 + 关键卡追 3星;成型锁血;高难遭遇对策装备 | 血健康吃息上 9;血危 all-in |
| P3 boss | Lv10 | 全员成型;装备齐 | 花光 |
...
- **财富宝钻(团队规模+1)**(🟢 D-19):后排 6 基准 +1 可变(无论是否穿戴);deploy 运行时实测槽位,不硬编码 6。
...
- 🔴 **升级费用逐级表**:无 web 源,须游戏内「数据银行」图鉴提取(代码估算 6→10 累计≈262 ≈ 攻略270,大致对);文档标待图鉴,不写死(02 `LEVEL_UP_COST_TABLE`)。
- 🟡 **V4.5 meta**:未找到专项,暂沿用 V4.4(本次调研);版本更新重核。
- 🟡 **副本数**:V4.4 3费实测 **18**(economy_research);1/2/4/5费待实机核。**02 A4 `POOL_SIZE_PER_CARD=9` 已被 a=18 推翻,待实机核后同步更新 02**(跨文档常量过时)。
...
- 🟢 [NGA tid=45570754](https://ngabbs.com/read.php?tid=45570754)(奖励关白嫖 / 买同费稀释牌池)
