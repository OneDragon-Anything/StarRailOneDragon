...
- **Date**: 2026-09-09
- **关联**: ADR-0601(动作 op 机械执行——本批修订其 §5「执行位现读不可信」论断辖域并在 §3 D2 增补豁免分支)、ADR-0554(环级无进展守卫——本局停机即其设计行为,T-167 F2 链实战首验)、dd-037(部署发射×执行契约——新增出口状态常量纳入可区分集)、ADR-0139(comp char_positions 覆盖——登记项 1 的口径双源出处)
...
2. **出口断言 CV 幻影残留**:deployed≥1 腿用 CV 占用现读,DD-030 幻影占用族可使「真板空」帧误判有人 → 假 round_fail → 环重试 → 守卫兜底,有界。可选缓解 = deployed 腿同走双源仲裁,挂账候裁。
