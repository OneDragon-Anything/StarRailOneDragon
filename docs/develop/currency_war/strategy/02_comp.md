...
> 「这局打什么阵容、何时定型/转型、跨局怎么分配」的语义。本篇:`cw_comps`(COMP_LIBRARY + select_comp/pivot)/ `cw_transition`(双轨过渡)/ `cw_line_tribunal`(审判层)/ `cw_run_allocator`(跨局分配)。玩法证据 → [game/research/plaza_methodology](../../../game/currency_war/research/plaza_methodology.md)(M1-M16);单套打法叙事(入场/退场/counter)→ [game/research/final_comps](../../../game/currency_war/research/final_comps/README.md)(终局阵容十类深读,单套 comp 知识单一源)。
