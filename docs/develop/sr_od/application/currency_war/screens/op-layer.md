...
- **观察标准化门(用户裁定 2026-09-22,全域行为规范)**:观察内容在所属域存在标准注册数据(名字类:投资环境/投资策略/角色/装备等注册表)时,观察 node 必须在观察时把读数转换成标准注册数据再上报——转换两段:①形变归一(分隔符/全半角等 OCR 形变族)后精确匹配;②不中再 LCS 相似匹配(`one_dragon.utils.str_utils::find_best_match_by_lcs`,阈值常量住代码);两段皆不中 = 转换失败。**任一候选转换失败 = 观察失败**:观察 node round_fail 早退、零写零上报,交外循环重观察重读(读不准重读,禁带病上报)。标准化后下游全链只在标准名上工作:决策消费标准名打分;动作上报只携带选择序号(idx),名字由容器标准名单一源按序号提供,动作/上报层零名字转换。收敛在途:投资环境屏 = 2026-09-22-screen-review 迭代首个落地;其余名字类观察屏为欠账,逐批收敛,禁新增未标准化直报。
