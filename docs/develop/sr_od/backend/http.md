...
> 把 `SrBackendContext`（见 [architecture.md](architecture.md)）以 **HTTP** 暴露。它是 **web 前端 + skill 的共同目标**——任何能发 HTTP 的客户端（含 AI 经 Bash / curl）都能用；MCP 客户端另走 [mcp.md](mcp.md)。两个适配器并行、共享同一 backend。
...
| 调用方式 | REST | tool-call |
| 共享对象 | 同一个 `SrBackendContext` | 同一个 `SrBackendContext` |
