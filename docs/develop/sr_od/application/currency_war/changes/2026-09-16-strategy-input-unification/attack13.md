...
- **§2.2/§2.3 机制面**：12 槽位表与代码一一对应（typed 选项类五者同住 cw_events.py:597/:711/:778/:806/:827，cw_events 模块级 import cw_game_state :24——契约 g 循环导入依据成立）；REGISTERED_ACTORS（:285-336）缺席新写端者恰 `CwScreenYinLang`/`CwScreenBoxPick` 两名——登记申报精确；`_PAYLOAD_DOMAINS` 三域（:171）与 leave_screen 域守卫（:3158）；属屏映射 10 行键与 `_dispatch_identity_screen` 分发臂（cw_loop.py:1364-1642）逐行对上（含列车同行/骇入策划/星徽秘典弹窗/备战-武装箱选择四反直觉行；武装箱弹窗 ：1431 派发 CwScreenArmoryBox 无 decide 核实）；挂点时序（早退 ：1754 < 识别 < 分发 ：1821，未知兜底 ：2032/:2083 在分发后循环尾）；「非身份臂语境无 decide 消费」逐臂核（阶段三特殊规则臂道具详情/消耗品/阿哈装备均无契约调用）；已 None 跳过先例（carry）；ChannelSig.group_id 随 write_seq+1 位移先例（flow.py:766-768）。
