...
def select_deployments(
    ...
    底线门/板空保底(系);唯一省略:SIFT 未识别(char_id 空)
    照旧上——调用方传空 char_id 即走该分支。
    cap 语义 = 已 deployed 数 + 本轮上场数 ≤ cap。

    (deploy 围栏锁定键放行):``locked_factions`` = 锁定帧
    体系键并入围栏放行集。

    配方底线门锁定线语境豁免:``recipe_floor_lock_exempt`` =
