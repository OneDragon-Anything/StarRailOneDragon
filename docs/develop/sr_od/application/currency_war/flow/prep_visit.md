...
run()
 ...
 │    → _observe(heavy=True)（期望态重建的唯一读屏点 = 对账,ADR-0517 决策 8）
 ...
 └─ ③④⑤单动作决策循环 for _vi in range(VISIT_ACTION_CAP=16)（ADR-0517;循环内零读屏）：
      ...
      → 投影 _project_prep_obs（纯计算零读屏）:已建模面（OpenBox/OpenTome 腾席/
        ...
        RunEquip）→ None = 保守回退:本访问终结交回外循环重观察（重观察语境禁猜;验证阶梯 = ADR-0518 遗留节）
