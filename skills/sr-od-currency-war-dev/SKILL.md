...
**sim 已知边界**:Δ池按深度分桶,±2 深度量级测不出;「散面 vs 集中」维度 sim 战斗层不敏感、可能与实机方向相反——此类维度以实机判读为准,并把偏差记录在案;「sim 上实机前必核代理语义」(曾因 bench 计数≠上阵的代理错位得出 1.8 倍增益伪影)。**真实性演进方向(2026-08-23 用户定调)**:校准层逐步换真机制——胜负用真实 outcomes 按成型指标分桶拟合(胜率=f(引擎数));连胜金/利息已接实测真值(cw_economy 单一源)。**「识别噪声注入」不做**:识别漏检/空名槽/误读是实机代码 bug——**噪声来自我们的代码→修,随机来自游戏→建**(给 bug 建噪声模型=给 bug 续命);sim 假设完美观测是正确终态。**sim 压力测试官(同日定调,常设角色)**:派子 agent **对着玩法本身发散构造场景**——不只复现实机问题,自行想象极端/边界/组合态(经济极端/牌库偏斜/连胜连败/节点序罕见组合/等级与息律冲突/boss 高压/装备路线分叉等),主动找策略问题,不局限不守旧;发现→修复→固化成 sim 检查项/单帧锁(检查网滚动生长);测改分离(测试官只找问题+设计检查项,修复归 worker);实机 40min/局 vs sim 秒级,sim 深度挖掘是长期高杠杆。**排程纪律(防执行滑坡)**:压测官是常设在飞——一批交付→消费+派修复→**立即派下一批(场景域轮换:经济/成型/站位/连胜态/装备路线/…)**,队列里恒有一个压测官在挖;指挥官心跳轮检查「压测官在飞否」,不在=排程失误。
