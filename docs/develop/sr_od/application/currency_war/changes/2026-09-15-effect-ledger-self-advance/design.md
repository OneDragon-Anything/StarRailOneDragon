...
- 系统级变化：
  1. **kernel 新增效果推进段**：`observe_screen_context` 管线尾段（类型派生之后、方法返回前，同临界区）。段逻辑：
     ...
     d. **节点边界金结算（推进与结算分离，双水位）**：
       ...
       - **触发条件（三 conjunction，完整）**：`effective` 非 None ∧ `effective > (boundary_settled_ord or 0)` ∧ **本帧为备战帧**。0q/0p/弹窗族推进帧 = 过渡标记，**不触发结算**（当帧节点镜像未刷新，取错窗参数——攻击 F1 实证），递延至该节点的备战帧；补给节点**无备战画面也不需要**——其轮首收入由游戏于离场/下一节点备战帧发放（用户裁定 2026-09-15），恰被下一备战帧观察直接捕获，`round_start_income` 的 supply 分支保留供 sim/规划投影，非 live 结算载体。
...
- 行为变化申报：
  ...
  - **补给节点入口收入**：维持观察覆盖载体（用户裁定：轮首收入于节点离场/下一节点备战帧发放并被直接观察，logic 记账无必要；`round_start_income` supply 分支保留供 sim/规划投影）；
