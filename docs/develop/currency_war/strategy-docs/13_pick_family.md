...
| `decide_encounter(options)` | E3（遭遇难度/词缀避开，可刷 1 次） | **双轨**（ADR-0536）：基线核 = 委托 `cw_events.decide_encounter`（未成型→低难保生存 / 成型+词缀利→高难拿奖励 / 全克→刷新换批）；mandate_v1 核 = E3 EV 判据 `mandate_v1/encounter.py`（EV(b)=V_r−Δλ_death·G_loss，奖励子型分立 + λ label 四态接死 fail 向选低难 + 刷新肢条件化非免费期权；现态双 provisional 槽 None 期恒 fail 向低难、零刷新建议，数值开闸链与验收口径见 ADR-0536 §3/§4） | **有规格**（基线=知识判据，M13/competitors 节点表；mandate_v1=EV 数学判据，语义单一源 ADR-0536）；08 E3 锚「收益侧挂 P26 同构」已由 mandate_v1 轨承载 |
