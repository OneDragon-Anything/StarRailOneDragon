...
evaluate = `_phase_weights(plane, hp)` 加权的(羁绊+经济+角色质量)。**2026-08-03 修正(review agent + 用户)**:前期 economy **不该压低**(利息越早到 50 越好、滚雪球)。原"plane1 压低 economy"把"前期"和"保血"混淆。现:**HP 危险(hp<HP_DANGER)→保血**(战力/角色加权、经济降权,任何位面);**plane3→锁血**(全力战力/星级);**其余健康→平衡**(economy 不压、snowball 到 50)。阶段键控权重的**具体值见 `_phase_weights` 代码(单一源,不在文档写,避免两边漂;阶段 6 实机校准)**。economy_score 利息项由 level_plan 硬 gate 主导(D-14;`economy_mode` **保留**作 eval 权重微调[D-18],与 level_plan 共存、不冲突);synergy 有高 ceiling 潜力项;char_quality 计 bench+deployed。**待校准**:HP_DANGER(A8 高难调高,待 difficulty 字段)+ win_streak(连胜保连胜>吃息,待 read_streak)+ phase 权重值实机(阶段 6)。
**hp 阈值统一走 config(r5,D-22 DONE)**:`config.hp_safe_threshold`(默认 40)+ 偏移系数(转型 0.75×、死局 0.5×、连败 fold 1.5×)。`_phase_weights(plane,hp,hp_threshold=HP_DANGER)`、`_refresh_cap(state,hp_threshold)`、`maybe_pivot`(`0.75×threshold`)签名带参数(**默认=HP_DANGER 保行为不变**);evaluate/plan 经 config 传入。死局 0.5× / 连败 1.5× 待 is_run_dead / losing_streak 实现时补。
