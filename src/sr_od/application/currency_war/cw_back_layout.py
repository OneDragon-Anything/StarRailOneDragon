...
#: 探针位 x(8 格档全列;6/7/8 三档各自居中不共享列位,探针取 8 格两端 464/
#: 1458 + 共享中段锚 606/1031——6 格 604..1316 含锚、7 格 534..1386 的锚位
#: 覆盖率足够、8 格全含;判别语义见 :data:`_CV_LEFT_STD_MIN` 注)。
#: 1080p 原生坐标(同 check_system_unit_layout 假设;非 1080p 帧
#: :func:`cv_back_slots` 越界守卫返 None)。
_CV_CAND_XS: tuple[int, ...] = (464, 606, 748, 889, 1031, 1174, 1316, 1458)
#: 两端探针位(6 格态此二位 = 排外背景;7 格 = 真端格半覆盖;8 格 = 真端格)
...
#: **左端(464)三值判据**(W209k 标定 + 2026-08-26 居中重排勘误):464 探针
#: 在 8 格局盖满真左1 格(std 62.5-148;空槽暗框 38.8 P3 局),在 7 格局只盖
#: 真 s1(中心 534)左半片(std 26.2-43.6,随立绘大小浮动)——**两态在
#: [26,48] 带重叠,单值无分辨力**。三值:≥48 → 有左格(狸猫局族 62.5-65.6);
#: ≤12 → 无左格(6 格背景实测 ≤2.9,464 在 6 格=排外背景);中间 → **CV
#: 不可判**(:func:`cv_back_slots` 返 None 退公式——7/8 靠 diff 公式(cap
#: 直读权威 + 等级漏读经经验条反推),比硬猜诚实;ADR-0385 决策 13 残余
#: 风险的量证收口)。旧「羁绊面板渗入」解释作废(那是 7 格真 s1 槽的
#: 左半覆盖信号,非面板渗入)。
...
def cv_back_slots(screen) -> int | None:
    ...
    方法:对两端探针位(464/1458)逐位判「槽签名存在」——右端用
    :data:`_CV_SLOT_STD_MIN`(8 格真右格/7 格真 s7 右半片均高 std,只判
    「≥6 有东西」);**左端三值**:≥48 有左格 / ≤12 无左格 / **中间带不可判
    → 整体返 None 退公式**(「8 格左1」(62.5-148,空槽 38.8)与「7 格真 s1
    左半覆盖」(26.2-43.6)std 重叠带 [26,48] 无分辨力——7/8 靠 diff 公式
    兜底,cap 直读权威 + 等级经验条反推)。
    格数 = 6 + 探针存在数(6/7/8;三档各自整排居中:6 格 604..1316 /
    7 格 534..1386 / 8 格 464..1458,ADR-0390 居中勘误)。
