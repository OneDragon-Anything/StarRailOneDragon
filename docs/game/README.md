...
- **证据分级(两层权威序)**:每条结论标来源与等级——**机制事实层**(游戏怎么运转,只能观测确立):`实机实测/图鉴实锤 > [口述]`(证词;未实测标「待实测」挂验证账)`> [米游社] > [社区]`;**策略命题层**(什么打法更优,可证明/可实证):`数学证明 + sim/实机分布实证 > [口述]`(直觉假设来源,立命题待证)`> [米游社] > [社区]`。存疑/未找到标 🔴。旧单层序(口述最高)已废;历史文档 `[口述·权威]` 标签=来源出处标记,解释权归本序,不批量改写。
