...
4. **开拓者形态归一挂容器写口**:开拓者按目标排换形态(char_id 随排切换,board 计数不再错账)= 上报函数内 `cw_exec_state.trailblazer_row_unit`(归一核单一源 = `trailblazer_row_identity`,与 swap 上报同源——benchchar-retirement P3 统一申报的缺陷修复:换排 = 命途切换);tracked 侧同核(`cw_vocab.py::mutate_bench_deployed` DeployMove 分支);
