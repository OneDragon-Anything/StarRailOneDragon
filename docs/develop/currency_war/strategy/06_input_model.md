...
| 经济 | `gold` / `level` / `level_up_cost` / `xp_progress` / `streak` / `shop_refresh_cost`(= 刷新实付金,恒基价常量 `REFRESH_COST_BASE=2`,ADR-0456——旧「刷新金币数」rect 是面板徽标=利息数值非刷价,OCR 已退主链)/ `refresh_probs`(商店开态概率条真值 {费用档:概率},投资环境轮岗翻倍的直接观测;None=未读/商店关→`_sample_cost` 退基线表) |
...
| 保真位 | `hp_readable` / `gold_readable` / `board_readable`(「值是否真读到」标记;int/dict 契约下动画帧 miss 与真值不可区分;遥测/对拍消费,决策默认不用。hp 失读时值=沿用真值非兜底 100,ADR-0282)+ **`hp_trusted`**(hp 值可信位,ADR-0282 语义细分、ADR-0428:True=真读帧或沿用 `last_hp_real` 真值的帧,False=仅「开局全无真值兜底 100」的假值帧;写入端唯一 = `read_game_state`,与 `hp_readable` 分工 = 决策谓词按「readable ∨ trusted」放行沿用真值帧、兜底假值帧仍拒) |
