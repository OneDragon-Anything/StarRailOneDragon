...
**P1 末窗承接门(ADR-0400;设计件 08 §4.2 Phase 1 落地)**:判据单一源 `handoff.handoff_gate_gap`(P1 ∧ r≥`handoff_gate_min_round` ∧ 开 → 投影承接档位(handoff_snapshot 决策入口现算)距 `handoff_gate_tier_target` 缺口数)双挂载:①formed_stop 承接维(下段成型停手门);②**EV 承接缺口项**(arbiter.interest_rule 买侧破息账的 V 加 `handoff_ev_gap_bonus`×缺口——末窗破息投资授权放宽,只辖买侧,D 平面 R 口径与升级平台账不动,auth `handoff_gap` trace)。**hp 维 boss 投影(ADR-0403;设计件 09 §3.1;W240/ADR-0404 键改净星深)**:flag `handoff_boss_project`(与门开关正交,默认关)开时末窗投影的 hp 维换「boss 后投影 hp」=hp+`handoff_boss_reward_bonus`(r8 奖励胜,唯一正项;r9 不加)−E[boss 伤害|净星深档](`handoff_boss_e_damage` 常数表,Δ池 boss 桶净星深桶键——上场件 Σ(star−1),`deployed_star_depth` 单一源,ADR-0399 star_depth 同源口径;删失剔除标定;缺桶走 `handoff_boss_e_damage_default`)——修「boss 前 hp 喂 boss 后切点」标定口径错位(板面 tier1 ∧ hp 临界局盲区);快照本身不动(P2 进场真值口径);投影 hp 披露 `session.v3_handoff_hp_proj`/sim 账本 `handoff_hp_proj`。**末窗星级定向授权(ADR-0405;W232 挂账 C 项,设计件 08 §4.2 Phase 1b)**:flag `handoff_star_directed`(与门/投影三 flag 正交,默认关,单独开=零行为)开且末窗 gap≥1 时对**同名副本买入**定向授权,授权点两半:candidates 层放行副本候选生成(r410 守卫+方向门的 gap 条件化豁免——放行≠授权:copies_cap/r408 同轮守卫/bench 容量照常辖)+ arbiter 非正分门放行 'copy' 标签候选进约束链(副本评分零维是「到不了 EV 账」的通道缺口,不是价值裁决);**授权值零新增**(EV 放行值由 `handoff_ev_gap_bonus`×gap 单一源独担,防双计);gap 判据经 `handoff.star_directed_gap` 复用 `handoff_gate_gap`(不建第二套缺口公式)。sim 账本轮行 `handoff_gap`;A/B=`simulate_handoff_ab`(三臂 off/gate/proj+proj_only 正交臂,同池同 seed 配对+P1 非末窗零漂移门;C 项四臂联合 A/B=off/gate/gate+proj/star 全开,见 ADR-0405)。
