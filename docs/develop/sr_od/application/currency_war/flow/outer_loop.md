...
> **处理列自 此后全部为画面 op 调用**——外循环只管识别分派，推进处理（含弹窗关闭）一律在 op 内；每次分发经统一包装 `_dispatch_screen_op` 落主日志 `[cw-op]` 行（op 调用流的日志载体，`telemetry/op_journal.py` 已随 2026-09-15 用户裁定退役）与决策帧留证。分发判定/排他/序位/守卫域归属不变（§2.1 判定原语、guards.md；守卫钩子经 on_result 调用点邻接闭包留在分支体）。标「新·推进」= 空决策形态 op（`../screens/op-layer.md` §1.5），标「经包装」= 既有 op 本次接包装。
...
4. "返回投资策略选择"按钮在 → 点去选策略（上游策略屏处理失败 symptom，计数报警）；
...
8. `CwScreenPrep(self.ctx).execute()`——**备战单轮**（[../screens/prep.md](../screens/prep.md)；环入口清场(揭示卡/书册卡/残留模态)收编于该 op 观察段;达标臂发射失败连续 3 次放弃短路回落本链，防线 C1）；
9. 失败 streak ≥5 → round_fail 交兜底链；成功 → `_battle_ts` 置位 + `_battle_wait_active=True`；
10. **环让位重入契约**：director 返回（含 overlay bail）后必经 return → 下轮 loop 顶全分支重判，不在同一迭代内直接回备战分支（`cw_loop.py::CwLoop.loop` 备战分支尾环让位段）。
