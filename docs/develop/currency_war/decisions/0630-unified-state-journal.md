...
- **GameState 改名计划(逐字锚,用户 2026-09-10)**:「迁移完之后将 BoardState 改名成 GameState」——game_state 设计目录(docs/develop/currency_war/game_state/)已统一用 GameState 命名;代码改名(kernel/cw_board_state.py `BoardState` → `GameState`)随旧流退役迁移批执行,改名前文档用 GameState、代码用 BoardState,两者指同一容器。
...
1. **守卫族终版**(决策 4 原文「守卫族 `SCREEN_CONTEXT_GUARD_PREV` = 结算窗 ∪ 开局链 ∪ BOSS简报」中「∪ BOSS简报」与开局链内「位面过渡」成员**作废**):守卫族终版 = {结算窗 token(战斗等待),简报,投资环境,等待1-1} 恰 4 员。0p(BOSS简报)与 0q(位面过渡)**出族**——两者各有确定性专用腿(规则③/规则②)负责自身推进;守卫族若残留其成员,专用腿推进后弹窗腿再 +1 构成级联双推进,弹窗腿缓存守卫只是掩码不是结构防线(攻击 R5 高-1,级联双推进破口;用户终裁 2026-09-11)。R1.1/E12 把 0p 列入本族的中段前驱勘误随规则③落码退役;boss 流真序结论不变(screen_flow_timing.md #26/#14/#27)。
...
## 修订(2026-09-11,局终域落码+obs_event 词表封闭集+G10 倒退留证落码;commit 门条件 = reviews/W2-局终域-落地审.md ④,返工批随车)

W2 局终域批(旧 runs 流的收编载体落码,遥测正本 §3.6.1 runs 行)与其落地审返工件的域键登记,与上文冲突处以本节为准:

1. **局终域域键 `match_final`**(bs_schema 域版本 1):一段一行(段级事实,恢复局跨段多行,game 级聚合取段序末行);唯一写口 `write_match_final`(渠道③,actor=MatchClose,写前查重 = 段内幂等 G12 收编);载荷 = 终局类型封闭集(win/loss/stopped/abnormal)+ 时点版本 id(at_version 与行头 v 恒等)+ **code_commit/registry_fingerprint 版本戳**(沿用 version_stamp 口径,模块级常量落账填充;runs 退役后 = 策略版本戳唯一在档载体,落地审 F1 补位定谳)+ 终局快照七读数 + 段级时长 + backfilled 位(abnormal 补写行 note=recovered 显影,G8)。**写口辖域勘误(落地审 F2)**:该口只辖段内补写与在线收口;启动扫描的**历史段**补写走专用装配通道(读流→构造→按显式 run_id 落行,遥测正本 §3.2.2 规则 6②,不经运行时写入口——run_id 取供给槽现读,历史段行挂现读段键 = 错归属),挂接线批批首(先例 = recover_dangling_run_summaries)。**监听触发语义勘误(落地审 F3)**:局终行监听槽 = 写口受理成功(过封闭集+段内幂等门)即触发;行落盘另以 sink 武装与 run_id 在场为准(未武装/局外 = 行不落而事件照发,事件 run_id='' 形态),消费方按此自滤。
2. **obs_event 事件词表封闭集落码**(决策 2「产生面 = 登记清单非全量」的机器面):`OBS_EVENT_EVENTS` = arbitrate/miss/popup,集外显式炸错;新登记点必须显式扩词表(封闭集即登记纪律)。
3. **G10 倒退留证落码**(决策 4 原文「倒退读数静默跳过(R1 基线;v3.2-G10 obs_event 留证增强候 M2)」的增强面落地):候选 < hist 时字段不动 + obs_event 占版本留证(event=arbitrate,actor 保留触发规则归因)。
