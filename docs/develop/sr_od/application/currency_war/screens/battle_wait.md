...
- 序位:0 系 overlay / 备战 / 0p / 0q 等先行分支之后、3c 大厅收口之前(序位表 = [../flow/outer_loop.md](../flow/outer_loop.md) §2.2「战斗窗」行)。外循环只回答「在不在战斗窗口」,单元内部分类(等待/结算页/败局链/过场屏)归本 op 分支链,不进外循环。
...
- **出战宽限(ADR-0250 口径)**:`BATTLE_WATCH_GRACE_S` 内未见结算屏 = 战斗进行中合法静止,不进未知帧计数;关窗 = 见结算屏(`saw_settlement`)。停滞 watchdog 的战斗窗口宽限同值协作 = [../flow/guards.md](../flow/guards.md) §2;`battle_ts` 由外循环注入。
