...
阶段三 特殊规则（无独立画面档/身份不可表达;互相锚互斥,顺序不承重）：
        ...
        （原 BOSS简报/位面过渡误读兜底已退役,见 §2.3 退役记录）
...
> **0p/0q 退役记录（2026-09-16 用户裁定）**：**未建档保存的故障形态不作兜底理由**——无法追溯的证据不算数，不为它预建兜底。两兜底同批退役，身份 miss 的真实帧走未知兜底停机留证（截图 + flag 自动归档），证据入库后再议加固。此前 0p 的原始事故（boss 标题持续误读）已有建档帧（`screens/货币战争-BOSS简报/boss_banner.webp`），其类已随锚换装徽记模板在身份层关账；0q 所辖「位面节点锚持续 miss」类**无建档实证**，不足为存留理由。两屏排他仍保留在阶段一位面过渡身份臂（boss 判别片段，判别单一源第四消费点）。
