...
- 实施进度:**P1-P5 全部完成 + 命名迁移全量落地(2026-09-03,流程层重构闭环)**。P1 商店链原子化(CwOpOpenShop/CloseShop/BuyCards);P2 决策接口+黑板(decide_prep_screen/decide_shop_screen,写者白名单 02-state §4.1,dd-014);P3 开局序列+overlay 族+编排切换(dd-017)——开局编排 0a0b/0r/0s 分支接管(OpeningSequence 壳已随退役删除批移除)、七 overlay 纯分发(委托壳溶解,主循环直派真身)、ctx 信箱退役;P4 战斗等待 op+期望态 infra+结算链(dd-019;自动战斗检测已落地 31de98ec);P5 收尾(ctx 信箱字段物理删除 dd6fd78d;**gate 模块已清尾删除 dd-024**——活件迁驻 cw_observation/cw_screen_prep,死写删除);命名迁移(cw_screen 21/cw_op 7/cw_entry/cw_loop,操作层终态五包)+退役删除(BattlePrepCycle/BuyShopCards/OpeningSequence/RunNode 族/decide_prep shim)。遗留挂账见 NAMING.md 与进度树(自动战斗检测已落地;暗态帧补采/位面页序/信任窗降频/身份链等实机数据类项等采集)。
...
现状病灶(亲读实证):
- **局状态不正式**:事实载体 StrategySession,但简报数据绕「ctx 信箱」(入口存 ctx → cw_loop 取走拷贝),无更新者白名单;
...
| CwScreenBriefing 简报观察收敛 | 01-opening | 三个写入点收敛为单 op(观察→写 session→点下一步) |
| 开局序列独立于 loop | 01-opening | CwScreenBriefing→位面过渡→投资环境→CwScreenWaitOneOne(~10s)→1-1 |
...
| 整局退出 | 05-battle | CwEntryExit 复用,固定时长 |
...
进对局(CwEntryStart)
  ...
  │    CwScreenBriefing(观察→写session→点下一步)
  │      → CwScreenPlaneTransition(点空白)
  │      → InvestEnvOp(3选1,含刷新)
  │      → CwScreenWaitOneOne(~10s,1-1 不自动开店特例)
  │      → 1-1 备战就绪
  │
  ...
  │    │         → BuyCard/RefreshShop 波循环 → 空序列 → CwOpCloseShop(含字段清理)
...
| 字段族 | 写者(白名单) | 清理 | 主读者 |
|---|---|---|---|
| briefing_affixes / briefing_bosses / enemy_difficulty | **CwScreenBriefing**(位面详情兜底=其重试形态) | 局终 | decide_encounter / 难度账 / 遥测 |
...
| shop_cards / 刷新花费 | 商店观察 | **CwOpCloseShop 完成承诺清理** | decide_shop_screen |
| refresh_probs | 商店观察 | 不清理(等级函数) | decide_shop_screen / 经济账 |
| node_type / 节点台账 | 节点探针(CwOpCloseShop 后,类型分派)/ 备战观察 | 位面切换覆写 | 节点分发 / boss 判定 / 难度账 |
| 投资日程账本(新增) | 投资策略op(选卡登记) | 局终 | 流程层日程调度 |
| 难度账/marginal_value 输入 | decide_encounter / 战果记录 | 局终 | decide_encounter |

纪律:新增写入点 = 违反白名单;ctx 信箱字段(cw_briefing_*)随 CwScreenBriefing 落地删除。
...
|---|---|---|
| P1 商店链原子化(=W970批A) | CwOpOpenShop/CwOpBuyCards/CwOpCloseShop 拆分(源码锁迁移清单前置);遥测写点随迁;LOCKED_RESUME 删 | 锁按清单更新后绿 + decisions.jsonl 逐决策对拍 + 全量测试 |
...
| P3 开局序列+overlay族+**编排切换**(=W970批C 全项) | CwScreenBriefing/CwScreenPlaneTransition/CwScreenWaitOneOne 新建;七 overlay op 统一模式;开局编排接线(主循环瘦身:0a0b/分支6/投资环境段退役);**纯分发器接管备战/商店常态编排(W970 批 C 全项:RunBuyPhase 解体、腾席链 b read_only、探针挂点、_handle_bench_full 合流)**;干扰弹窗分支保留 | 实机单局:开局序列走查 + overlay 各触发一次 + 常态编排对照 |
...
| 黑板模式脏读(陈旧字段) | 生命周期三分类 + CwOpCloseShop 式清理者白名单 |
...
- cw_loop 结算分支**遥测读点重组细节**归 P4 实施设计(本篇只定归属与红线)。
