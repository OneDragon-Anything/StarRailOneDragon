...
| 7 | ~~C6 装备转移~~ **无效实现待删改**(装备不能角色间直拖;转移途径=卖角色[免费主力,装备自动回区]或扳手拆[消耗稀缺工具,策略决策]——机制纠正 2026-09-02,用户批评「没看扳手是什么」后修正) | 转移期望态 = 卖角色(deployed−1+owned+装备返还+gold+)→重买/重穿;扳手拆另计「owned 扳手−1」(稀缺资源消耗,策略层决策非免费操作) |
