...
- **sim 可观测性(L8)**:四规则落 mandate_v1 决策层,sim 默认被测体与实机同源 → sim 可见、可 A/B;**盲区单列**:扑满守卫分支依赖过热环境建模,sim 批不含过热环境局时该分支结构性不可见(只实机验证),A/B 报告须带此声明防假阴性。
  - **勘误注(2026-09-08 奖励帧策略审查·建议①可见性批)**:上句「四规则 sim 可见、可 A/B」对**规则①与 ②(b) 失真**——两者帧型判据消费同一谓词 `reward_node_suppressed`(D3 同谓词同向),而 sim 决策帧(引擎自带 `st`)在本批前零 `node_type` 写点,谓词在 sim 全域 None fail-open:**基础抑制分支对 sim 结构性盲区,彼时盲区单列只报了扑满守卫分支,同型盲区漏报**(规则①/②(b) 的 sim A/B 在填充前不可能,申报口径与事实不符)。修复 = sim 引擎决策帧填充 `st.node_type`(`sim/engine_p1.py` 决策帧写点,与 `session.node_type_current` 同源同点;实机对应面 = ADR-0587 店开帧台账制);填充后规则①/②(b) 在 sim 奖励帧按实机语义生效,该生效是可见性批的申报行为面(逐条申报见引擎写点注释)。③④ 判据节点无关,不受此盲区辖。
