...
| 卖前对拍不符 | **守卫断言两级分型**（2026-09-05 双账 HIT 实证定谳）——`guard_expected_vs_tracked`:①多集等价(成员同、槽位序异)⇒ WARNING「槽位布局漂移」+按 tracked 真值就地重播种逻辑态 bench,**不炸环**;②真多集分歧 ⇒ AssertionError 炸出(双属归因不变)。`guard_proposal_vs_expected`(提案 vs 期望态)恒炸;满栏合成买双账同构豁免,豁免仅剩非合成满栏买像素差漏检的 fail-open 残余窗 | 不卖错件(卖出不可逆)。两守卫零读屏(tracked 纯内存),详见 `../screens/op-layer.md` §1.3/`action_exec.md` §4 |
