...
```shell
uv run python -m sr_od.backend.entry.server --host 127.0.0.1 --port 24001
```

启动流程：

1. 创建 `SrContext()`。
2. 创建 `SrBackendContext(ctx)`。
...
同一进程同时挂载 MCP（tool-call）和 HTTP（REST）两套适配器，共享同一个 `SrBackendContext`：
...
页面会低频轮询 `/game/status`，并尾读 `.debug/sr_od_mcp/main_server.log` 到消息框；这些日志只用于 GUI 展示，不通过 MCP tool 返回给 agent。GUI 会过滤自身轮询、`GET /mcp` 探测和 Windows 连接重置这类噪音，并限制消息框保留行数，避免长时间打开后卡顿。
...
- server 进程独立持有一个 `SrContext`。
- 每个进程内通过运行槽保证同进程单跑道。
- 常驻 `SrContext` 可避免 OCR / YOLO 冷启动成本。
