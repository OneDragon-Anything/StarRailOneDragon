...
session → 返回」,供 `run_operation` 单独调起,不等 battle_loop 的备战稳定帧
...
复用明细(不重造 reader):采集本体 = :class:`CollectPlaneIntel`
...
#: 位面详情屏 id_mark(collect_plane_intel 同名常量的本地引用,免循环依赖)
_PD_SCREEN: str = '货币战争-位面详情'
_PREP_SCREEN: str = '货币战争-备战'


class TakeoverCollectPlaneIntel(SrOperation):
    ...
    def takeover(self) -> OperationRoundResult:
        """入口门 + 跳过门 + 委派 CollectPlaneIntel。

        入口契约:必须在备战或位面详情屏(对局中的合法画面);其它屏 =
        调用时机错误(局外/大厅/战斗中),快速 fail 并存证截图——battle_loop
        ...
        # 真值已在(battle_loop 早前采过/上一轮本 op 成功)→ 零点击直接结论。
        ...
        _log.info('[cw-takeover] session 无位面序真值 → 委派 CollectPlaneIntel 实采')
        sub = CollectPlaneIntel(self.ctx)
    ...
    def write_back(self) -> OperationRoundResult:
        ...
        if not bosses:
            ...
            if sess is not None and getattr(sess, 'briefing_bosses', None):
                return self.round_success(TakeoverCollectPlaneIntel.STATUS_SKIP)
        ...
        if self.round_by_find_area(screen, _PD_SCREEN, '标识-位面详情标题',
            ...
            # CollectPlaneIntel 出口契约保证已回备战;仍见详情屏 = 转场未落地
        ...
        # 已有真值保护:session 本有真值(battle_loop 早前采过 / 上一轮本 op
        ...
        if getattr(sess, 'briefing_bosses', None):
            ...
            _log.info('[cw-takeover] %s:已有真值不覆写,池已清',
                      TakeoverCollectPlaneIntel.STATUS_SKIP)
            return self.round_success(TakeoverCollectPlaneIntel.STATUS_SKIP)

        # 保位写(None=徽章态位面原样占槽,滤掉=后续位面名字左移错序,
        # W221/ADR-0398;与 battle_loop `_names = list(...)` 同口径)
        names = list(bosses)
        ...
        _log.info('[cw-takeover] %s:bosses=%s affixes=%s',
                  TakeoverCollectPlaneIntel.STATUS_DONE, names,
                  getattr(sess, 'briefing_affixes', None))
        return self.round_success(f'{TakeoverCollectPlaneIntel.STATUS_DONE}:bosses={names}')
