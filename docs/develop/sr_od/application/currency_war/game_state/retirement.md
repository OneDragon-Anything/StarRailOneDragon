...
> 排期过程件 r5-migration-plan(单源直迁八波)已删(考古走 git 历史),本文不复写排期;
> 变更史归 ADR,本文文末只留版本历史表。
...
| cw4_counters | 流退役(R5 W4 落码;键收编审计定谳:效果域 0 键/策略行为键 256 字面+16 闭族 80 实例+9 开放族全保留=策略侧决策行 strategy-state 载体,无消费删 0 键) | 键=策略 state 容器(StrategyState.cw4_counters),sim 红则/轮差分/预注册不经流照旧;局终级全键聚合=局终域行载荷 ``MatchFinal.cw4_counters``;键全集登记单一源=封闭锁(test_cw4_key_closure) |
...
退役排期与波次的原单一源(r5-migration-plan §2,单源直迁八波 W1-W8)已随过程件清理删除(考古走 git 历史);本文只记各流当前处置形态,不复写排期。
原影子口径的 M1-M5 排期框架已随直迁裁定作废(推翻影子双写;
删除波 1 已按直迁口径执行 9 流写入端删除)。旧编号引用对照:

| 旧框架(M1-M5,影子口径,已作废) | 直迁八波去向 |
|---|---|
| M1 写入口批 | 写入口/渠道签名/流水落盘 = R1 系列已交付;journal 常开化 = W1;obs_event 登记面收编义务折入 W1 |
...
r5-migration-plan(已删过程件,git 可溯) §4 逐条重构表,本文只留当前形态:

1. 黑窗消解三条件:**消解**——无双写窗即无黑窗概念;其保护意图(决策源/sim/判读
   三面不悬空)转为波序门(W7 删写面前 W3-W5 读面已切,W8 删本体前 W6 消费面已切)。
2. obs_event 登记面收编:**已交付**(W1)——obs_conflicts 写面
   删除前收编在位,拒读留证义务不蒸发。
...
| 2026-09-11 | 直迁重构:影子期/M1-M5 排期框架与前置六条影子口径作废——§5 排期改指 R5 八波正本(附旧编号对照注)、§6 前置条件改直迁形态、逐批落点对照叙述链收敛为本表;§2 board_state_archive 行「影子期继续在产」改「停写前继续在产」 | (直迁裁定)/ (删除波 1 落档+本重构兑现);排期正本 = r5-migration-plan(已删,git 可溯) |
...
| 2026-09-12 | §7 #10 行修订:迁移动作列「消费切换子集落地前 state_ref 带 pin_scope 标记」随字段退役删除作废——行面钉字段 = state_ref 单字段,历史档案判读知识指 schema 正本 §8-2;批次列改「接线 = W6 后另批(定谳 C6)」 | 候裁 10 终裁 C 案(定谳记录 = unified-state 迭代定谳记录,git 历史可溯);字段退役执行(commit b0775d8b) |
