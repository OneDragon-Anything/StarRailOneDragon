...
- 0536 退役语义承接核对（E-2 改投后「遭遇判据语义承接 = 13_pick_family E3 行」仍成立）+ proofs/（p64、T-175:194 等）引用处置核对——存档性引用加注、现态引用改指代码本体/正本 ← 末阶段
