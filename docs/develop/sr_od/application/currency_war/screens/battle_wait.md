...
**动作 op 与交回对照表**(本篇唯一动作清单;本屏无注册表动作 op、无策略器问询,全部动作 = 驻留状态机内的留守机械臂;「交回外循环」= 本访问结束、控制权交回 `cw_loop.py::CwLoop.loop` 重判):

| 动作 op(词表参数) | 发出方式 | 上报 | 触发返回外循环 |
|---|---|---|---|
| 无注册表动作 op——继续挑战推进臂(挑战成功结算段) | 画面 op 留守臂(`_dispatch_frame`:采集钩子 → 读点前等 1.5s + 重截 → `_record_round_outcome` → 新帧指纹计数 → 点「货币战争-结算.按钮-继续挑战」;停留 ≥`SETTLE_STAY_LONG_PRESS` 轮长按兜底) | 无自上报(结算读数走结算域 `_record_round_outcome` 观察写端,非动作上报) | 否(非终结):发出后 `round_wait` 驻留轮询 |
