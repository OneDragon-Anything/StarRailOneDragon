...
|---|---|---|---|
| `CwActionPickInvestOp`(`CwActionPickInvestParam`,投资两屏共用注册行) | 注册表工厂 `action_op_for`(决策半组装 `OverlayPickExecEnv`:定位点/确认钮/裁决词) | 自上报 `report_action_pick_invest_param` 发射相(意图遥测,容器零写);落地相由本 op 重入裁决出口持 `EVIDENCE_OVERLAY_CLOSED` + `session` 形参调用——portal 支整支走获得链 `gain_invest_env`(active_env 注册 + portal 登记 + 环境赠卡入席,见 §6) | 否(非终结):发出后 `round_wait` 循环推进;落地由重入裁决判——锚不在 = success 交回外循环(见 §5) |
...
├─ 选卡(选择事实零容器写,active_env/portal 登记不在画面 op——均迁获得链
│    `kernel/cw_gain_chain.py::gain_invest_env`,落地相 = 重入裁决出口上报;
│    上报函数 portal 支支走获得链,session 形参显式传入[局外/测试 = None
│    登记腿跳过];欢愉契约 provisional 条件腿随获得链回调 on_env_gained
│    枚举迁移,行为逐位等价)
...
- `active_env` 写入:动作落地相获得链写(`kernel/cw_gain_chain.py::gain_invest_env`,
  写行署名 `GAIN_CHAIN_PRODUCER`;本屏零时点写,原点卡前写时点已退役,用户裁定
  2026-09-21;链式规范正本 = [../game_state/gain-chain.md](../game_state/gain-chain.md))。
- portal 效果登记:`gain_invest_env` 链内腿 → `kernel/cw_effect_inventory.py::
  register_portal_from_env`(best-effort,失败留证不阻塞;登记宿主已迁获得链,
  经济判据接登记数据归后续批)。
...
- 测试锁:两 node 行为锁 + 写入流对拍 = `sr-od-test/test/sr_od/application/currency_war/test_cw_obs_arch_phase_screens.py`(重入裁决×门组合/观察门 miss 早退与 report/active_env 写时点迁移锁:点卡前无写、落地相经获得链有写)。
