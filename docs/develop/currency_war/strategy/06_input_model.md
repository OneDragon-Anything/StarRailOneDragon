...
| 经济 | `gold` / `level` / `level_up_cost` / `xp_progress` / `streak` / `shop_refresh_cost`(= 刷新实付金,恒基价常量 `REFRESH_COST_BASE=2`,ADR-0456——旧「刷新金币数」rect 是面板徽标=利息数值非刷价,OCR 已退主链)/ `refresh_probs`(商店开态概率条真值 {费用档:概率},投资环境轮岗翻倍的直接观测;None=未读/商店关→`_sample_cost` 退基线表。**cw3 决策消费语义**:非 None 帧直传 `resolve_prob_table` 作真值,不退基线;`node_type` 消费侧真值链优先级=`node_type_current`(ADR-0276)→`last_node_type`→`node_type`,读不到保守按战斗) |
...
| 保真位 | `hp_readable` / `gold_readable` / `board_readable` / `level_readable`(level 值来源位:False=纯启发式兜底值,「真读 4」与「兜底 4」判读可分;写入端=`read_game_state`,缺省 True=sim 恒真读约定)/ `enemy_difficulty_live`(敌难值来源位:True=当轮真读,False=回退简报恒值或双源皆无,False 帧不作难度爬升曲线样本)/ `hp_trusted`(hp 值可信位,ADR-0282 语义细分、ADR-0428:True=真读帧或沿用 `last_hp_real` 真值的帧,False=仅「开局全无真值兜底 100」的假值帧;写入端唯一 = `read_game_state`,与 `hp_readable` 分工 = 决策谓词按「readable ∨ trusted」放行沿用真值帧、兜底假值帧仍拒)(「值是否真读到」标记族;int/dict 契约下动画帧 miss 与真值不可区分;遥测/对拍消费,决策默认不用。hp 失读时值=沿用真值非兜底 100,ADR-0282) |
