...
class DecisionV2Registry:
    ...
    #: 语义:plane==2 中段(线已锁 ∧ 形态缺口 ∧ 溢余段)的定向支出授权
    #: ——四条件合取(T1 锁定/T2 缺口/T3 血线恶化报警面/T4 g≥50)+两层
    #: 强度(常授权层=买牌预算地板「花完仍≥50」;加急层=允许破息至
    #: 保留金下限)。授权只改「门开不开+预算上界」,候选/评分公式不动
    #: (P12 j=0 负例/升级三合法判据/血预算门 AND 全部既有单一源不动)。
    #: 总开关:False=现行为逐位一致(零漂移锚,A/B 基线臂)。
    #: **开臂判据挂账(事前写死,双钥匙缺一不开;策略开关生命周期第 1 态
    #: 准入理由=行为输入未就绪)**:
    ...
    #: ② sim A/B 预注册协议主判据达标且护栏全绿(协议=.debug/temp/
    #:    currency_war/w757_p2_spend_auth/SIM_AB_PREREGISTRATION.md v2,
    #:    n=500/臂配对 McNemar,主判据 M1-M3,护栏 G1-G6)。
    #: 验证不过的出口=删码留 ADR-0480(生命周期第 4 态),禁悬置默认关。
    ...
    #: 常授权层贴线带逐点判是 P25 待证项——证成前保守=不买(设计占位
    #: 子句,不做生产决策);本常量只是分带定义值(非判据输出)。
