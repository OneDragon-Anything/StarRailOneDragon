...
| [dd-033](dd-033-economic-freeze-fix.md) | DD-033:经济冻结型败局三病灶治本——目标空窗(R3 断供驱逐单向门+准备域无 K 回退→引擎全停金 127 攒死;修=驱逐可逆+P1 物化不空窗+准备域 K 回退单一源)/posture 接而不用(授权面-执行面零对账,修=帧级 reconcile 置位 posture_unfulfilled)/P2 选线绕过 weak_planes(修=意向信号面统一过滤);P2 目标移交随批落地 | accepted | 2026-09-04 |
| [dd-034](dd-034-supply-confirm-merge-buy-crisis-level-yield.md) | DD-034:第 7 局复盘三候选迭代——线重推供给确认加速(在店 streak+方向外体系连续 2 轮在售⇒驱逐门槛减半,K 派生自驱逐阈值)/M2b 合并完成买入(第三张 affordable 即 2★ 合成,序位 M2 后)/危机带经验让位(level_spend_blocked 统一双栈断层,与危机买入闸门成对);sim paired:P1 冻结窗 0.97→0.80、2★ +5、终局 hp +1.17 | accepted | 2026-09-04 |
| [dd-035](dd-035-line-feasibility-supply-infeasible-exit.md) | DD-035:P2 锁线可行性三维护栏——H=min(剩余节点,⌈hp/vd_p2_loss⌉)×G=∏(1−(1−q)^H) 零新参数进三消费位(移交候选门/信号缓锁门/撤销出口③ 供给不可行降级=三证据合取+可逆不写 evicted)+ shop_unbought_reasons 席/回金投影修复与双栈语境声明;sim n=30 中性(无定谳力),n≥100 复核与实机验收挂账,R5 主死因修正=下批移 spend 侧转化与 P25 Δλ_death 标定 | accepted | 2026-09-04 |
| [dd-036](dd-036-settlement-damage-rows-temp-collection.md) | DD-036(已撤):结算屏逐角色伤害行临时采集链路——为 ΔV_2★ 阶段 2 判据立(被动观测 CI 跨 0 结构性不足+sim 循环依赖),后因用户裁定「策略核心不依赖战力建模」整体 revert(58d65d51,SCHEMA_VERSION 回 1);本文载立撤完整周期,重启从 git 历史回溯 | superseded(已撤) | 2026-09-04 |
| [dd-037](dd-037-deploy-launch-exec-contract-seam.md) | DD-037:部署「发射×执行」契约接缝修(局11 G3 停机根因)——发射门与执行器共用 kernel select_deployments 单一源谓词(has_deployable),计划空不发射 RunDeploy(bench 稳态合法);执行器 placed=0 二分 STATUS_NOOP/round_fail,no-op 不再伪装 ✓「已部署角色」;op 侧选人副本退役,P24 补部署受配方底线辖 | accepted | 2026-09-04 |
