"""HTTP 适配器：``/game/*`` 端点，把 ``SrBackendContext`` 暴露给 web/skill。

本模块在后端 game 切片（``SrBackendContext``）之上架设一层 HTTP 传输适配：
...
from sr_od.backend.http.service_routes import register_service_routes
...
async def handle_game_window(backend: SrBackendContext, _request: Request | None = None) -> Response:
    """处理 ``GET /game/window``：返回游戏窗口状态 JSON。

    通过线程池调用 backend 的 ``check_window``，将其结构化结果序列化为 JSON。

    Args:
        backend: 提供游戏切片能力的 ``SrBackendContext``。
...
async def handle_game_capture(backend: SrBackendContext, _request: Request | None = None) -> Response:
...
async def handle_game_analyze(backend: SrBackendContext, request: Request | None = None) -> Response:
...
async def handle_game_enter(backend: SrBackendContext, request: Request | None = None) -> Response:
    """处理 ``POST /game/enter?block=``：打开并进入星穹铁道游戏。
...
async def handle_game_stop(backend: SrBackendContext, _request: Request | None = None) -> Response:
...
async def handle_game_close(backend: SrBackendContext, _request: Request | None = None) -> Response:
