...
- `skills/sr-od-dev-pr-finishing/`(项目根,跨工具源 —— 即「多工具共享级」,多个 skills 感知工具都能用)。
- `sr-od-dev-` 前缀:项目开发流程类 skill。
- junction 到 `.claude/skills/sr-od-dev-pr-finishing`:Claude Code 扫 `.claude/skills/` 发现;Windows 用 junction(`mklink /J`)免管理员。
...
## 当前状态
团队已统一采用 superpowers,本 skill 已 unignore 并提交(目录名 `sr-od-dev-pr-finishing`)。CodeRabbit 限定不阻塞(团队各项目统一采用 CodeRabbit)。
