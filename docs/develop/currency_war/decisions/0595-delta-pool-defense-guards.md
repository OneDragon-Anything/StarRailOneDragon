...
## 修订（2026-09-09，Δ池守卫失效核查批）

「后果·提交快照（460e6031，16391/2246 行账）持续受保护」在本批定谳为**实现未兑现**：
塌缩守卫基线读盘面文件而非 git 提交版，盘面被 2026-09-08 三次静默覆写改成塌缩件后，
286/48 微语料再生在守卫在场下持续放行（塌缩自我延续；穿透定谳与修正见 ADR-0612）。
基线来源已修正为 git HEAD 提交版优先（盘面兜底）；本 ADR 其余裁决（run 隔离双轨、
50% 阈值、快照即真值不回灌、锚重推禁令）不变。
