...
**防死循环三层**:动作验证失败 → 同动作连败 2 次触发恢复原语(按已知弹层分型)→ 恢复无效 BailToOuter(环让位外环)或本环屏蔽该动作实例;环级连续零进展且恢复试尽 → 强制出战;外环由对局步数预算兜底。`cw_reconcile` 在环入口对账 tracking。

## 2. prep_actions:原子动作全集

| 域 | 动作 |
|---|---|
| 奖励 | ClickSphere(s) / OpenBox / PickBoxCard |
| 商店 | BuyCard / LevelUp / RefreshShop |
| 席位 | SellBench(身份感知,物理槽位)/ SellDeployed / DeployMove(腾席链) |
| 装备 | WearEquip / Synthesize(defer) |
| 战斗 | StartBattle(含未达上限确认勾选) |
| 观察管理 | EnsureShopOpen / EnsureShopClosed / CloseOverlay(gold 只在开态、HP 只在关态可读) |
