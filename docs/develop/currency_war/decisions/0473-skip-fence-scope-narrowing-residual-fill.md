...
W748 检查网自捕 2 起 `deploy_fills_cap`(642763 r5-r6 / 642795 r6-r7,dep 停滞 4/7、5/7),双臂一致。逐帧归因:两局 skip 轮 bench 残余**全部**是同名副本(与在场同名素材 + bench 内同名对),被首版保留集 ①「同名同星全场 ≥2 即保留」整批扣下(res_held=5-6),与检查器「可上货 = 非在场同名」口径正面相撞。裁决 = **检查器口径没错,保留集 ① 过宽(修复面变更)**:

- 3合1 合并域是**全场**(bench∪deployed,`cw_state._merge_bench`/生产 DeployBench 同口径)——部署 bench 对之一不破坏合成进度;生产围栏 dedup(r404-A2/5.1.7)只拦「**在场**同名」,bench 对的一张在生产照常会上;`cw_plan` 同名对保护辖的是**卖出**(防卖素材),不辖部署。首版把 bench 对整对保留 = 把卖保护误译为部署保留,sim 偏离生产保真度。
- **修法(本增补)**:保留集 ① 删除——素材副本交围栏 dedup 自然 held(在场同名者恒不进 up 集,行为等价);bench 对(无在场同名)照常补部署。保留集只剩 final 买而不上件(locked/forced hoard 名单)。
- 验证:642763/642795 复放归零(dep 6/7、7/7,lags=0,fills_cap 0)+ 6 立案种子集 6/6 归零 + 回归锁改判(bench 对一张照常上=新回归面;在场同名素材 held=等价面锁)+ cw_quick 全绿 + 640400-99 检查器口径复验。
- Considered Options 修订:原「3合1 素材副本放行一张:否决」的否决理由(「策略语义是攒合成不是凑板面」)被本波证据推翻——攒合成的域是全场,部署与攒合成不冲突;该选项即本增补的落地方。
