...
- **`refresh_ev_budget(state, session) -> int`(刷新预算)**:`min(6, ⌊(g−R*)/刷价⌋)`——只花溢余(W610 spec 预算式;6 刷帽单一源 = REFRESH_ROLL_CAP)。**合法 0 帧**:g≤R* 常态帧与应急/血预算停手帧(`filters.is_emergency` 单一源)——0 流过 scoring P2 窗判据(`>0 即窗开`)与存息准入门;定向刷新授权是独立车道不并入。
