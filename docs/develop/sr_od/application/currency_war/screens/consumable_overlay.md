...
- 外循环分支 0f′:双 OCR 条件「消耗品」∧「拖动到」(lcs 0.9),判定留外循环([../flow/outer_loop.md](../flow/outer_loop.md) §2.2);`entry_ok` 与分发判定**同源同参**。无画面档(`SCREEN_NAME = ''`):「消耗品」= 类型 label、「拖动到」= 拖动使用说明(仅消耗品详情 modal 有,备战底部消耗品栏无),两条件合取不误匹配备战。

## 2. 画面形态声明

**空决策形态**。推进型变体(`_progression_base.py::CwProgressionScreenOp` 子类):入口观察 → `progress_once` 单次推进 → 重入观察裁决交回;节点预算 = 2(合同 = [../flow/screen_op.md](../flow/screen_op.md))。
