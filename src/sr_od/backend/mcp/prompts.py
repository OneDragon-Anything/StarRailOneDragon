...
def render_instructions() -> str:
    ...
    """
    return """星穹铁道一条龙 MCP:操作一台 Windows 上的星穹铁道实机(1080p)。
...
def render_prompt_guide(name: str, app_id: str | None = None) -> str:
    ...
    Args:
        name: 指南名称。
        app_id: 独立应用 ID，仅 ``sr_run_standalone_app`` 使用。

    Returns:
        操作指南文本；未知名称返回可用名称提示。
    """
    if name == "sr_check_status":
        return render_check_status_guide()
    if name == "sr_run_one_dragon":
        return render_run_one_dragon_guide()
    if name == "sr_run_standalone_app":
        return render_run_standalone_app_guide(app_id)
    if name == "sr_dev_validate_op":
...
def register_prompts(mcp: FastMCP) -> None:
    """把星穹铁道一条龙常用操作 prompt 注册到 FastMCP(仅 user 模式项;dev 走 tool)。
    ...
    def sr_run_standalone_app(app_id: str | None = None) -> str:
        """生成独立应用运行提示词。"""
        return render_run_standalone_app_guide(app_id)
...
def register_prompt_tools(mcp: FastMCP) -> None:
    ...
    def list_mcp_usage_guides() -> list[dict[str, str]]:
        """列出 sr_od MCP 可用操作指南,相当于帮助目录。观察类。
    ...
    def get_mcp_usage_guide(name: str, app_id: str | None = None) -> str:
        """读取 sr_od MCP 操作指南，相当于某个任务的 --help。观察类。
