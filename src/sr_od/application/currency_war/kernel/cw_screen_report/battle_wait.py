"""战斗等待屏观察契约与上报(kernel 纯数据)。

观察面 = 入口裁决:门判定 + 稳定帧引用。report 写点转录来源 = 无——
本屏现役零 op 层观察容器写点:结算覆盖写端在 ``apply_settlement_cover``
动作/结算链(cw_screen_battle_wait._record_round_outcome),属结算域
...
def report_screen_battle_wait_obs(gs: GameState, obs: CwScreenBattleWaitObs, *,
    ...
    形态占位;摄入面出现时落此处(占位不构造不校验 sig——无写即无
    渠道面)。"""
