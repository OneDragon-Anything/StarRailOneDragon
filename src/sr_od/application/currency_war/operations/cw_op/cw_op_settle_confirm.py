"""货币战争 结算确认动作 op(规范 = flow/action_exec.md §2.1 触发点 1;
screens/op-layer.md §2.1 画面框 op 形态;game_state/node-derivation.md
§3.3 跃迁判定)。

非策略动作面(不经动作注册表、无 CwAction param;命名与构造从画面框
op 惯例 ``CwOpOpenShop``)。职责 = 点击「货币战争-结算/按钮-继续挑战」
(含长按兜底的点击力学)→ ``report_node_advance(
trigger='settle_confirm')`` → 返回。

**去证据门 = 点击即上报**:动作 op 不做任何确认——点了就是
上报推进;推进里只有 node_ord 为观察态(source==observation 且 value
在场)才落账(观察态门为唯一门,kernel ``report_node_advance``
结构性承载)。转移证据机制(探锚/等待/bail/补报)已退役。无证据等待
⇒「上报先于下一节点任何画面渲染」恒成立。死点击自愈链:幻影上报
(logic 态)→ 重试点击再上报被观察态门挡(零推进)→ 某次点击落地 →
备战帧读数 = hist → 等值 reanchor 翻锚定 → 下一节点解锁(最坏损失 =
幻影那次多 tick 一档发放,用户已知悉接受)。

**归属界定**:随本 op 的仅 = 点击/长按兜底/兑现回调/推进上报;结算
读点、``_record_round_outcome``、rounds_done 计数等结算链记留宿主
``CwScreenBattleWait``。战败分支不进本 op、不推进(终局流转,
节点序止于终局)。完成判据白名单(``SETTLE_COMPLETION_ANCHORS``/
``hit_settle_completion_anchor``)= 宿主 ③段出口判定独用
...
class CwOpSettleConfirm(SrOperation):
    """货币战争-结算确认 原子动作 op(点击 + 长按兜底 + 推进上报;
    去证据门形态 = 点击即上报)。

    生产路径 = ``CwScreenBattleWait`` ②段委托本类 ``execute()``。单 node
    驻留形态:每轮点「继续挑战」(停留 ≥3 轮长按兜底)→ 点击即上报
    ...
    def confirm(self) -> OperationRoundResult:
        """驻留轮:点「继续挑战」(停留 ≥3 轮长按兜底)→ 兑现回调 → 点击即上报;
