...
- 行为变化申报：
  ...
  - **条件族采样金窗**：刷新余额的条件族（本金充裕系）按段执行时点 `bs.gold` 现值评估——0q/0p 推进帧的观察仅 phase_round 不读金，采样金为上一读数沿用金，与现码（备战帧现读金含战斗胜金）存在 ±1~3 档漂移，**申报接受**（free_refresh_balance 纯 logic 域无观察纠偏，漂移留存整局；采样窗精化挂效果域批 M3）；
