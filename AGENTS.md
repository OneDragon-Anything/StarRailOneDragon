...
- **变更史不进注释**：「何时改的 / 从什么改成什么 / 勘误过程」归 ADR（与 `docs/develop/` 文档同判据，见下「文档与测试要求」）；注释只留当前值成立的理由 + 指针。自动重生成的文件，其内嵌说明同此收敛——只留当前语义 + 版本历史表，由生成器模板保证，禁止逐批续写叙述链。
...
- 写文档 / skill / 入口文件用直白表述，避免自造黑话或项目内部缩写（行业通用术语可用），首次出现的项目术语给定义 + 例子；注释的黑话与密度判据见上「注释规范」。
- **玩法知识/设计文档按「描述对象 × 变更驱动」归位（任何玩法 app 通用）**：描述游戏的知识进 `docs/game/<玩法>/`——`sources/` 放外部原文存档（按版本冻结，只带来源元数据头，原文不改，我们的批注不进存档），`research/` 放我们提炼或核实过的知识（每条标来源与证据等级，用户口述为最高权威；随游戏版本原地更新，版本号是唯一允许的时间性；提炼≠复制，只收被应用或被核实的结论；细则见 [docs/game/README.md](docs/game/README.md)），游戏数据在代码注册表全量建模后删除对应 data 文档；描述我们系统的进 `docs/develop/<玩法>/`——设计正文 as-built 无状态（下一条），决策 why 进 `decisions/`（ADR）；实现进度不进任何文档，归该玩法的进度追踪。归位判据：随游戏版本变 → game 子树；随架构变 → develop 子树；随 commit 变 → 进度追踪。
- **`docs/develop/` 文档无状态（as-built，只描述系统「现在是什么」）**：正文只写结构 / 语义 / 接口契约 / 数据流 / 边界；数值 / 权重 / 阈值不写进文档，只写常量名（单一源在代码）。四类内容禁止入正文：①进度标记（✅/❌/待做/日期——归进度追踪）；②变更与历史叙述（归 ADR，正文只留「决策见 ADR-NN」指针）；③「规划中 / 已实现」状态位（设计定案进 ADR、落地进度进进度追踪）；④会随实现演进失效的现状快照。自检判据：这句话在系统继续演进后会变成错的或废话吗？会 → 不属于设计文档。正文只在架构变化时修改，不随实现推进「同步状态」。
- 修改代码后，同步更新对应的 `docs/develop/` 文档与 `sr-od-test/` 测试。
- **sr-od-test 纪律：写/改测试前先读 [sr-od-test/README.md](sr-od-test/README.md)「测试纪律」（单一源，7 条硬规则）**。高频违点：① `test_context` 是 session 级共享，改属性一律 `monkeypatch.setattr`（裸赋值污染后续测试文件，conftest 守卫会警告+还原）；② 测试不写真实 `.debug/` 路径（用 `tmp_path`）；③ 测试不发真实网络请求（触网链路在入口 mock）；④ op 流程测试的 `execute()` 必须包 harness 的 `fast_sleep()`；⑤ 锁契约不锁分布数值，昂贵计算同次运行内只算一次多条共享；⑥ 批量模拟 n 取断言成立的最小值、不落盘优先。运行口径：串行为准（`uv run pytest sr-od-test/`），`-n 8` 仅本机可选不作规范。
