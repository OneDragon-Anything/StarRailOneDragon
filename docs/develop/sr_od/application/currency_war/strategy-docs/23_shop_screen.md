...
> 判据本体的唯一现行家 = [11_shop_decisions.md](11_shop_decisions.md)(六序/买面/卖出/刷新/升级),**引用不重复**;数值单一源在代码(常量名形态)。流程编排 = `../flow/shop_visit.md`;决策本体 = `strategies/impl/mandate_v1/shop.py::decide_shop_action`(全函数:f(期望态)→恰一个动作),单动作循环 = `operations/cw_screen/cw_screen_buy_cards.py::run_buy_waves`。
