...
**sim 已知边界**:Δ池按深度分桶,±2 深度量级测不出;「散面 vs 集中」维度 sim 战斗层不敏感、可能与实机方向相反——此类维度以实机判读为准,并把偏差记录在案;「sim 上实机前必核代理语义」(曾因 bench 计数≠上阵的代理错位得出 1.8 倍增益伪影)。**真实性演进方向(2026-08-23 用户定调)**:校准层逐步换真机制——胜负用真实 outcomes 按成型指标分桶拟合(胜率=f(引擎数));连胜金/利息已接实测真值(cw_economy 单一源)。**「识别噪声注入」不做**:识别漏检/空名槽/误读是实机代码 bug——**噪声来自我们的代码→修,随机来自游戏→建**(给 bug 建噪声模型=给 bug 续命);sim 假设完美观测是正确终态。**sim 压力测试官(同日定调,常设角色)**:派子 agent **对着玩法本身自由发挥**——指挥官**不预设场景清单**(预设=把压测官降级为执行者,问题域由压测官自己判断;2026-08-23 用户二次纠偏),只给约束:测改分离、产出格式、sim 零改动;提示可自由浏览 ledger 找「看着不对劲」的形态再定向验证。发现→修复→固化成 sim 检查项/单帧锁(检查网滚动生长)。实机 40min/局 vs sim 秒级,sim 深度挖掘是长期高杠杆。**排程纪律(防执行滑坡)**:压测官是常设在飞——一批交付→消费+派修复→**立即派下一批**,队列里恒有一个压测官在挖。**心跳=点名制(2026-08-23 定型)**:进度树「常设在飞点名册」节列全部常设角色(压测官/实机/哨兵/早停/runs断流/审计)及各自的在飞判据与空窗动作,schedule 心跳轮对着点名册逐项点名,缺项立即补——从泛泛自查变点名册,空窗无处藏(压测官空窗 25 分钟的教训反推)。
