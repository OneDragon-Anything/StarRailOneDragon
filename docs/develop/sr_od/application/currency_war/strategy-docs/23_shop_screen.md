...
> 判据本体的唯一现行家 = [11_shop_decisions.md](11_shop_decisions.md)(六序/买面/卖出/刷新/升级),**引用不重复**;数值单一源在代码(常量名形态)。流程编排 = `../screens/shop.md`;决策本体 = `strategies/impl/mandate_v1/shop.py::decide_shop_action`(全函数:f(期望态)→恰一个动作),商店画面 op = `operations/cw_screen/cw_screen_shop.py::CwScreenShop`(两 node;决策动作 node = 单动作 round_wait 循环)。
...
| 刷新 | `RefreshShop` → `cw_refresh_shop_action.py::CwActionRefreshShopOp`(**访问终结**) | 刷新门路径总账 + R2 熔断 + P92 存在性门 + P36-a(§1 刷新门行) |
