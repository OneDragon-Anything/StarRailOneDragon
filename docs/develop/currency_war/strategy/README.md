...
| 插件 | `cw_strategy`/`cw_strategy_manager`/`strategies/default_strategy` | 可替换决策大脑(第三方策略/比赛) | [07](07_plugin.md) |
| 离线工具 | `cw_weight_search`(CEM 权重搜索)/`cw_divergence_stats`(姿态分歧频率)/`cw_plan_replay_audit`(plan 对拍) | 消费 telemetry 的离线分析,不进生产链 | [05](05_observation.md) |
