...
- **清单排除项与理由**：备战（部署态/补给 revisit 按钮/攻略位移使右上角与棋盘标签锚不可靠,语料 18/57 误杀,dc435da4d → 双锚默认分支）、补给锁定（其「返回补给阶段」按钮在普通战斗节点 revisit 备战同样出现,#18 接线暂缓裁定的延续）、道具详情弹窗（道具名可变无固定身份）/消耗品浮层/阿哈装备（无独立画面档,阶段三特殊规则）。
- **语料纪律（分发身份准入）**：每画面身份须两向验证——本画面全部已存档实机帧 True + 堆叠底图/兄弟画面帧 False;备战扩展锚语料误杀事故即缺此验证所致(验证脚手架 = 测试仓 screens/ + is_target_screen 离线批跑)。
- **历史序位职责的归宿**：原「浮层先于备战」序位 = 补偿「锚命中≠前台」;现由各浮层自身精确身份承载。0m 锁定族以自身「返回XX选择」文字锚进阶段一;0a0/0a 碰撞由列车同行整行副题锚消解(git c1feb8245);0e 横幅中间态 = 身份天然 miss + 阶段二守卫等待(#30 裁定),OCR 全短语腿与 N5 复探退役。
