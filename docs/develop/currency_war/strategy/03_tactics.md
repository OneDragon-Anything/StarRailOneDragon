...
**成型停手门(ADR-0343;[13] 停手线显式化;W119/ADR-0347 收编 form_ok;W227/ADR-0400 承接维)**:层2 后置**动作级**步(`filters.formed_stop_active`+`filter_candidates` 尾段)——P1 ∧ r≥max(锁定线 `typical_form_round`,`formed_stop_min_round`)(comp 派生辖轮)∧ `form_ok`(谓词本体在 `decision_v2/phase`:意向锁∧`form_tiers` 全键满足∧核心**上场** 2★;无等级项;①锁局 P1(comp 锁)另要求体系对引擎数
≥`phase_fallback_min_engines`——[13] P1 验收仍是体系对,ADR-0367),丢弃全部 BuyCard 候选(应急态不豁免;levelup/refresh/卖/合装例外);**末窗承接维(ADR-0400)**:P1 末窗(r≥`handoff_gate_min_round`,判据单一源 `handoff.handoff_gate_gap`——投影承接档位(handoff_snapshot 现算)未达 `handoff_gate_tier_target` → **不停手继续投资**([18] 位面末 ALL IN 的承接扩展;开关 `handoff_gate_enabled`,非末窗 gap 恒 0 零漂移)。标志 `session.v3_formed_stop`→sim 账本(轮内 OR 聚合,另带 `handoff_gap` 缺口观测)/生产遥测 `formed_stop` 字段;`overflow_gold_zero_buy_streak` 检查器对成型轮重置 streak。开关 `formed_stop_enabled`(registry)。
