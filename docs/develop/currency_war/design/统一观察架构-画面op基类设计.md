...
> (记录模型详设现居 `docs/develop/sr_od/application/currency_war/changes/2026-09-11-unified-state/details/BoardState-数据结构设计.md`,迭代期;
> 正本入口 = docs/develop/currency_war/game_state/README.md,下称「记录模型设计」)——
> 记录模型管「记什么」,本文管「谁在什么时机把观察喂进去、决策怎么消费、
> 动作怎么落回」。两文冲突时以记录模型设计为字段语义详设口径,本文为流程/端口
...
- **点击链载体**:PrepActionExecutor(备战单动作执行器,CwScreenPrep 持有)、
  ...
  循环 + 投影)、confirm_and_verify / safe_click(_overlay_confirm,浮层
...
- **验真锚退役与迁移路径**(v12;现役判效面 as-built 照录,禁按旧语义
  ...
  §6.5-1);过渡期(批 3 落码前)现役判效面照旧运转,本节为语义出处锚。
...
- **applied 语义两域分轨(T-223/F11 修订,v12;原「两域对齐」申报
  ...
  ADR-0619 门)是动作应用语义,引擎拒买分支照旧;实机侧发射型恒真,
  不携带落地判定——规则性拒绝的实机承接 = 策略层 ADR-0619 发射前置谓词
...
> 「验真=」标注(v12 申报)= 现役判效面 as-built 照录——验真锚已退役
> (§6.2),各动作的落地确认语义承接归观察侧 reconcile 对账;执行侧原地
> 判效拆除归批 3 同窗,禁按本列字面新写判效面。
...
- **B 测试锁五条**:①登记表封闭集锁(集外锚 = 红);②触发时点型申报
  ...
  ⑤**读向隔离锁(消费侧守卫,方向沿 ADR-0577 先例)**:决策/跟踪代码
