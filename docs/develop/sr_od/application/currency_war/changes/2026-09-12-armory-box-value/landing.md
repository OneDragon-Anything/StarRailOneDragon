...
**范围**：design.md §2.2-§2.6——`decide_box_card` 打分改 `(tier, base)` 字典序（tier 3 直击 / 2 近兑现 / 1 key 材料 / 0 通用；base = 通用值 + 材料通用性）；锚定 `state_of(session).target_comp` → `locked_comp` 两态（`_ensure_intention` 解析，`get_comp` 失败落未锁 + 日志哨兵）；tier 2 近兑现接 `session.last_owned_equips`（判定式与边界裁决按 design.md §2.3，禁增减条件）。旧锁处置：`test_cw_material_score.py` 的 target_comp 锚改 locked_comp（锚定语义已被本设计取代，锁重推非机械跟绿；docstring 记改锚原因），其两条断言语义在重锚后保持。新增行为帧锁对照 design.md §2.6 行为变化表逐行（变化行锁新行为、等价行锁现行行为保持，含 R1 加法→序数等价回归帧）。边界：不碰 decide_supply / decide_planner / equip_pick 判据、不碰 obs 层与执行器（`_pick_box_card` 机械链不动）。
