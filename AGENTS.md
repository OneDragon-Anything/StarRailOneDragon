...
- **变更史不进注释**：「何时改的 / 从什么改成什么 / 勘误过程」归 ADR（与 `docs/develop/` 文档同判据，见下「文档规范」）；注释只留当前值成立的理由 + 指针。自动重生成的文件，其内嵌说明同此收敛——只留当前语义 + 版本历史表，由生成器模板保证，禁止逐批续写叙述链。
...
- 写文档 / skill / 入口文件用直白表述，禁自造黑话或项目内部缩写（行业通用术语可用），首次出现的项目术语给定义 + 例子;注释同判据，见上「注释规范」。
- **知识归位（任何玩法 app 通用）**：描述游戏的知识 → `docs/game/<玩法>/`(`sources/` = 外部原文存档、`research/` = 提炼结论，只收被应用或被核实的，细则见 [docs/game/README.md](docs/game/README.md));描述我们的系统 → `docs/develop/<玩法>/`(设计正文 as-built 见下条，决策 why 进 `decisions/` ADR);游戏数据在代码注册表全量建模后删除对应 data 文档;实现进度不进任何文档，归进度追踪。判据:随游戏版本变 → game 子树;随架构变 → develop 子树;随 commit 变 → 进度追踪。
- **`docs/develop/` 正文 as-built 无状态**：只写结构 / 语义 / 接口契约 / 数据流 / 边界;数值 / 权重 / 阈值只写常量名（单一源在代码）。进度标记、变更历史（归 ADR）、状态位、会失效的现状快照四类禁入正文;正文只在架构变化时修改。
- 修改代码后，同步更新对应的 `docs/develop/` 文档;复杂功能、架构调整或新自动化流程，先补设计/说明文档，再继续实现。
...
修改代码后同步更新 `sr-od-test/` 测试;**提交前验证**：① 改常量 / 签名 / 数据字段前先 grep 消费点与测试锁值（预判波及面）;② `ruff check` + 直接受影响测试;③ 相关测试全量一次通过才提交——耦合模块「子集绿」是伪安全;无法本地完成时说明缺失前提。
