...
- **问题**:①「任一契合位」= 酌情措辞,违写作硬规则 2(实现者无需再设计);②emit 编排 ① 实体面「控制流与 overlay 切换优先于三遍」(entry.py:437-441)——armed 帧恰逢 obs.boxes/spheres 时,前置位在实体面前(不收箱/球直接发射/访问)还是之后(先收球)是行为语义选择,未定且未进行为变化申报;③「骨架 pass 之前」在 ①′/②/③ 之间仍有多个可插槽,§9.6 位次「动作链之前」不足以唯一定位。
- **严重度**:次要。
