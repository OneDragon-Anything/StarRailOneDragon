...
class PcControllerBase(ControllerBase):
    ...
    def click(self, pos: Point = None, press_time: float = 0.1, pc_alt: bool = False,
        ...
        Returns:
            不在窗口区域时不点击 返回False
        """
        self._check_stop_guard()  # 停机守卫:停机后任何点击前中止
        if self.background_mode:
            if gamepad_key:
    ...
    def drag_to(self, end: Point, start: Point | None = None, duration: float = 0.5,
        ...
        """
        self._check_stop_guard()  # 停机守卫:停机后任何拖拽前中止
        if start is None:
    ...
    def mouse_move(self, game_pos: Point) -> None:
        ...
        鼠标移动到指定的位置
        """
        self._check_stop_guard()  # 停机守卫:物理光标移动同样抢用户鼠标
    ...
    def park_cursor(self, game_pos: Point,
        ...
        光标会停在上次交互处(前提:截图不保证无光标);点击/拖拽目标与后续识别区域
