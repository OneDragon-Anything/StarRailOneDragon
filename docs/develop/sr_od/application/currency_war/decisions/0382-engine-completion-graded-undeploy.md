...
- sim A/B n=300(同池 861fc9f6 导出件重放,seeds 0-299,invest on,
  ...
  (`deep_read/W202_报告.md`(原始件已灭失,2026-09-12 清理))。
- 门值标定三跑(门=2/3/4)全记录于 W202 报告,门=2/3 的坏翻转
  局明细(65/119/172)如实记档。
- **硬门口径注记(W203 巡检裁决,随 W205 批补录)**:门值标定的
  回退基准(锚 mal 24)是**当时 HEAD**(W201 后、W202 未落地)
  的读数——分布级基线随修法落地而换锚(现锚 W205c 后 mal 20),
  后续若重开同族门值标定,回退基准一律取「标定时点的 HEAD」而非
  历史锚值;硬门判据以聚合指标(benign→mal=0)为准,涉 192 型
  互搏的翻转(已知良性震荡局)须在聚合门外单独呈报,不得以其
  归入 benign→mal 计数掩盖方向。**192 型识别式(W207 定型)**:
  「同体系阈值下穿≥2 ∧ 终局 strict_mal」(`w207_evicts.py` 口径,
  .debug/temp/currency_war/w207_churn/;strict 前缀 churn 指标已
  退化勿用)——独立块 300-599 复核 192 型=0,维持良性震荡判
  (W201 §3 候选修法挂档不动)。识别式中的「终局 strict_mal」为
  strict 口径(非宽口径 benign;两口径块A 各命中 192 与 {192,76},
  报数须声明口径)。
