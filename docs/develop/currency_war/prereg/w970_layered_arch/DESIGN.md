...
- 实施进度:批 A(op 原子化)完成——`OpenShopOp`/`BuyCardsOp`/`CloseShopOp` 抽出(`operations/prep/open_shop.py`/`buy_cards.py`/`close_shop.py`),BuyShopCards 改编排壳,LOCKED_RESUME_ENHANCED 已删;源码锁迁移清单见 ADR 与测试仓对应锁改动;批 B/C/D 未开始
...
**待用户裁决项(已决,2026-09-02)**:F7 架构——判稳轮询与 DD-011「不设第二道稳定验证」字面冲突。**用户裁决 = 方案①**:DD-011 amended,完成判定扩为两种合法形态(固定时长自等 ∨ 画面身份确认轮询——后者按用户口述判稳标志判定**操作是否生效**,非指纹测量;判稳标志纪律 = 目标画面独有锚);判稳轮询不算 gate 残留,不阻塞批 D。dd-011 已回写(amended 节 + 迁移清单开商店行更正 + 判稳标志纪律),本文 §4.2 完成承诺列同口径。
