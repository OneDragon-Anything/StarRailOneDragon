...
- **match 生命周期前置**(对抗轮 1 代码现实 P1 补):现役 session 在 run 首帧 handle_init 才建立——CwScreenBriefing 直写 session 要求 **match 建立前移**(进对局即建 session,先于首帧识别);P2 实施时同步迁移,否则 CwScreenBriefing 无写目标。
...
- **阶段顺序修正**(对抗轮 1 P0 采纳):ctx 信箱的**删除**不随 P2(写路径收编批)——CwScreenBriefing 在 P3 才存在,P2 删 copy 段 = 词缀/boss 链静默断流(中性退化对拍测不出)。修正:P2 期 CwScreenBriefing 与旧 ctx 拷贝段**双写过渡**,P3 CwScreenBriefing 稳定后删 ctx 段。
...
- 现役对照:快照链「每帧新建 GameState」天然无残留;黑板化后清理从「隐式(帧新建)」变「显式(完成承诺内清理)」——CwOpCloseShop 承诺 = 收起点击 + 1.0s 自等 + **商店族字段清理**,三者一体。
...
见总纲 §4 表(本篇与总纲 §4 同源,总纲为准;此处不重复)。纪律:新增写入点 = 违反白名单;ctx 信箱字段随 CwScreenBriefing 落地删除。
...
| briefing_affixes | 持久 | CwScreenBriefing 内联直写(仅空时写;ctx 信箱字段已随 P5 物理删除) | decide_encounter(mechanics_fit)/ 遥测 |
| selected_difficulty / enemy_difficulty | 持久 | 入口 establish_new_match(职级,经 ctx.cw_selected_difficulty 活通道)/ CwScreenBriefing 直写(难度) | 难度账 / read_game_state |
| briefing_bosses | 持久 | CwScreenBriefing 直写 / CwScreenPlaneIntel(实采覆写)/ cw_screen_prep 补采 | boss_fit / reconcile_briefing_vs_plane_intel(对账双输入之简报侧,**与实采侧不合并**) |
...
| megastar_candidate_clicked | handler 内 | CwScreenMegastar | handler 幂等 |
| _supply_refresh_used / _encounter_refresh_used | 持久(每局 1 次门) | 补给/遭遇 handler | 同 handler |
| last_node_type / node_type_current / upcoming_types / nodeseq_probe_anchor / plane_node_table / plane_node_table_plane / plane_lengths_seen | 持久(位面切换覆写) | 节点探针(cw_screen_prep._probe_node_type,CloseShop 后挂点)/ 备战观察 | boss 判定 / 节点分发 / 遥测 |
...
| last_owned_equips | 新鲜快照 | CwOpEquipAll | _pseudo_state / 遥测 |
...
| last_state | 新鲜快照 | 备战观察(director._observe)/ 商店观察(cw_op_buy_cards 波顶) | overlay handler 近似 / session 锚 |
...
| pending_deploys | 环级 | 旧部署意图组装 | CwOpDeploy |
| defer_count / prep_phase / prep_phase_retry | 环级(环入口清零) | 决策函数(黑板豁免类目)+ director | 环步状态机 |
| pending_buy_expect | 单元(消费即清) | shop.py 买牌单元收尾 | CwScreenPrep 对账 |
| xp_expect_ledger | 持久 | CwScreenPrep(XpLedger) | 经验对账 |
| v3_spend_auth / v3_posture_receipt / v3_posture_unfulfilled | 帧级 | 决策预算核(attach)/ 仲裁收口(reconcile) | 预算-回执对账门 |
| **prep_obs_frame**(P2 新增) | 新鲜快照(每次备战观察覆写) | 备战观察装配点(cw_screen_prep._observe)/ 破警告派生帧 / 兼容期 decide_prep_action 薄委托 | decide_prep_screen(黑板唯一输入) |
| **shop_state_frame**(P2 新增) | 画面态(进店波顶覆写;CwOpCloseShop 完成承诺清理随 P3 接管) | 商店观察融合段(cw_op_buy_cards.run_buy_waves 波顶)/ 兼容期 decide_prep 薄委托 / sim(独立批) | decide_shop_screen(黑板唯一输入) |
...
| CwOpCloseShop | 商店族字段清理 | shop_cards 等(§3) |
