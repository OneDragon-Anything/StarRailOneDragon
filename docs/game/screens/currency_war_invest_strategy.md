---
screen_name: 货币战争-投资策略
appears_in: [currency_war]
last_updated: 2026-08-04
source_image: screens/货币战争-投资策略/default.webp
---

# 货币战争-投资策略(3 选 1)

备战内触发的投资策略选择(区别于开局"投资环境"):从 3 个策略祝福里选 1,给小队 buff。

## 何时出现 + 状态流转

- **入口**:备战阶段触发(特定轮次 / 条件)→ 本屏(遮罩在备战之上)。展开为**两段**:备战画面先完整可见 → 中部先出「请选择投资策略」横幅(选卡未出现;横幅文字在 y≈510,**不在** id_mark 位 [855,78,1065,118])→ 随后展开为全屏选卡页(标题落顶部)。**横幅中间态无可交互元素,裁定不处理、等展开完成(2026-09-15)**。
- **出口**:点一张卡选中 →「确认」→ 回备战。或「返回备战界面」(右上)取消。
- 与"投资环境"(开局 3 选 1,定阵营走向)不同:投资策略是**过程中**的策略 buff(334 池,代码单一源 `cw_invest_data.py`,plaza API)。

## 识别特征(稳定锚点)

- 独有文字:「请选择投资策略」+ 卡标题(XX 祝福 / XX 星徽)+「刷新次数」+「确认」+「返回备战界面」。
- 区别于投资环境:标题"投资策略"非"投资环境";有"返回备战界面"。

## 可交互元素

- 3 张策略卡:左/中/右,点**描述区**(y≈545)选中 —— 点卡名(y≈474)开详情不选中(实测点名 540+ 次从没选中 → 确认灰 → 卡死 18min)。OCR 读卡名(y≈490 行)按 center-x 排序。
- 「确认」:底部,screen_info ``按钮-确认`` center ≈ (978,983)。
- 「刷新次数：N」:每卡底部 y≈841。
- 「返回备战界面」:(1717,44)右上(取消回备战)。

> screen_info ``currency_war_invest_strategy``:``标识-请选择投资策略``(id_mark)+ ``区域-卡牌描述行``(给选中 Y=545)+ ``按钮-确认``。op ``cw_screen_invest_strategy`` 经 ``cw_observation.area_center`` 读(screen_info 缺失才用兜底常量)。

## 关键数据(策略相关)

> bot 已接 ``decide_event``(``cw_screen_invest_strategy``):OCR 3 卡名 → 按事件白名单打分 → 点最优卡 + 确认(2026-08-04)。overlay 时 board 不可读 → 空 board stub。

- **3 选项(本场实例)**:
  1. **幸运星祝福** — 得【幸运星】;小队 +15% 幸运一击率 + 5% 速度。
  2. **生命之花祝福** — 得【生命之花】;小队 +15% 生命增幅 + 5% 治疗强度。
  3. **治疗星徽** — 得【治疗星徽】+ 1 个【风堇】。
- **投资策略池**:plaza API V4.4 共 334 个(代码 `cw_invest_data.py` 全量),每场随机出 3。

## 识别快照

- 匹配画面:`货币战争-投资策略` **精准命中**(id_mark ``标识-请选择投资策略`` + ``按钮-确认`` 全中,conf 0.999)。
- 关键 OCR:「请选择投资策略」(859,82)、3 卡名(愚者恶作剧/量产型装甲祝福/借力打力,@y≈476)+ 描述、「刷新次数1」×3(@y841)、「确认」(951,967)、「返回备战界面」(1717,44)。
- 视觉大模型(主观):3 卡横排,各含策略图标 + 描述;默认无卡选中。

## 备注

- **选中 mechanics(2026-08-04 实测)**:默认无卡选中;点**卡名**(y≈474)开详情不选中(曾致卡死 18min);点**描述区**(y≈545)才选中。op 用 OCR 卡名 center-x + 描述区 Y 拼 click(screen_info ``区域-卡牌描述行`` 给 Y)。
- 归档:`screens/货币战争-投资策略/default.webp`。
- screen_info:`currency_war_invest_strategy`(task#20)—— ``标识-请选择投资策略`` / ``区域-卡牌描述行`` / ``按钮-确认``。
- 关键点击防吞:关键 click 前 mouse_move(零移动不被判 drag)。
