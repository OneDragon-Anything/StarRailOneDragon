...
- 阶段一身份分发(号制已退役):id_mark 锚「货币战争-备战-武装箱选择.标识-请选择」。入口链 = 备战访问内 `OpenBox` 点开启(**终结化**——开箱即交回外循环)→ 外循环按本画面分发本 op(选卡画面盖备战,历史无身份行时误派备战 op ping-pong);单一源 = [../flow/outer_loop.md](../flow/outer_loop.md) §2.2。
