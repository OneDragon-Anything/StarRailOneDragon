...
#   encounter 结构性恒败(p=0);
#   boss    rung 表 (0, 0, 0.25) + rung2 桶外推(跨节点
#           外推边界已声明)。
...
# ⚠️ 数据边界:语料全部来自旧策略(line_strategy)病局
...
# 星级分量(`w230_star_form/`/):star_depth=上场件 Σ(star−1)
# (全量口径);engines=1 桶内 sd=0 → 0/8 胜,sd∈{1,2}
# → 3/15(0.20)——方向为正;sd≥3 零胜但 n≤4 不可辨 → 保守 0.5
# (一颗 2★ 折半台引擎)+ 敏感性端点 0/0.25/1.0。
@dataclass(frozen=True)
class P2CombatCalib:
    """P2 段战斗存活层参数族(`w193_p2sim/`/;单一注入点,A/B 同通道)。

    ``calibrated=False`` = 逐位回 `w157_p2/`/行为(Δ池 plane=2 桶
    优先 + ``P2_BATTLE_WIN_P`` 恒值回退档)——A/B 回退对照臂。
    """

    #: 总开关:True=参数化校准层辖 plane≥2 战斗类结算(绕过 Δ池
    #: plane=2 合并采样——该路径被防饥饿守卫抹平条件性,
    ...
    #: form 键的星级深度折算权重(`w230_star_form/`/:star_depth=上场件
    #: Σ(star−1) 全量口径,同 HandoffSnapshot;core2/board_tier
...
#: 防饥饿守卫门槛(批③ F1:battle 桶6 n=1 恒 -11,把跨深度 6
