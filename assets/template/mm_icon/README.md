...
- **状态（2026-09-12，裁定）**：
  - `mm_ol_24` 界域定锚：已验证（旧图锚点掩码 conf 0.729），可用
  - `mm_ol_50` 商店：已验证（合成黑菱形底座后旧图 conf 0.739）
  - **菱形底座合成路线放弃**（用户裁定：实机裁剪+扣 mask 更省事）——官方图没有菱形底，需合成才能对上游戏渲染的类型（特殊商店/其他 sp 类）一律走实机裁剪（`init_tp_with_background`/`init_sp_with_background` 既有工具），官方图只作设计比对参考
  - 地图跳转：官方图标是蓝菱形新设计、与游戏金色 E 门不同款，用实裁 mm_sp_02
  - 历战余响：官方无图标，用实裁 mm_boss_*