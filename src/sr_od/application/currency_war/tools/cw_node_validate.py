...
身份注记:本模块是**可复用的离线语料校验器**,非一次性审计锚——新语料
重采后仍用它判脏行;其 ``P1_NODE_TEMPLATE`` 是 P1 众数节点词表的单一源,
测试仓 ``test_cw_w336_p1_vd_benefit_calibration.py`` 以同源词表引用本常量,
两处必须同改(同源关系,勿复制词表制造第二源)。
...
- P1 r5(补给)34/34 run 全缺 —— 补给节点无战斗结算屏,outcome 写端只挂
  ...
  W306 后补给完成会写 ``source='synthetic_supply'`` 合成行(带选择/gold 快照),
  旧语料仍缺(消费旧数据时保留本条认知);
