...
把 owned 装备库存网格内的一件拖到目标角色的已穿槽位。词表 = `kernel/cw_vocab.py::WearEquip`(坐标参数化机械动作:字段 `item_name` / `char_name`('' = front-only 回退步)/ `row` ∈ front/back / `slot` = 画面物理槽位 1 基;**不在 `cw_vocab.py::Action` 联合内**——零容器动作转移语义,同「视觉域动作容器零写」契约)。op 载体 = `operations/cw_op/cw_wear_equip_action.py::WearEquipOp`(非终结;体迁自执行器,`prep_actions.py::PrepActionExecutor._wear_equip` 薄委托)。发射形态(R2 穿戴原子通路)= 决策核逐帧发 WearEquip 序,发射序即执行序;计划构造 kernel 单一源 = `kernel/cw_equip_wear_plan.py::_build_equip_wear_plan`(决策侧逐帧现算,构造点唯一;`EquipWearStep` 为动作构造源,branch = m7 角色级分配 / front_only 身份读失败回退)。
...
2. **tracked 双侧**(`apply_op_effect` WearEquip 分支):owned 侧 = `gs.equips` 现读-摘件-回写;tracked 侧 = dep[idx].equips 追加(目标槽位表下标 = `deployed_idx_of(row, slot)`,槽位空 = 跳过);角色装备上限 = `kernel/cw_comps.py::EQUIP_CAPACITY`(计划面容量扣减消费);
...
- `apply_op_effect` 分支 owned 无此件 = 只跳 owned 侧(机械动作仍发出,tracked 侧照记);
...
[../action-logic-state.md](../action-logic-state.md)(视觉域动作容器零写契约);`kernel/cw_exec_state.py::apply_op_effect` WearEquip 分支 docstring(发出即记账形态与 emitted 门);`kernel/cw_equip_wear_plan.py` 模块头(计划构造迁居/构造点唯一);[fields.md](../fields.md) §4.1(穿着即合成豁免)/ §4.2 RunEquip 行(基础行为面);`research/equipment_mechanics.md` §1(穿着即合成/装备上限 3 件)。
