...
> **⚠️ 改任何策略前的强制入口序**(2026-08-20 定调;权威序 2026-09-07 ADR-0482 改两层):①先读
> `game/currency_war/research/user_playstyle.md` **全文**(直觉假设登记簿——**整体考虑全部在册假设及其证明状态,不是逐条打补丁**;策略命题的权威依据=math_proofs 证明与 sim/实机分布实证,未证条目按待证假设对待);②攻略证据以 plaza 精读为准
...
v1 的四个假设被 784 篇投稿深读(r167-r187)与用户系列裁定(r149-r192)推翻:①终局阵容成员写死(实际:核心角色固定,副羁绊按来牌灵活替换);②过渡与终局两套独立数据靠某时刻"切换"(实际:阵容连续演化,开局引擎由来牌决定);③没有"当前阵容能否过接下来的节点"的判断(买/升/存息缺时机依据);④装备与领航员静态绑定(实际:装备公式跟核心驱动型走且有时序,领航员是时间函数)。v2(现行 decision_v2 族)围绕这四点重建。
...
| decision_v2(唯一策略载体) | `decision_v2/`(strategy=独立 DefaultCwStrategy 实现+discipline 纪律族+candidates/filters/scoring/arbiter 四层+remediation 补偿子模块+registry) | 战略层=cw_intention 意向分层(update_target 写 v3_hoard/target_comp=COMP_LIBRARY v2 真 Comp);目标件=hoard_target_set+体系卡引擎件+PLUGIN_LIBRARY;演进=cw_evolution(evolution_step 进决策循环发显式 CompTransaction);纪律族=应急/boss_breaker/carry_gate/掉血三臂(报警梯度:①自然窗→②弃息 D→③位面末 ALL IN,ADR-0313)/C1 溢余必花定向收窄(Δp_board 符号判定;濒死带 C3 同族结构已定谳清理,ADR-0426 增补节)/保血通道(追赶态已随 ADR-0349 退场——人口落后由人口位升级+概率等级窗+EV 总账涌现);**血预算停手·停升级线**(ADR-0448:P1/P2 备战帧 hp≤各自停升级线拒购买经验,与息线门独立谓词 AND 非第五覆盖态,唯一豁免=位面末 ALL IN 窗;接线三面=约束 blood_budget_stop+稳态组+补偿臂①,详见 10 号件 §3);**血预算停手·末窗支出降格/搜索型刷新停付**(ADR-0451:P1 末窗 hp<p1_exit_blood_target 帧承接门定向投资授权豁免通道停(降格为减损保血)+刷新收尾授权前置拒付(急救型/ALL IN 豁免;数值停刷新线挂 β 门),详见 10 号件 §3.1);carry_gate 弱序与保护集口径(ADR-0314;S5 统一弱序见 ADR-0327);**层4 末段补偿趟**(ADR-0326:资源型拒绝结构化捕获 → 同轮单趟 remediation_pass 定向补偿 → 事务性重验 → 重试;liquidity 通道已收编,触发源=working.gold 实际缺口;remedy_buy_tags/remedy_min_score/remedy_alarm_refresh 常量在 registry);**评分活性**(ADR-0332:war 破息窗 50 平台破碎只付真实息损——息崖平滑;未成型+引擎件买入顶正——成型补充偏置;常量 forming_bias/forming_bias_val_max 在 registry);**经济循环总模型**(ADR-0346/0347/0349:相位地板+EV 授权+V_D 批口径三通道调度——refresh 候选=vd_refresh_score 金口径批账(收益=核心 2★ 成型跳变,成本=expected_refreshes×刷价;war/危机同账),升级总账含省刷金项(k 放大),refresh 附庸闸十一参与 refresh_budget 约束退场;**P1 早期新件买入口**(ADR-0372:FORM 地板对配方对缺件的同息档购买放行,`p1_early_gate_enabled` 三常量在 registry)) | [0290](../decisions/0290-decision-framework-candidate-scoring.md)(框架本体)+[0309](../decisions/0309-decision-v2-sole-carrier.md)(载体迁移)+[0313](../decisions/0313-blood-alarm-semantics-final.md)+[0314](../decisions/0314-carry-gate-weak-order.md)+[0326](../decisions/0326-rejection-remediation-loopback.md)+[0327](../decisions/0327-sell-priority-key-unified.md)+[0332](../decisions/0332-forming-scoring-activity.md)+[0336](../decisions/0336-delete-line-strategy.md)+[0347](../decisions/0347-switch-authority-phase-ev.md)+[0349](../decisions/0349-switch-dispatch-vd-batch-caliber.md)+[0372](../decisions/0372-p1-early-buy-gate.md)+[0448](../decisions/0448-blood-budget-levelup-stop.md)+[0451](../decisions/0451-blood-budget-downgrade-refresh-stop.md)+**死亡窗支出分配器**(decision_v2/allocator.py,ADR-0474:停手窗∨死亡域辖域帧,P_t=⟨Π_t,R_t,O,D⟩ 正式模型——三元组公理/两域分域参数集 O 出清/辖域谓词现值直用/帧内精确组合选择;只接管常规管线未支出的坐息攥金帧,记账位=v3_alloc_frame 各渠道获配金;参数集按 P23.2 版本化,弱标定带只作登记数据) · [07](07_plugin.md) |
| 位面 2 支出授权 | `decision_v2/p2_spend_auth` | P2 中段两通道支出授权谓词与预算带单一源:通道 A 定向授权(T1 线锁定[消费 P16 滞回后 v3 locked 位]∧T2 形态缺口[form_ok 锁定线分支取反]∧T4 g≥息线;T3 连败/hp 警戒带升格加急层)+通道 B 止血授权(T3∧T4∧帧资格即触发,与锁定位/形态缺口解耦;方向梯级 v3.2=L1 高价值定向[核心卡/2★ 凑档件/线内 3-5 费件]→L2 定向 D[P12 不变]→L3 低价兜底[仅 L1 全败净支出闸时,梯级=hoard→桥池→[31]① 空窗填充],投影失败帧禁整库保守域,纯散件恒不授权,bench 保留 ≥1 空槽不足降额);**净支出闭环闸**(v3.2:L1 按单笔支出降序累计 Σ>本轮预期收入[基础奖励+连胜档+息,备战帧单一源转发]才获授权放行,目标选择器非收门,「高价值」无新魔数);预算带对齐=常授权层无独立地板(与既有息账门同判「只开门不收门」),加急层破息至保留金公式化下限(5×min(剩余备战轮数,cap),常量 `p2_spend_auth_reserve_rounds_cap`/异常地板 `p2_spend_auth_reserve_floor` 在 registry);消费点=arbiter 金地板加急臂/息账加急豁免/非正分门核心必买,候选评分零触碰(P12 j=0 负例与升级三判据不动);交界=接管帧(分配器 v6 active)禁出手(末窗豁免收窄归此谓词)、economy 存息姿态让位序、血预算门(ADR-0448/0451)=discipline 层不受让位影响;拦断面普查=逐帧拦截原因枚举经 session 披露键进 telemetry 决策迹(协议 M0 分层消费);开关 `p2_spend_auth_enabled` 默认关,开臂=协议 V3 双钥匙(P25 证成∧M0a/M0b/M1-M3 达标护栏全绿,见 ADR 挂账) | [0480](../decisions/0480-p2-spend-authorization.md)/[0481](../decisions/0481-p2-spend-auth-v31-two-channel.md)/[0483](../decisions/0483-p2-spend-auth-v32-net-expense.md) |
| P1→P2 接口机制(**已删**) | `decision_v2/p1_iface` | 五项接口判据面(锁线 v2/carry 装备/遭遇备战授权/连败金流/血线三带,五开关默认关)经 W790/W793 两轮 A/B 定谳无效——触发面全开火后主判据仍双败,模块与消费点(arbiter 授权臂/`p1_iface_gate` 约束/intercept 披露键/equip_all duty 臂/registry 扩格)整体删除;锁线主机制=[23] 锚(cw_intention 信号驱动)不受影响;M3 质量维正信号(板面一致性锁锁错率 2.02%)记档 | [0484](../decisions/0484-p1-iface-v2-impl.md)+[0487](../decisions/0487-p1-iface-verdict-cleanup.md) |
| P1 档位推进目标函数 | `decision_v2/tier_push` | 存量目标型杠杆(P1 评分补项,与配方完成度加权融合不替换):缺口差分项 ΔG·W(r)·V_tier(激活档=deployed 域注册表自算,禁面板快照;成型档组三线=仙舟3/列车2∧DOT2/列车2∧希儿系;逐帧存量差分无跨帧账本;买/升级/部署共用,线外候选经既有 κ 通道降级)→ scoring;散装板硬门(r4 起 max_bond_tier<2 拒零增量散件,豁免与缺口差分同一 dist 函数,压库 ≤2 费/每帧 ≤2 张不穿透 bench 挤占门)→ filters 后置步;r6 预算承诺(EV 门式授权臂,血线辖域门最小实现=state.hp+可信位守卫+`blood_margin_low_hp` 报警线)→ arbiter gold_floor;刷新超几何项(缺档成员 Δp_tier·P·W(r),金水位辖域门沿 W795 同款,Δp_tier 与 realization 单一源共享);伞 `p1_tier_push_enabled`+三子旗标默认关零漂移,开臂判据挂 PREREG_tier_push_AB v2 | [0494](../decisions/0494-p1-tier-push-objective.md) |
...
| 终局意向 | `cw_intention` | 意向分层纯逻辑:信号五层/锁线撤销析取两出口(miss-N 窗口冻结逐出 / 高层信号过可达对照)+停滞评估第三降级转移(P2 锁定态窗级「采购集缺口收敛度」无收敛∧hp 净降∧form 恒未成型→降 weak 带 kind='stagnate' 证据,复用出口①字段契约与既有换线下游;防振荡=单向降级+stagnate_cool 每线每位面一次;salvage=stagnate-weak 的非 absorbing 子模式,hoard mode='salvage' 停线内投入)/降格终局 absorbing;P2 无信号空位弱占位方向(weak_placeholder,hoard mode='p2_weak_target' 优先于⑤兜底,可被任何信号推翻);换线落锁前串联换线存活轮数门(C4,判据单一源 `cw_line_switch.survival_gate`);锁后只输出囤货目标集合写 v3_hoard(买侧唯一消费面,不改板上)(ADR-0319;停滞臂/弱占位=双开关 `intention_stagnation_arm_enabled`/`p2_entry_weak_target_enabled` 默认关,阈值族 stagnate_*/salvage_* 在 registry) | [02 §11](02_comp.md)+[0429](../decisions/0429-gate-v2-line-switch-wiring.md)+[0509](../decisions/0509-stagnation-transform-arm.md) |
