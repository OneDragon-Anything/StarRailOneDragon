...
> 分工分界:分发判定与分支序位的单一源 = [../flow/outer_loop.md](../flow/outer_loop.md) §2(序位冲突必须全局可见,本目录各篇只写画面特有的排他/穿透形态);画面 op 统一规范(单动作决策循环/终结 op/期望态生命周期/对账边界)= [op-layer.md](op-layer.md);基类五段结构与观察解析工具箱 = [op-layer.md](op-layer.md);动作执行契约 = [../flow/action_exec.md](../flow/action_exec.md)。
