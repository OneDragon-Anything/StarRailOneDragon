...
# 必须逐弹窗从画面档取坐标的实证)。id_mark 组合 = 「套组标题 ∧ 合成公式」
# (两处面板结构家具,恒稳;原类别行锚「流派星徽」随徽记类别可变已降为普通
# area):标题锚实测对徽记名前缀变长稳健(「燃血星徽套组」@0.9 命中),
# 合成公式锚位置绑定(角色详情装备子态的同名文本在 (1100,815) 异位,
# 语料反向验证不击穿)。星徽详情已知触发场景全在对局内,入口链无已知触发
