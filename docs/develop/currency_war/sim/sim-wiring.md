...
| bench | 开局 4 张+买入+卖出+上阵;买入/卖出执行 = `cw_state.simulate` 单一源(ADR-0561,引擎只余预检披露+池/账本转录)+上阵 pop(ADR-0271) | 备战栏 SIFT 跟踪 | 已接(ADR-0276 起 merge 同源 `_merge_bench`;合并数入账本 sim.merges) | — |
...
| equips | supply 3选1 采样(池=注册表过滤后的装备名,ADR-0294 件2)+equip_allocation+分配结果回写 BenchChar.equips(ADR-0312,星徽羁绊贡献进 board);卖出回收走 `cw_state.simulate` 单一源(`s.equips.extend`,ADR-0561,C6 守恒对账覆盖 sim 域);带钻是词缀元数据→披露计数 `phantom_supply_picks`,不进 owned 池 | 装备区 OCR;tracked_deployed[].equips(deploy_bench 读回) | 已接(代理) | — |
...
| level_up_cost | LevelUp 花费载体 = `action.cost`(策略 `xp_click_cost` 真值优先,ADR-0561);值仍 4 = 兜底 | OCR 购买经验金币数;xp_click_cost 真值优先 | 未接(sim 4=fallback 值,真值注入待接) | P1 |
...
| active_strategies | **注入通道**(ADR-0364):`simulate_p1(invest=)` 注入——剧本 = `cw_sim_invest.sample_invest_profile(seed)`(plaza 频次)或显式 `SimInvestProfile`;轮收入结算后按日程 append session(去重)+state 镜像;默认 invest=False = 恒 [] 零漂移 | 已持有投资策略;economy 聚合/spend_mode/effect_ledger;经济聚合子集在 sim 生效(息帽/gold_per_node/instant_gold/免费刷) | 已接(注入;默认关) | — |
| active_env | **注入通道**(ADR-0364):同上,开局写 session.active_env + state 镜像 | 已选投资环境(简报);ENV_COMP_AFFINITY/①资格通道 | 已接(注入;默认关) | — |
...
| hp_trusted | 恒默认 False;决策消费经 `hp_readable or hp_trusted`,sim 帧 readable=True 短路 → 行为逐位等价 | hp 值可信位(False=仅开局无真值兜底 100 假值帧;写入端唯一=read_game_state,ADR-0428) | 豁免(同上;FLIP 假帧守卫已按或位兼容 sim 默认态) | — |
...
- **事件金 = 状态分布校准总闸(ADR-0447)**:`EVENT_GOLD_BY_ROUND` 按实机
...
- **boss 胜负面(回退层)= 实机实测阶梯**(ADR-0308):Δ池不可达
