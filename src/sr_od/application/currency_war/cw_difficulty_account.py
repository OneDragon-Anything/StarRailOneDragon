...
**诊断(36 号,历史)**:难度曾「可算不可读」——OCR 常空全链路退先验;读侧已通
(``cw_observation.read_enemy_difficulty`` 旗牌两级管线,58 帧 fixture 对拍
41/41 有旗牌帧读对),本模块主体仍是**记账恒等式**(自持选择自己知道);
OCR 读数经 ``reconcile`` 对账。−18% 敌血 flat 惩罚三态同值(大胜值 0/边际值
一条命/无解值 0)——方向对场合全错;200+ 溢出 bug 使「堆难度」从自杀变
终局武器,系统看不见(读通后可见)。
